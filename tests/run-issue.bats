...
@test "AUTO_SESSION_ID does not fall back to .tmp/auto-session-current when PGID file absent (Issue #1317, no misattribution)" {
    # Issue #1317: run-issue.sh's inline AUTO_SESSION_ID resolve no longer falls back to
    # .tmp/auto-session-current — that file is written only by /auto Step 1, so a wrapper
    # invoked outside /auto (e.g. a manual `/issue` run while another /auto session is
    # active) has no claim to it. Reproduces the same shell snippet as scripts/run-issue.sh
