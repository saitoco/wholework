...
@test "worktree-recovery: SCRIPT_DIR pointing at a removed worktree falls back to main repo and completes trailing steps" {
    # The default no-op $MOCK_DIR/git mock does not implement `worktree`
    # subcommands, so use real git for this test only.
    rm -f "$MOCK_DIR/git"
    ...
    (
        ...
        mkdir -p scripts skills/code
        cp "$MOCK_DIR"/*.sh scripts/
        chmod +x scripts/*.sh
        cp "$BATS_TEST_TMPDIR/skills/code/SKILL.md" skills/code/SKILL.md
        git add -A
        git commit -q -m init
    )
    ...
    # Simulate the linked worktree already having been removed (e.g. by /code's
    # own worktree-lifecycle Exit) by the time run-code.sh captures SCRIPT_DIR
