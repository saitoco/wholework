...
# No fallback to auto-session-current: that file is written only by /auto Step 1, so a
# wrapper invoked outside /auto has no claim to it, and reading it risks misattributing
# a concurrent /auto session's session_id when the PGID file cannot be resolved
# (Issue #1317). If the PGID file is absent, AUTO_SESSION_ID resolves to empty (fail-closed).
