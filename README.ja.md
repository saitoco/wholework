...
3. **エージェンティック実行** — `/auto` は Claude Code を long-horizon なコーディングエージェントに変えます。1 本の Issue、サブ Issue に分解する XL Issue、Backlog 全体のどれでも引き受け、必要な時間だけ全フェーズを無人でオーケストレーションし続けます。
4. **手持ちのツールで動く** — 既存の GitHub リポジトリと標準的な GitHub Flow に沿って動作します。どのフェーズでも介入できます。
