...
- `scripts/check-skill-change-observation-ac.sh` — Issue 本文が `skills/*/SKILL.md` を変更する場合に、post-merge の `verify-type: observation` AC から `session=next` 未付与を検出；`skills/issue/SKILL.md` Step 4 から warn-only で呼び出される
