...
- `scripts/resolve-merge-strategy.sh` — `.wholework.yml` の `merge-strategy` キーを戦略名または `gh pr merge` フラグ (`--flag`) に解決する。不正値・特殊文字は `squash` にフォールバック (stderr に警告。呼び出し元が LLM エージェントであるため、値は印字可能文字 40 字に切り詰めてから出力する)、真に空の値は警告なしで `squash` にフォールバックする (`get-config-value.sh` 自身のデフォルト置換により吸収され、未設定と区別できないため)。`get-config-value.sh` 失敗時は依存コマンド失敗を明示する専用の警告付きで `squash` に fail-closed する。`skills/merge/SKILL.md` Step 2 から呼ばれる
