...
├── modules/             # スキルから参照される共有モジュール（46 ファイル）
│   └── <module-name>.md
...
- `modules/verify-classifier.md` — マージ後条件の検証可能性分類（同じ `manual` タグ値が、別経路 (`/issue`) 経由でマージ前条件にも付与される — 詳細はモジュール本文を参照）
...
- `scripts/detect-unrecorded-kills.sh` — `.tmp/auto-events.jsonl` の `phase_start` 重複 (respawn シグナル — 連続するペアの間に `wrapper_exit`/`phase_complete`/`manual_intervention` が存在しない) を `docs/reports/orchestration-recoveries.md` のエントリと `--window` 秒 (既定 300 秒) の許容差で突合し、未記録の external kill をバースト単位で検出する。`/verify` Step 15 から呼ばれる (Issue #1387)
