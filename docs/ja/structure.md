...
│   ├── spec/            # Issue 仕様
│   ├── reports/         # 最適化・監査レポート（routines-adoption.md を含む — 追わない方針、記録として保持）
...
- `modules/orchestration-fallbacks.md` — orchestration レベルの fallback パターン参照カタログ（#319 tier 2・#316 recovery sub-agent・#318 learning loop から参照）。発火実績ゼロかつ live 参照元なしで退役したエントリは `docs/reports/orchestration-fallbacks-archive.md` へ退避 (#1180)
- `modules/ci-failure-classifier.md` — CI プラットフォーム障害判定の SSoT（判定シグネチャ表、3 値 verdict、消費側ごとの応答表）
- `modules/domain-classifier.md` — 改善提案の Domain 分類（composable、LLM-in-context）
- `modules/retro-proposals.md` — Improvement Proposal 収集、Tier 判定（retro_proposal_classified イベント emit 付き）、Issue 作成（/verify Step 16、/auto Step 4a、/auto Step 5 の共有モジュール）
...
- `scripts/append-consumed-comments-section.sh` — フェーズ自身の作業ブランチ上で Spec へ `## Consumed Comments` を追記; `skills/spec/SKILL.md` / `skills/code/SKILL.md` / `skills/verify/SKILL.md` (いずれも `--no-push` 付き) から in-session (必須) で呼び出される。加えて run-spec.sh / run-code.sh からの post-processor フォールバック (pre/post カウント比較) があるが、`/code` pr route ではゲートオフされる (#1058)
...
- `scripts/collect-verify-retention-stats.sh` — verify-type 別に `phase/verify` の滞留状況 (post-merge AC がどれだけ未解決で残っているか、どれだけ解決済みか、未解決分の滞留日数) を計測する。比較ウィンドウ日付フィルタをオプション指定可能。`/audit stats --retention` 向け
...
- `scripts/verify-executability-marker.sh` — `/verify` Step 8b の Claude 実行可否判定を記録する `type=verify-executability` マーカーを生成 (`format`) ・解決 (`resolve`、latest-wins) する
...
- `scripts/resolve-batch-query.sh` — `/auto --batch --until <query>` の `label:`/`status:` クエリを解決し、一致する open Issue 番号を昇順・除外集合適用済みで出力する（決定的処理、bash 3.2 互換。Issue 単位の Status GraphQL 解決失敗はハードエラーではなく fail-closed で除外扱いにする）
...
- `scripts/reclaim-stale-worktrees.sh` — 完了済み (CLOSED/MERGED) の Issue/PR に対応する stale worktree と孤児ブランチを棚卸し・回収する。既定は dry-run、`--apply` でローカルの削除を実行。並行セッション除外 (locked かつ HEAD が main と一致)、未コミット変更の保護、squash merge されたブランチの安全な削除 (`git branch -d` が拒否された場合、MERGED PR の `headRefOid` とブランチ tip が一致するときのみ `-D` にフォールバック) を備える。`origin` 上に残留した孤児 `worktree-*` ブランチも回収対象: 常に列挙され (dry-run レポート)、`--apply-remote` (`--apply` から独立) を指定した場合のみ実際に `git push origin --delete` を実行する。安全策はローカルと同水準 — ローカルにチェックアウト中のブランチは除外 (ローカル回収パスに委譲)、`kind=issue` は `origin/<default-branch>` の祖先であることを確認、`kind=pr` は MERGED PR の `headRefOid` と完全一致することを確認する
