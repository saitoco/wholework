...
**`--batch --until <query>`**: 条件駆動のバッチモードです。クエリは内部で空白区切りされるため、必ずダブルクォートで囲んだ単一引数として渡してください (例: `/auto --batch --until "label:retro/*"`)。クエリ文法は空白区切りの `key:value` 句で、対応する key は 2 つのみです — `label:<glob>` (必須。Issue のラベル名に対して照合し `*` glob 可) と `status:<value>` (任意。Project v2 の Status フィールドに対する完全一致・大小文字区別あり。`In progress` のような空白を含む値は非対応でパースエラーになる)。各ラウンドで `scripts/resolve-batch-query.sh` によりクエリを解決し、一致した Issue を `--batch N1 N2 ...` (List mode) と同じ手順で処理したうえで再度クエリを解決し、これを繰り返します。クエリが 0 件を返した時点でループは収束します。同一セッション内の前ラウンドで complete / fail 済みの Issue は以降のラウンドから除外されます (blocked-by ゲートでスキップされた Issue のみ例外で、同一セッション中に blocker が解消しうるため次ラウンドで再評価されます) — この除外がないと label 単独のクエリでは完了済み Issue を再ヒットし続け、収束しません。収束とは独立した安全弁として `--max-rounds <N>` (既定 3) がラウンド数の上限を課し、無効化はできません。`--checkin-per-round` (opt-in) を指定すると、対話モードでは次ラウンドに進む前に AskUserQuestion で確認を求めます。`--non-interactive` では警告のうえ無視されます。`--batch --until` 実行の中断後に `--batch --resume` で再開すると、最終ラウンドの `remaining` を単発の List mode バッチとして処理するのみで until ループ自体は再開されません。ループを継続したい場合は `--batch --until` を再実行してください。
