...
Core Phases を順次連鎖させるオーケストレーター。各フェーズは独立した `claude -p` プロセスとして実行され、コンテキスト分離を保証します。デフォルトは `--permission-mode auto` を使用。`.wholework.yml` に `permission-mode: bypass` を設定すると `--dangerously-skip-permissions` を使用します（詳細: [SECURITY.md](../../SECURITY.md)）。`/auto 123 [--patch|--pr] [--review=full|--review=light]` で Size ベースのルーティングを含む E2E ワークフローを駆動します:
...
`phase/ready` が無い場合 `/auto` は `/spec` を先に自動実行します。`phase/*` ラベルが全く設定されていない場合は issue triage/refinement から始めます。**fix-cycle 例外**: (a) `wholework-event: type=verify-fail` marker comment が存在、(b) `phase/*` ラベルが全て削除済み、(c) Spec ファイルが存在 — 3 条件をすべて満たす場合、`/auto` は fix-cycle state と判定して issue/spec phase を skip し、直接 code phase を実行します（Step 2a）。詳細: [`skills/auto/SKILL.md`](../../skills/auto/SKILL.md)
...
**`--resume N`**: 中断された単一 Issue を再開します。現在 phase の権威ソースは GitHub labels + `reconcile-phase-state.sh`（reconciler-first）です。チェックポイントファイル（`.tmp/auto-state-N.json`）は verify iteration カウンタのみを保持します。phase 状態はライブのラベルから再読み込みするため、正しいステップが自動検出されます。主な用途: verify ループ中断後のカウンタ復元。
...
### `/audit` — ドリフト・フラジリティ・Recovery 検出

`/audit drift` は AI を用いて Steering Documents + Project Documents とコードベース実装の意味的乖離を検出し、コード側修正の Issue を自動生成します。最良の結果を得るには、まず `/doc sync --deep` を実行してドキュメント側の drift を吸収してから `/audit drift` を実行することを推奨します。これにより `/audit drift` をドキュメント更新だけでは解消できない意味的乖離の検出に集中できます。`/audit fragility` は構造的に脆弱な箇所（コアモジュールのテスト欠如、Architecture Decision 違反など）を検出しリスク改善 Issue を生成します。`/audit`（引数なし）は drift + fragility の両観点を一緒に実行します。`/audit stats` はプロジェクト全体の Issue メタデータ（スループット、構成、First-try 成功率、Backlog Health など）を集約してプロジェクト健全性診断レポートを生成し、drift と fragility に並ぶ第 3 のレンズとなります。`/audit stats --retention` は phase/verify 滞留メトリクスと Icebox 滞留メトリクスをエスカレーションベースの retire 提案コメント投稿とともに追加します。`/audit recoveries` はクロス Issue オーケストレーション recovery ログ（`docs/reports/orchestration-recoveries.md`）を読み込み、頻度閾値（デフォルト: 3 回）を超えた再発パターンを Issue 化して運用知見を構造的改善へ変換します。`/audit progress <XL-親 Issue 番号>` は指定した XL 親 Issue 配下の sub-issue 進捗スナップショットを表示します: ステータス内訳（Done / In progress / Blocked / Stale / Pending）、フェーズ分布、時間推定（完了済みの中央値 + 残時間推定）、直近 24h アクティビティを 1 コマンドで確認でき、`gh issue list` を繰り返し目視する必要がなくなります。`/audit auto-session <session-id>` は `.tmp/auto-events.jsonl`（`session_id` でフィルタ）から `/auto` セッションの retrospective レポートのデータ層を生成します: Summary メトリクス、Per-Issue 所要時間、Recovery Events、Verify 残留、Concurrent Sessions、改善候補 — Narrative skeleton（手動補完）付き。`/audit auto-session --full <session-id>` を指定すると全 4 セクション（What worked / Limits and gaps / Improvement candidates surfaced / Conclusion）の LLM draft を生成し、`[LLM draft — human review required]` マーカーと改善候補分類（既存 Issue / 新規起票候補 / 凍結推奨）を挿入します（Issue 自動起票は行わず human gate を維持）。`--no-ja` を付与すると日本語 sibling ファイルの生成をスキップします。期間集約モード（`--day YYYY-MM-DD`、`--since {N}d`、`--range START..END`）は複数セッションを横断するクロスセッションナラティブレポートを `docs/sessions/_period/` に生成します: Sessions covered / Cross-session patterns / Improvement candidates / Trend — 週次・月次定期点検に活用できます。詳細: [`skills/audit/SKILL.md`](../../skills/audit/SKILL.md)
