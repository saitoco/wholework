...
**`--batch N`**: バックログから新しい順に N 個の XS/S Issue を一括処理します。**`--batch N1 N2 ...`**: 指定した Issue 番号を空白区切りで列挙し、ユーザ指定順に処理します（XL 以外の全 Size を受け入れ）。`run-auto-sub.sh` が各 Issue で成功した後、親セッションがラベルを再フェッチし、`phase/verify` が付与されていれば親セッション内で `/verify` を呼び出します。これにより検証可能な Issue は同一バッチ実行内で `phase/done` に到達できます。`--non-interactive` など非対話実行時は verify ステップをスキップし、`phase/verify` 残しで継続します。
