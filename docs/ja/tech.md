...
- **`/auto` スキル**: `run-*.sh` 経由で spec→code→review→merge→verify を順次連鎖させるオーケストレーター。各フェーズは設定可能なパーミッションモード（デフォルト: `--permission-mode auto`、`.wholework.yml` に `permission-mode: bypass` を設定すると `--dangerously-skip-permissions`）で `claude -p` 独立プロセスとして実行され、フレッシュなコンテキスト分離を保証する。`verify-max-iterations`（デフォルト: 3、最大: 20、`.wholework.yml` で設定可能）が verify-reopen ループを上限で止める。カウンターが上限に達すると Issue は `phase/verify` に留まり人間の判断を待つ。`/auto` スキルは verify 出力の `MAX_ITERATIONS_REACHED` を検出し、無限ループの代わりに連鎖実行を停止する。フラグ動作・バッチ処理・レジューム・リリースブランチワークフローは [docs/workflow.md § Orchestration](../workflow.md#orchestration) を参照。
  - **2 階層オーケストレーション**: `/auto` 本体（親オーケストレーター）はユーザーの Claude Code セッションで動作し、LLM 推論を使った適応的判断を行う（ラベル状態の評価、サイズベースのルーティング、サブ issue 依存関係分析）。XL Issue については `run-auto-sub.sh`（子オーケストレーター）が各サブ issue のフルフェーズシーケンスを実行する。`run-auto-sub.sh` は bash オーケストレーションを維持しつつ、段階的適応リカバリを備える: (1) `reconcile-phase-state.sh` 完了チェック、(2) `apply-fallback.sh` 既知パターンリカバリ、(3) 未知異常時に `spawn-recovery-subagent.sh` が `claude -p` で `agents/orchestration-recovery` を起動。通常経路は bash のままコストと並列安定性を維持し、Tier 1–2 が失敗した場合のみ `claude -p` を診断に使用する。`WHOLEWORK_MAX_RECOVERY_SUBAGENTS` cap（デフォルト 1）により並列コストを制限する。
    - **親セッション主導の手動再スポーン (Tier 1/2/3 機構の外側)**: バックグラウンドの `run-*.sh` wrapper 自身のプロセスグループが外部から kill された場合 (`modules/orchestration-fallbacks.md#external-kill-parent-respawn` 参照)、上記のいずれの Tier も発火できない — Tier 3 の Layer B リトライは、kill されるのと同じプロセスグループ内で動作しているためである。この kill を観測して再スポーンできるのは親 `/auto` セッションのみであり、`run-auto-sub.sh --write-manual-recovery` (`#manual-recovery-spec-write` 参照。任意の `--cause SLUG`/`--diagnosis TEXT` フラグを指定すると機械可読な cause 行が追加され、`collect-recovery-candidates.sh` が同一 symptom で原因の異なるイベントを別集計できる — Issue #1123。任意の `--notification CLASS` フラグ (`harness-stop`/`external-signal`/`indeterminate`/`unobserved`) は、親セッションが観測した停止したバックグラウンドタスクの task notification 文言の分類を記録する — Issue #1153) 経由で recovery を記録する。記録先は `docs/reports/orchestration-recoveries.md` と `manual_intervention` イベントであり、Tier 1/2/3 の語彙・Metrics とは別系統である。第二の呼び出し元として、backstop 目的で `skills/verify/SKILL.md` Step 12 が同じ `--write-manual-recovery` 呼び出しを事後的 (merge 後) に行う場合がある — `orchestration-recoveries.md` にも既存の Spec エントリにも記録されていない Manual recovery を検知した際、復旧作業そのものの最中にこの real-time hand-off が見落とされたケースの穴を埋める (Issue #1049)。
...
- **`/triage` スキル**: メインワークフロー開始前に Type/Priority/Size/Value/Theme を割り当てる初期 Issue 評価。タイトル正規化（`modules/title-normalizer.md`）と 4 視点の深い分析（`/triage --backlog`）を実行する。Issue に `phase/*` ラベルがない場合、`/auto` が自動的に `/triage` を連鎖させる
...
- **operate route (diff-less workflow path)**: Size とは直交する `/code` の第三の route。Spec がリポジトリ内ファイル変更を伴わない外部ツール操作 (CMS 編集・インフラ操作) のみを記述している Issue が対象で、Spec の `## Changed Files`/`## Implementation Steps` から機械的に判定され、新しいラベルや Issue メタデータは追加しない。新規 Skill ではなく `/code` の既存 patch/pr 分岐点への 3 値目として実装した — #437 の教訓 (新メカニズムより既存パターン流用を優先) に従ったもので、独立 Skill 案は追加で 6〜8 件の SSoT ドキュメントに影響していた。結果は git diff の代わりに `## Execution Log` Issue コメントとして記録される。`phase/code` → `phase/verify` のラベル遷移は patch route と同一で、PR が存在しないため `/review`・`/merge` はスキップされる。operate route は `closes #N` コミットを一切生成しないため、`code-patch` の completion signature (`scripts/reconcile-phase-state.sh`) は Execution Log/Execution Plan マーカーコメントも代替の成功シグネチャとして受理する — 詳細: `modules/phase-state.md` § "Operate Route Completion Signature"。詳細: `modules/size-workflow-table.md` § "Diff-less Axis (operate route)"
...
- **Autonomy tier（L0 書き込みガバナンス）**: Wholework は 4 層で動作する — L0（GitHub state: Issues, Labels, PRs, blockedBy, `closes #N`）、L1（Claude Code primitive: `/loop`, `/goal`, `ScheduleWakeup`, `CronCreate`）、L2（Wholework skill 内部: Spec, retro, `auto-events.jsonl`）、L3（OS スケジューラ — 現時点で Wholework のいかなる機構でも使用されていない。`CronCreate` は上記 L1 に分類済みで最も近いプリミティブだが、それ自体 session-scoped/in-memory であり OS レベルの永続スケジューラではない — 詳細は `modules/autonomy-tier.md` 参照）。`.wholework.yml` の `autonomy:` field は、skill が L0 をどこまで書き込め、L2→L1 経路（A Advisory / B CronCreate / C ScheduleWakeup）をどこまで使用できるかを宣言する。tier × 経路マトリクスと Tier × L0 書き込みマトリクスの SSoT は [`modules/autonomy-tier.md`](../../modules/autonomy-tier.md)、surface 別の L0 書き込み分類は [`modules/l0-surfaces.md`](../../modules/l0-surfaces.md)。このレイヤーは `permission-mode`（Claude Code subprocess 権限を管理）とは直交する。Skill frontmatter `loop-paths-used` は skill が使用する L2→L1 経路を宣言する（例: `loop-paths-used: [A]` = advisory print のみ）; #700（`/verify` auto-retry-on-fail tail 拡張）で実装済み。
...
| audit（skill） | audit | Sonnet | — | drift 検出 (`drift`)・脆弱性解析 (`fragility`)・プロジェクト健全性統計 (`stats`)・XL サブ Issue 進捗 (`progress`)・/auto セッションレトロスペクティブ (`auto-session`)・Issue 前提失効 (`premise`); Sonnet で十分。インライン実行 (`run-*.sh` ラッパーなし) のため effort は設定しない |
...
**Fable 5（Mythos クラス）**: Fable 5（`claude-fable-5`）は Opus より上のティアであり、`opus` エイリアスでは**到達できない** — 明示的なモデル文字列 `claude-fable-5` が必要。以下のハード制約によりデフォルトモデル変更ではなく**オプトイン**のみ許可: コスト $10/$50 per MTok（Opus 4.8 の 2 倍、Sonnet 4.6 の 3.3 倍）、30 日 retention 必須（ゼロデータ保持 org は非対応）、サブスクリプションの usage credit ゲート。Fable 5 オプトインを公開するスキル（例: `/spec --fable`）はスキル単位でドキュメント化する。採用ガイダンスは `docs/reports/claude-fable-5-impact-strategy.md` §3.3 および §5.2 を参照。Fable 5 上で実行する場合、レビューフェーズのセキュリティ関連クエリは cyber classifier によって Opus 4.8 へ自動ルーティングされる可能性がある（CLI 経由では透過）— Fable 5 がセキュリティ分析を直接処理することを前提としないこと。
...
- **watchdog タイムアウトのキャリブレーション**: `scripts/watchdog-defaults.sh` のフェーズ別タイムアウト定数は、支配的な親モデルの per-token レイテンシに対してキャリブレーションされている。デフォルト親モデルが変更された場合は再キャリブレーションが必要（例: Fable 5 → Sonnet 4.6 移行で `WATCHDOG_TIMEOUT_ISSUE_DEFAULT` を 600 → 1200 に引き上げ: #628）。**#903 再校正 (Sonnet 5)**: #878 が実測した Sonnet 5 の ~1.3-1.4× トークナイザー比を受け、実際の `/code`/`/review` 本番 wall-clock サンプル (n=10 / n=9、`docs/reports/sonnet-5-watchdog-recalibration.md`) を計測したところ、両フェーズとも p95/max がタイムアウトの80%超という再校正基準に既に達していた (code: p95 81.3%、max 93.8% (対 3600秒); review: p95 92.2%、max 100.2% (対 2000秒) — 実サンプル1件が既にタイムアウト上限に到達)。`WATCHDOG_TIMEOUT_CODE_DEFAULT` を 3600→**4680**、`WATCHDOG_TIMEOUT_REVIEW_DEFAULT` を 2000→**2600** に引き上げた (いずれも #878 実測比率の保守側である ×1.3。#628 の 2× precedent よりあえて控えめとし、Icebox #596 が指摘するタイムアウト引き上げと真のスタック検知遅延のトレードオフに配慮)。`WATCHDOG_TIMEOUT_SPEC_DEFAULT` / `_ISSUE_DEFAULT` / `_MERGE_DEFAULT` は #903 のスコープ外 (根拠はレポート参照)。**#939 (`WATCHDOG_TIMEOUT_SPEC_DEFAULT` 再確認、`--opus` 代理計測)**: Fable 5 の実トラフィックは Issue #560 の1件 (テレメトリ未取得) のみで変わらず (`docs/reports/watchdog-recovery-strategy.md` § 2026-07 re-measurement 参照)。2026-08-07 の Issue Retrospective により、AC1/AC2 の許容エビデンスが `--opus` 実トラフィックも代理指標として認める範囲に拡大された。`/auto` は Size L の spec dispatch に `--opus` を自動付与し、同じ `WATCHDOG_TIMEOUT_SPEC_DEFAULT` 予算を共有するためである。実測 (`docs/reports/watchdog-recovery-strategy.md` § 2026-08 re-measurement、N=20、2026-06-28〜2026-08-07、issue+session_id で join した `size=="L"` の `sub_start` と `phase=="spec"` の `max_silent_window`): min 660s / mean 978.5s / **p95 1340s (対 1800s で 74.4%)** / max 1460s (81.1%)。post-Opus-5 部分集合 (2026-07-24以降、N=11) は max 1340s (74.4%)。この集合内で `phase=="spec"` の `watchdog_kill` は0件。**判定: `WATCHDOG_TIMEOUT_SPEC_DEFAULT=1800` を据え置き** — #903 の再校正基準 (実測がタイムアウトの80%以上に達したら引き上げを検討) を適用すると、p95 (74.4%) は閾値未満であり、唯一80%を超える max (81.1%) は 2026-07-05 の Opus 5 以前の単発サンプルで、反復パターンではない — post-Opus-5 部分集合のみでは 74.4% どまりである。`tests/watchdog-defaults.bats` への変更なし (値は未変更)。**フォローアップ (#1301)**: その後の 2 セッションにわたる `/auto --batch` 実行 (2026-08-08、2026-08-09) で、`phase=="spec"` の閾値超過 5 回・`watchdog_kill` 1 回を記録し、クリーンサンプルの余裕は既定 1800s に対し最小 10 秒まで縮小した — 本判定自身が設定した「再発時に再評価」という条件を満たした。グローバル既定値の改定ではなく、`#1301` では `.wholework.yml` に project-local override (`watchdog-timeout-spec-seconds: 2340`、#903 と同じ ×1.3 係数) を追加した。これは `#939` の review フェーズパスと同じ「即時 override → 後で昇格を検討」という順序 (`PR #1201` → 本エントリ) を踏襲している。

**#939 (`WATCHDOG_TIMEOUT_REVIEW_DEFAULT` 再校正、Issue #1058 / PR #1201)**: 2026-08-06 の本番実測 (Issue #1058 / PR #1201) で、当時の `WATCHDOG_TIMEOUT_REVIEW_DEFAULT=2600` において `watchdog_kill` (`silent_window_sec=2600`) が発生した。直後の再実行は `.wholework.yml` の project-local override `watchdog-timeout-review-seconds: 5400` の下で行われ、`max_silent_window=4110s` (対 5400s で 76.1%)・総所要 ~4303s で完走した。#903 と同様の ×1.3 係数を実測 4110s に適用すると (4110×1.3≈5343s)、すでに本番検証済みの override 値 5400s の範囲内に収まるため、新たな値を再導出するのではなく本パスで **`WATCHDOG_TIMEOUT_REVIEW_DEFAULT` を 2600→5400 に引き上げる**。透明性のため反証データも記録する: 同日、Issue #1214 (PR #1216) で新設定の 5400s ちょうどで `watchdog_kill` が発生したが、直後の再実行は 1250s (対 5400s で 23.1%) で完了しており、真に追加の計算時間を要したのではなく stuck/hang による kill だった可能性が高い (Icebox #596 の timeout-inflation-vs-stuck-detection トレードオフ)。この1件はさらなる引き上げの根拠にはしない。`tests/watchdog-defaults.bats` に review フェーズ用の新規 `@test` (既存の code フェーズ用テストと同型、期待値 `5400`) を追加する。同レポートには #878/#903 で挙がった2件の **prompt slimming** 候補の検討結果も記録されている: `/auto` の L3 auto-retrospective「notable judgment」ステップ (完全に機械的な判定のためにセッション全体の `events.jsonl` を丸ごとコンテキストに注入していた — #913 で生ダンプを `jq -sc` によるイベント件数集計に置き換えて対応済み) と、`/issue`/`/review` の L/XL parallel investigation sub-agent input (diff・変更ファイル全文を使用 — スコープ/リスク/バグ検出の精度低下リスクがトークン削減効果を上回るため slimming 不要と判断)。
- **code/spec フェーズ自動リトライ (silent no-op)**: `auto-retry-on-fail.enabled: true` かつ `autonomy: L2/L3` の場合、`run-code.sh` と `run-spec.sh` (#1369) はそれぞれ `reconcile-phase-state.sh` から `matches_expected: false` (silent no-op) を検出した際に内部でリトライする。最大リトライ数は `auto-retry-on-fail.max_iterations` から取得 (レガシーキー `threshold` も受け入れ; デフォルト: 3)。リトライカウンタ (`CODE_RETRY_COUNT` / `SPEC_RETRY_COUNT`) は `exec` ベースの再起動を通じて export された環境変数で引き継がれる。`skills/verify/SKILL.md` Step 11(b) の verify 側自動リトライと対称的 (同じ tier ゲート: L2/L3 + `AUTO_RETRY_ENABLED=true` + count < max)。ただし verify 側の tier ゲートには documented-deferral escape hatch (#947) が前置されている点が異なる: FAIL が意図的かつ既に文書化された deferral であると検出された場合 (marker 属性 `deferral=true`、または Spec の retrospective/Phase Handoff セクション経由)、tier/config/count に関わらず tier ゲートを無条件でスキップする。組み込みリトライが有効な場合、`apply-fallback.sh` の `code-patch-silent-no-op` Tier 2 ハンドラは二重リトライを防ぐため抑制される (spec フェーズにはそもそも Tier 2 ハンドラが存在しない — `modules/orchestration-fallbacks.md#auto-retry-on-fail-code_retry_fire` の Phase Scope Decision を参照)。`exec` ベースのリトライ再実行の直前に、preflight ステップが silent no-op 自体の副産物として残った parent-main の untracked file を stash へ退避する (`git stash push --include-untracked`、`docs/sessions/**` は除外) ため、リトライ側の `check-verify-dirty.sh` 再チェックが直前の試行自身の stray output でブロックされることを防ぐ (#886)。同じく `exec` 直前に、各 wrapper はリトライ発火を `docs/reports/orchestration-recoveries.md` へ記録する (`modules/orchestration-fallbacks.md#auto-retry-on-fail-code_retry_fire` を参照) — `exec` によりリトライ後のプロセスは直前の失敗を記憶できないため、`wrapper-retry-on-kill` 側の記録との非対称性をこれで解消する (#1320、#1369 で `run-spec.sh` にも拡張)。`code-pr` フェーズに限っては、上記のリトライ適格性チェックに入る前に、`run-code.sh` はまず `reconcile-phase-state.sh` の出力に `worktree_commits_found:true` が含まれるかを確認する — これは worktree ブランチに既にコミット済み (未 push) の実装が存在することを意味する。該当する場合、`exec` ベースのリトライは無条件でスキップされ、wrapper は即座に `EXIT_CODE=1` で終了し、実装をゼロからやり直して既存の作業を破棄する代わりに Tier 1/2/3 の復旧に委譲する (#1391、`code-pr-tier3-recovery` の #1224 再発を受けた対応)。
- **costly/irreversible な Implementation Step のマーキング (#951)**: `/spec` は非対話で costly (実費/非自明なトークン消費を伴う) または irreversible (production への副作用を伴う) な Implementation Step を pre-authorize し、`/code` (同じく非対話・`AskUserQuestion` 不可) がその判断を独自に再検討して deferral する、という再発パターンが2回発生した (**#903**: Spec で pre-authorize された新規ベンチマーク実行を `/code` がコスト理由に却下、**#939**: Spec で pre-authorize された新規 `--fable` 実行を `/code` がコスト・副作用・authorization 理由に deferral)。いずれも acceptance criteria は当該ステップが実行された前提のまま残された。**採用: (a) `spec-approval-needed` マーカー + (c) Deferral Protocol 必須記載** (`modules/costly-step-protocol.md`) — `/spec` が costly/irreversible なステップにマーカーを付与し、deferral 時に `/code` が代わりに何を行うべきかを記録する。`/code` はマーカーを検出し、非対話モードでは独自に再判断せず High-Stakes Decision (`modules/ambiguity-detector.md` の skip tier) として扱う。**不採用: (b) `/code` から親セッションへの interactive fallback** — `/code` の非対話実行 (`run-code.sh`) は live な interactive 親セッションが存在する保証がない設計 (`/auto --batch` やスケジュール実行など、無人実行が正常系に含まれる) であり、「親セッションへの fallback」という新チャネルはこの前提と矛盾する。既存の Three-Tier Policy の skip tier がこの種の問題を新規メカニズムなしにそのまま扱える。本 Issue は **#947** (`/verify` の documented-deferral escape hatch、Step 11(b)) の producer 側対応にあたる: #947 は `/verify` 時点で既に発生した deferral を reactive に検出するのに対し、#951 は `/spec`/`/code` 時点で曖昧さそのものを proactive に防ぐ。両者は `## Code Retrospective` > `### Deviations from Design` という同一の記録先を共有するため、#947 側の検出ロジックへの変更は不要だった。
- **`recoveries-auto-fire` の既定 opt-out 化 (#1179)**: 本リポジトリ自身の `.wholework.yml` における `recoveries-auto-fire` の opt-in (`enabled: true`) を撤回し、配布デフォルト (`enabled: false`、`modules/detect-config-markers.md` 参照) に戻した。2026-08-05 時点の実測で、open Issue の 81% (83件中67件) が `retro/verify` であり、`orchestration-recoveries.md` エントリの58%が `parent-session-manual-recovery` — external kill によりフェーズが落ちた後の定型的な再実行であった。根本原因は上流 `anthropics/claude-code` の未対応 Issue にあり、本リポジトリの実装では解消できない (`docs/reports/external-kill-investigation.md`)。閾値の引き上げ (3→8) は発生ペース次第で同じ状態に戻るため構造的な解決にならないとして不採用、#1123 の group-key 細分化の撤回は #1181 の `--cause`/`--diagnosis` フローにとって独立した価値があるため不採用とした。`/verify` Step 15 の自動起票分岐ロジック自体は変更していない — 変更したのは本リポジトリの opt-in 設定のみである。頻度の可視化は失われていない: `bash scripts/collect-recovery-candidates.sh docs/reports/orchestration-recoveries.md --threshold 1` または `/audit stats --retention` (Section 10: Recovery Candidate Frequency) で group-key ごとの発生回数を引き続き参照できる。
- **Sonnet 5 effort 再校正 — code/review (#921, C2)**: Sonnet 5 の effort curve widening (impact strategy レポート §3.3/§4.2) を踏まえ、`run-code.sh`/`run-review.sh` の effort を `high` から `medium` に下げられるか再評価した。**判定: 両方とも `high` を維持**（詳細分析: `docs/reports/sonnet-5-effort-recalibration-code-review.md`）。`run-code.sh`: impact strategy レポートが `medium` 候補として挙げていたのは XS/S patch-route Issue に限定されていた (§4.2) が、`--effort` フラグは Issue サイズによる条件分岐のないグローバル設定であり、#229 の手戻りリスクの論拠 (14 ステップの推論チェーン、sub-agent への fan-out なし) はモデル世代交代によって変化しない。`run-review.sh`: orchestrator は dispatch 以外にも実質的な推論作業を行っている — Step 7.2/7.4/7.6 は外部レビューのフィードバックを解釈して fix コミットを作成しており、`run-code.sh` 自身の実装推論と同種の作業である — これは impact strategy レポートの「mechanical」という位置づけと矛盾する。加えて `review-bug`/`review-spec` (Opus) は orchestrator セッションから effort を継承する (Claude Code CLI changelog で確認: エージェントレベルの `effort:` frontmatter override が未設定の場合、sub-agent はセッションの extended-thinking/effort 設定を継承する。`agents/review-bug.md`/`review-spec.md` には現状この override が設定されていない) ため、降格すると精度が重要なこれらの sub-agent の推論深度も暗黙に低下する。両判定は `docs/reports/sonnet-effort-recalibration.md` (#229、2026-04-18、Sonnet 4.6 ベースライン) を Sonnet 5 の観点から再確認するものである。**フォローアップ (#1063 で実装済み)**: `agents/review-bug.md`/`review-spec.md` (および上表の他の Opus/Sonnet sub-agent) に `effort: high` frontmatter が追加され、orchestrator の設定から effort が分離された — 現状は上表の各行を参照。
...
- **Opus 5 effort 再校正 — `run-spec.sh --opus` (#1064)**: `run-spec.sh` の Opus fallback パス (`--opus`、Size L の Issue 限定、#217 で Opus 4.8 のガイダンスに基づき設定した `xhigh` デフォルト) が、Opus 5 のガイダンス変化 (上記「Opus 5 effort calibration」ノート参照: coding/agentic タスクは `xhigh` から開始し、そこから下方 sweep を推奨。`low`/`medium` は想定以上に強いと報告されている) の下でも引き続き妥当かを再評価した。本 Issue は `#922` (C3、Sonnet デフォルトパスを評価し、Opus fallback を「転用不可能な根拠」として明示的にスコープ外とした) の対称的なカウンターパートである。**判定: `xhigh` を維持** (詳細分析: `docs/reports/opus-5-effort-recalibration-spec.md`)。`#922` では唯一記録されていた diminishing-returns 根拠が Opus 4.8 に限定されており Sonnet 5 には転用できなかったのに対し、本 Issue では Opus 5 の下方 sweep ガイダンスは実際にこのパスが動作しているモデル (Opus 5 自身) に直接該当する。しかしこのガイダンスは「sweep して確認せよ」という探索的な推奨であり、このワークロードにおいて特定の下位 tier が既に `xhigh` と同等であるという主張ではない。また `--opus` は Size L の Issue に限定されている — これは構造的に最も複雑な Issue の部分集合であり、Sonnet デフォルトパスでは推論要求を満たせないと判断されたからこそ Opus が意図的にルーティングされている対象である。`/spec` の構造的リスクプロファイル (`#922` の Analysis 2: sub-agent fan-out のない単一 19 ステップ推論チェーンであり、エラーは code・review・merge に伝播する) はスキル自体の性質でありモデルに依存しないため、ここでも同様に適用される。下方変更を裏付けるために必要な telemetry も現時点では存在しない: `#1228` により spec フェーズの `token_usage` 発行はブロック解除されたが、本レポート時点で `.tmp/auto-events.jsonl` の `--opus` 実サンプルは依然として 0 件。wall-clock 代理指標 (同一 `xhigh` effort での比較: Opus 5 世代 9 件平均 16:22分 vs Opus 4.8 世代 5 件平均 13:22分、+22%) は存在するが並行セッション負荷と交絡しており、いずれの方向にも headroom を示さない。根拠のない状態で指針的ガイダンスに基づいて行動することは、パイプライン中で最も波及範囲の広いフェーズに対する判定としては不十分と判断した。なお `--max` 明示オーバーライドフラグの位置づけは**再確認・維持**とした: Opus 5 のより狭い「極端に困難かつ latency 非感応な場合に限定」という枠組みは、このフラグの既存の opt-in・都度呼び出し・人間/オペレーター判断によるエスカレーションという性質に、Opus 4.8 の曖昧な枠組みよりも少なくとも同程度以上に適合する。`run-spec.sh`・本マトリクス表・`tests/run-spec.bats` はいずれも変更しない。**再評価トリガー**: `--opus` の実 telemetry サンプルが本番サンプル調査に十分な量まで蓄積した時点で再評価する。その際は `/spec` 実行中の Size 昇格 (M→L、Opus/Sonnet の dispatch 判定後に発生しうる。実例: `#1175`) により Spec の最終記録 Size が実態と乖離しうる点に注意し、`sub_start` イベントの dispatch 時点の `size=L` フィールドから Opus 生成のサブセットを抽出すること。
...
| `WHOLEWORK_CI_MIN_CHECKS_WAIT_SEC` | `120` | `gh pr checks` が登録済み check 0 件を返したとき、`wait-ci-checks.sh` がポーリングを継続する猶予期間（秒）。この猶予期間を過ぎても check が 0 件のままの場合、明示的な警告を出して待機を終了する。テストでは `0` を設定して猶予期間をスキップできる |
...
| `WHOLEWORK_SPAWN_DETACH` | *(unset)* | 実験用 opt-in (issue #1142)。`1` を設定すると `scripts/run-auto-sub.sh` は python3 `start_new_session=True` shim (macOS には setsid(1) バイナリがないため) を介して自身を新しいセッション/プロセスグループのリーダーとして再起動し、spawn 元グループを狙ったプロセスグループ単位の外部 SIGKILL が wrapper サブツリーを巻き込めないようにする。detach 前に shim が detach 前 PGID の pointer から `AUTO_SESSION_ID` を解決するため、emit されるイベントの session_id は欠落しない。内部変数 `_WHOLEWORK_DETACHED=1` は再起動後の子を示す再帰ガード (手動設定禁止)。デフォルト (未設定) では spawn 挙動は完全に不変。この値は `.tmp/auto-events.jsonl` の `phase_start` イベントの `spawn_detach` フィールドとして記録される (Issue #1387)。背景: `docs/reports/external-kill-investigation.md` (H-a vs H-b' の切り分け)。 |
...
| `HAS_PR_PREVIEW_CAPABILITY` | `capabilities.pr-preview: true` | プロジェクトの PR が preview URL を生成するとき `true`。`/issue` Step 4 の pre-merge-preview AC 分類のゲートとして機能する。preview 環境でしか確認できない AC は、verify command の有無を問わず pre-merge セクションに配置される (auto サブケース: verify command あり — `ac-tier: preview` タグと `--when="test -n \"$PREVIEW_URL\""` ガードを付与し `/review` 時に実行。manual サブケース: verify command なし — `verify-type: manual` タグを追加、ガードなし)。いずれも `/review` 時に実行/提示され、`/verify` post-merge では二重検証防止のため skip する。ただし `/review` が投稿する最新の `type=preview-ac-unverified` マーカー (`modules/l0-surfaces.md` § "Machine-Readable Event Marker" 参照) がその AC を未検証としている場合、auto サブケースは `/verify` が本番 URL に対するフォールバック検証を行う (manual サブケースにはフォールバックできる verify command がないため、人間による確認項目として提示される)。`/code` の pr route Step 13 (Preview Build Verification) のゲートも兼ねる。`true` のとき `/code` は PR 作成後にデプロイ/preview ビルドの完了を待ち、失敗時は code フェーズ内から修正コミットを push する (最大 3 回まで)。上限に達した場合は `/review` に処理を委ねる。この capability を宣言していないプロジェクトでは、`/code` は従来どおり PR 作成時点でビルド待ちなしに完了する。`scripts/run-review.sh` の review セッション起動前 preview デプロイ待機のゲートも兼ねる。`true` かつ `PREVIEW_URL` 環境変数が export 済みの場合、`run-review.sh` は GitHub Deployments API のルックアップを完全にスキップする fast path を通る。これは `skills/review/SKILL.md` Step 8.0 が既存で持つ「`PREVIEW_URL` 設定時に Deployments API lookup を skip する」という契約と一致する部分であり、加えて `PREVIEW_URL` への HTTP 到達性 (2xx に加え、Basic 認証保護下の preview を示す 401/403 も稼働とみなす) をポーリングする — この reachability probe 自体は `run-review.sh` 側の新規挙動であり、Step 8.0 自体の契約には含まれない。`curl` が存在しない場合は probe を skip し、export 済みの `PREVIEW_URL` をそのまま accept する (fail-open、stderr に warning を出力)。`PREVIEW_URL` が未設定の場合は、従来どおり GitHub Deployments API で PR ブランチの最新デプロイ状態をポーリングする分岐にフォールバックする (後方互換)。いずれのポーリングも上限 `WHOLEWORK_PREVIEW_TIMEOUT_SEC` を持ち、時間内に稼働が確認できない場合は review セッションを起動する代わりに `PENDING` (exit code 2) で終了する |
