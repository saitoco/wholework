...
- **`/auto` スキル**: `run-*.sh` 経由で spec→code→review→merge→verify を順次連鎖させるオーケストレーター。各フェーズは設定可能なパーミッションモード（デフォルト: `--permission-mode auto`、`.wholework.yml` に `permission-mode: bypass` を設定すると `--dangerously-skip-permissions`）で `claude -p` 独立プロセスとして実行され、フレッシュなコンテキスト分離を保証する。追加機能: `phase/*` ラベル未設定時は issue triage/refinement から自動開始、`phase/ready` がない場合は `/spec` を自動実行、`--batch N` はバックログから N 個の XS/S Issue を処理、`--batch N1 N2 ...` は指定した Issue 番号を空白区切りで列挙しユーザ指定順に処理（XL 以外の全 Size を受け入れ）、XL Issue はサブ issue 依存グラフ（`blockedBy`）を読んで独立サブ issue を並列実行（worktree 分離）し依存先は順次実行、`--base {branch}` でリリースブランチ対象。`verify-max-iterations`（デフォルト: 3、最大: 20、`.wholework.yml` で設定可能）が verify-reopen ループを上限で止める。カウンターが上限に達すると Issue は `phase/verify` に留まり人間の判断を待つ。`/auto` スキルは verify 出力の `MAX_ITERATIONS_REACHED` を検出し、無限ループの代わりに連鎖実行を停止する。**`--resume N`** と **`--batch --resume`** で中断された実行を復元: 現在 phase はライブの GitHub labels を権威ソース（reconciler-first）として再読み込みし、チェックポイントファイル（単一 Issue: `.tmp/auto-state-N.json`、batch: `.tmp/auto-batch-state.json`）は verify iteration カウンタまたは残 batch リストのみを保持する。書き込みは atomic（`*.json.tmp` → `mv`）。stale チェックポイント（issue_number 不一致またはライブラベルとの `phase/done` 矛盾）はラベル状態を優先して破棄する。実装詳細は `scripts/auto-checkpoint.sh` を参照。
