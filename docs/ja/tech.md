...
- **`/merge` skill**: CI 通過とレビュー承認後に PR をマージ (`.wholework.yml` の `merge-strategy` で戦略を設定可能、既定 `squash`) し、リモートブランチを削除する。判断基準: AC 検証が GREEN (または明示的に承認済み) の場合のみマージする。`/review` から Phase Handoff を読み、`/verify` 用に Phase Handoff を書き込む。機械的な操作であり、`model: sonnet` + `low` effort で十分。
