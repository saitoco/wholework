...
| `HAS_PR_PREVIEW_CAPABILITY` | `capabilities.pr-preview: true` | プロジェクトの PR が preview URL を生成するとき `true`。`/issue` Step 4 の pre-merge-preview AC 分類のゲートとして機能する。URL/UX 系 AC に `ac-tier: preview` タグと `--when="test -n \"$PREVIEW_URL\""` ガードを付与し、`/review` 時に実行、`/verify` post-merge では二重検証防止のため skip する。ただし `/review` が投稿する最新の `type=preview-ac-unverified` マーカー (`modules/l0-surfaces.md` § "Machine-Readable Event Marker" 参照) がその AC を未検証としている場合は、`/verify` が本番 URL に対するフォールバック検証を行う |
