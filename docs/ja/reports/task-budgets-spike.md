[English](../../reports/task-budgets-spike.md) | 日本語

# Task Budgets (Beta) スパイクレポート: Wholework `/auto` への導入評価

**作成日**: 2026-04-18
**対象 Issue**: #222
**対象範囲**: `task-budgets-2026-03-13` beta を `/auto` および `run-*.sh` に導入可能かを評価
...
公式ドキュメント・`docs/reports/claude-opus-4-7-optimization-strategy.md` §2.3 の記載によると、task budgets は agentic loop 全体のトークン予算 (thinking + tool calls + tool results + final output) をモデルへヒントとして伝え、残予算に応じてタスク完了戦略を自律調整させる機能である。API key 認証環境での期待挙動は以下のとおり:
