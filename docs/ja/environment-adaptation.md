...
classifier 出力（`domain-classifier.md`）は frontmatter `domain:` キーの値を直接 echo します — 命名規約からの推論は行いません。
...
**Step 0: 既存パターンの網羅確認（前提条件）**

新メカニズムの設計に着手する前に、既存の adapter と capability を網羅的に列挙し、新要件が確立済みの adapter-resolver lazy chain で表現可能かを確認する:

- `modules/verify-executor.md` の変換表で `adapter-resolver.md` に委譲している行（`browser_check`、`browser_screenshot`、`lighthouse_check` 等）をすべて列挙
- `modules/{capability}-adapter.md` の bundled adapter（`browser-adapter.md`、`lighthouse-adapter.md` 等）をすべて列挙
- 同じパターン（Core 1 行追加 + bundled adapter + `HAS_{CAP}_CAPABILITY` gate）で新 capability を追加すれば要件を満たすかを、新メカニズム導入前に確認する

このステップを省略すると、既存 `adapter-resolver.md` の機能と重複するメカニズムを設計してしまうリスクがある（Issue #441 では当初「bundled custom verify handler resolution」機構の新設案を進めていたが、再評価により adapter-resolver が既に同じ 3-layer resolution を提供していたことが判明）。
...
6. **（ガイダンスコンテンツ）** capability に使用ガイダンス（適用シナリオ、既存 verify command との比較、採用判断基準）が必要な場合、`skills/{skill-name}/{capability}-guidance.md` を Domain file として作成し、frontmatter に `type: domain`、`skill: {skill}`、`load_when: capability: {capability}` を指定する。eager-load される共通モジュール（`modules/verify-patterns.md` など）にセクションを追加してはならない。参考実装: `skills/verify/browser-verify-phase.md`（`capability: browser`）、`skills/spec/visual-diff-guidance.md`（`capability: visual-diff`、Issue #441 で追加）。

### Eager-load と Lazy-load の分類（拡張要素別）

新 capability を最終決定する前に、この表で domain 外 overhead を見積もる。目標は **domain 外 eager overhead を ~200 tokens 以内** に抑えること:

| 要素 | Load 種別 | Domain 外 overhead | 備考 |
|------|----------|---------------------|------|
| `modules/{capability}-adapter.md` | Lazy | 0 | verify command 命令時のみ読み込み |
| Bundled sub-agent（`agents/{name}.md`） | Lazy | 0 | adapter Processing Steps から Task 起動 |
| `skills/{skill}/{capability}-guidance.md`（Domain file） | Lazy（capability gate） | 0 | `HAS_{CAP}_CAPABILITY=true` のときのみ読み込み |
| `modules/verify-executor.md` 変換表の行 | Eager | ~30 tokens / 行 | `/review` と `/verify` から常に読まれる |
| `modules/adapter-resolver.md` の capability 例示 | Eager | ~10 tokens / 単語 | capability 単語の追記のみ |
| `modules/detect-config-markers.md` マーカー行 | Eager | ~20 tokens / 行 | 設定検出時に常に読まれる |
| `docs/structure.md` Key Modules エントリ | Eager | ~30 tokens / 行 | Steering doc — 各 skill から参照される |
| `docs/environment-adaptation.md` 表エントリ | Eager | ~70 tokens / 行 | 設計参照用アーキテクチャ doc |
| **`modules/verify-patterns.md` のガイダンスセクション** | **Eager** | **~1500 tokens / セクション** | **避ける — Step 6 の Domain file 化を使う** |
