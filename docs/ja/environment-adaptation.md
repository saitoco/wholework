...
異なるツール構成を持つ環境間で同じ Skill 定義を動作させるためのアーキテクチャ。4 つのレイヤーで構成。
...
Layer 4: Execution      safe/full モード分岐 + アダプター委譲
```

## Layer 1: 宣言（`.wholework.yml`）

プロジェクトルートに配置する YAML ファイル。プロジェクトで利用可能なケイパビリティとツールを宣言。
...
各設定フィールドの詳細は `modules/detect-config-markers.md` のマーカー定義表を参照。

## Layer 2: 検出（detect-config-markers + ToolSearch + CLI 検出）

Layer 1 の宣言を読み込み、不足している情報をセッション内で動的に検出。
...
SKILL.md のコアを軽量に保ち、環境依存ロジック（Domain）を条件付きで読み込む。

### 判断基準

「このロジックは対象ツールやプロジェクト種別を使用しないプロジェクトでも必要か？」→ No なら Domain ファイルに抽出。

### 抽出パターン

| パターン | 条件チェック | 例 |
|----------|--------------|---|
| マーカー検出 | `.wholework.yml` の値 | `external-review-phase.md`（`copilot-review: true` の時に Read） |
| ファイル存在 | 特定ファイルの存在 | `skill-dev-recheck.md`（`validate-skill-syntax.py` が存在する時に Read） |
...
## Layer 4: 実行（verify-executor + アダプター）

検証コマンドを実行。ツール固有の処理はアダプターに委譲。
...
| `lighthouse_check` | UNCERTAIN | adapter-resolver 経由で `lighthouse-adapter.md` に委譲、CLI 検出はアダプター内部 |
...
アダプターはケイパビリティ（例: `browser`）をカプセル化。3 層の解決順序（`modules/adapter-resolver.md` 参照）でツール固有の実装を選択:
...
アダプターは 3 ステップで動作: 検出 → コマンド変換 → 実行委譲。

### アダプターコントラクトのテンプレート

アダプターは統一されたコントラクトに従う。ユーザーはこのテンプレートに従い、上記の project-local または user-global パスに配置することでカスタムアダプターを作成可能。

リファレンス実装: `modules/browser-adapter.md`。新しいアダプターを作成する際はこのファイルをガイドとして使用。

#### 必須セクション

**1. 検出手順**

利用可能なツールを自動検出する手順を記述。複数ツールをサポートする場合は優先度表に列挙。
...
verify command 表記からツール固有コマンドへのマッピングを記述。検出ツールごとにサブセクションを作成し、各コマンドの実行をステップ番号で記述。
...
ツールが検出されなかった場合の振る舞いを定義。原則として UNCERTAIN を返し、ユーザーを手動検証に誘導。
...
**セキュリティ制約** — ツール固有のセキュリティ制約（URL フィルタリング、認証情報のマスキング、SSRF 防止など）を記述。

**セットアップ手順** — 初回利用時のインストールガイドや前提条件を記述。
...
受入条件内の個別検証項目に環境ゲートを設定するためのメカニズム。
...
Layer 1–3 が "Skill のどの部分を読み込むか" を制御するのに対し、`--when` は受入条件内の個別検証項目レベルで環境ゲートを提供。
...
1. `modules/{capability}-adapter.md` を作成（`modules/browser-adapter.md` を参照）
2. `modules/adapter-resolver.md` の説明に capability を追加
3. `verify-executor.md` の変換表に新しいコマンドを追加し、adapter-resolver 経由で委譲
4. `modules/detect-config-markers.md` のマーカー表に `capabilities.{name}` を追加
5. `docs/structure.md` の Key Files テーブルに追加

### 新しい Domain ロジックの追加

1. `skills/{skill-name}/{domain}-phase.md` を作成
   - このファイルが参照するモジュールのフルパスをファイル先頭に列挙（例: `${CLAUDE_PLUGIN_ROOT}/modules/detect-config-markers.md`）。省略形（例: `detect-config-markers.md` のみ）は不可。フルパスを先頭に列挙することで、呼び出し側が読み込み前に参照モジュールを把握可能。
2. SKILL.md に条件付きの Read 指示を追加（マーカー検出パターンまたはファイル存在パターン）
3. `docs/structure.md` の Domain Files テーブルに追加
