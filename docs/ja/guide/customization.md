...
カバー範囲: このキーは `scripts/run-review.sh` の preview 待ちゲート (`/auto`・スケジュール実行・wrapper 直接実行) と、`/review` を skill として直接呼び出す経路の Step 8.0 Fast path の両方から参照されます。
