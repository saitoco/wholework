...
| Non-interactive mode | Skill execution invoked via `run-*.sh` with `claude -p --dangerously-skip-permissions`, where `AskUserQuestion` is unavailable. Triggers a three-tier policy (auto-resolve / skip / hard-error) at decision points. Signaled by `--non-interactive` in `ARGUMENTS` | run-*.sh, /auto | 非インタラクティブモード |
