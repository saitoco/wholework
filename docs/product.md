...
| Issue triage | Initial assessment phase that assigns Type/Priority/Size/Value before the main workflow starts. Implemented as `/triage` skill. `/auto` chains triage automatically when an Issue lacks a `phase/*` label | /triage, /auto | Issue triage |
