...
- **Pre-merge AC 8件はすべて PASS と判定** (rubric 6件・section_contains 1件・command 1件)。UNCERTAIN なし。`command "bats tests/"` は CI reference fallback で `Run bats tests` job の SUCCESS を参照して PASS。
- **確定 MUST/SHOULD/CONSIDER issue は 0件**。Workflow パイプライン (finder → adversarial verify) で 2件の finding (SHOULD 1件・CONSIDER 1件) を検出したが、いずれも adversarial verification で refute (false positive) された。詳細は本ファイルの `## review retrospective` 参照。
- **外部レビューツール (Copilot/Claude Code Review/CodeRabbit) は全て無効**のため Step 7 は全体スキップ。
...
- **`/code` フェーズの Phase Handoff (`<!-- phase: code -->`) が見当たらなかった**。worktree `code+issue-995` が PID 死亡済みのロックを残したまま放置されており (直近コミット履歴の Tier 3 リカバリイベントと符合)、正常な `Exit: merge-to-main` を経由しなかった可能性がある。`/merge` 実行者はこの点を認識しておくこと (実害はなし — 実装 diff 自体は origin と一致しクリーンだった)。
- **`skills/review/workflow-guidance.md` の Verify ステージに Workflow スクリプトのバグを発見**(`parallel()` 未使用による findings 消失)。本 PR の変更対象外のため未修正。Improvement Proposal として `/verify` 集約時に起票候補とすること。
