...
#### spec
- Issue 本文にコードの問題箇所（行番号・再現タイムライン）が詳細に記載されており、受け入れ基準も具体的で検証可能な形式だった。曖昧性も3点事前解決されており、品質は高い。

#### design
- Spec の `## Changed Files` と `## Implementation Steps` が実装と完全一致。最小修正方針（else 分岐への guard 追加のみ）が適切に設計され、設計逸脱なし。

#### code
- コミット履歴は1コミット（fix: exclude target label...）のみ。fixup/amend なし、レビューコメント取り込みもなし。最小修正として clean な実装。

#### review
- パッチルート（PR なし）で直接 main にコミットされたため、PR レビューは実施されていない。小規模なバグ修正（数行の guard 追加）であり、回帰テストで品質担保されているため問題なし。

#### merge
- パッチルート。`closes #132` により Issue は自動クローズ。コンフリクトなし。

#### verify
- Pre-merge の2条件はいずれも PASS（grep によるコード確認 + bats 全13テスト合格）。回帰テスト #13 が else 分岐の fix を直接検証しており、verify コマンドと実装が正確に対応している。
