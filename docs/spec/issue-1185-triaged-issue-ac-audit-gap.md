...
- **Tier 2 recovery の永続記録が失われている (#1181 の回帰)** — retro-proposals パイプラインで Tier 1 と分類したが、同一領域を扱う既存の **#1098** に追記した (新規起票なし)。`scripts/run-auto-sub.sh` の `orchestration-recoveries.md` 書き込み経路は manual / wrapper-retry / Tier 3 の 3 つのみで **Tier 2 の経路が存在せず**、#1181 が `_write_tier2_recovery_to_spec()` を削除した結果、Tier 2 の記録先は gitignore された `.tmp/auto-events.jsonl` だけになった。`docs/spec/issue-1015-fix-recovery-spec-split.md` の Notes が「Tier2 側を修正する場合は recoveries log への書き込み追加とセットで設計する必要がある」と事前に警告していたが、そのセットは実施されなかった。波及として `collect-recovery-candidates.sh` の閾値判定に Tier 2 が載らず、#1191 (`/audit stats --retention` の recovery 頻度セクション) を実装しても Tier 2 は可視化されない。あわせて `skills/verify/SKILL.md` Step 12 step 3 の skip 判定 (「Tier 2/3/Manual recovery が `## Auto Retrospective` に記録されていなければ notable content」) も #1181 の記録先変更に追随していないため、Tier 2 が起きる度に verify retrospective の skip が構造的に不可能になる。
