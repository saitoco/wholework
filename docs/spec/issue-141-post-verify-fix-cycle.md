...
- `tests/setup-labels.bats` の `command` verify ヒントは CI 参照フォールバックに依存しているため、レビュー実行タイミングによって UNCERTAIN になる場合がある。今回は CI が IN_PROGRESS であったため UNCERTAIN となった。`/verify` (full mode) 時は直接実行されるため問題ない。
