...
### Key Decisions
- Rebase時に `docs/spec/issue-934-observation-condition-gate.md` の `## Auto Retrospective` (HEAD側) と `## Code Retrospective` (main側) が競合したため、両セクションを削らず並記する形で解消 (異なる内容の追記であり、どちらか一方を落とす必要はなかった)
- squash merge は `gh pr merge --squash --delete-branch` 実行時に worktree 構成 (`main` が別ディレクトリでチェックアウト中) が原因で local branch 操作がエラーとなったが、リモートへのマージ自体は成功済みだったため、`--delete-branch` を外した再実行で状態確認し、リモートブランチ削除のみ個別に実施
...
- 実際の呼び出し元 (`/review` SKILL.md 等) への `--context-file` 配線は Spec のスコープ境界どおり本 Issue に含めていない

### Notes for Next Phase
- Rebase後に `tests/opportunistic-search.bats` / `tests/observation-trigger.bats` を再実行し、計33ケース全て PASS を確認済み
- CI は全ジョブ SUCCESS 済み (マージ前提の状態を維持)
