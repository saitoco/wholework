...
### Key Decisions
- mergeable=true (CI SUCCESS・review approved) だったため conflict 解決は不要で通常の squash merge を実行した。
...
#### issue
- `/issue 962 --non-interactive` の refinement で、ユーザーコメント (`spawn-recovery-subagent.sh` 経由の実例報告) を受けて横断 grep 調査を先行実施し、AC を 1 件の rubric から 5 件の機械検証 AC + 1 件の rubric に再構成した判断は適切だった。当初の狭いスコープ (1ファイルのみ) のまま実装が進んでいたら、同一 root cause を持つ他 4 ファイルが見落とされたままになっていた。

#### spec
- Issue #966 (`run-auto-sub.sh` の同型修正、先行事例) の実装・テストパターンへの完全準拠を明記しており、設計判断に迷いがなかった。`git rev-parse --show-toplevel` が worktree 内でも worktree 自身のルートを返すという実測結果を #966 から継承して活用していた点も適切。

#### code
- bats フルスイート実行で Spec の Changed Files に記載のなかった `tests/run-verify.bats` の FAIL 4 件を検出・その場で修正した (Design Gaps/Ambiguities 節に詳細記録済み)。「`scripts/*.sh` の横断確認」が `tests/` 側の網羅を含意しないという教訓は今後の同種 Issue で再利用可能 (`git grep -l "<対象スクリプト名>" tests/` での事前洗い出しを推奨、と既に記録済み)。

#### review
- review-light がフルスコープ再確認を行い新規の乖離なしと判定。Recurring issues 節で `dirname "$SCRIPT_DIR"` 誤算出パターンが #966 → 本 Issue (5ファイル) → `scripts/check-file-overlap.sh:36` (未着手) と3件にまたがって出現していることを正しく指摘し、follow-up 候補として明示的に記録した。

#### merge
- mergeable=true / CI SUCCESS / review 済みで一発完了。conflict なし。

#### verify
- Pre-merge 6件 (grep×5, rubric×1) はいずれも UNCERTAIN なく一発 PASS (ローカル main の pull 漏れで一時的に FAIL に見えたが、`git pull` 後に再検証し解消 — verify 実施者側の手順ミスであり実装の問題ではない)。Post-merge の manual AC (別プロジェクト実地確認) は Claude 非実行のため未チェックのまま `phase/verify` を維持。

### Improvement Proposals
- **`scripts/check-file-overlap.sh:36` の同型 repo-root 誤算出パターンを修正**: review retrospective で特定済みの follow-up。`dirname "$SCRIPT_DIR"` による誤算出パターンが #966 (`run-auto-sub.sh`) → #962 (5ファイル) → 本箇所と3件目の出現であり、既に確立された修正パターン (`git rev-parse --show-toplevel 2>/dev/null || pwd`) をそのまま適用できる、スコープの狭い (1ファイル1箇所) 修正。
- **repo-root 算出パターンの構造的な再発防止 (lint ルールまたは共有ヘルパー化)**: review retrospective が指摘した通り、同一の root cause パターンが複数 Issue にまたがって散発的に発見・修正されている。新規スクリプト追加時に `git rev-parse --show-toplevel` ベースの共通ヘルパー使用を強制する仕組み (shellcheck 相当の lint ルール、または `scripts/lib/repo-root.sh` のような共有関数化) があれば、今後の新規スクリプトでの再発を防げる可能性がある。ただし対象範囲・実現方式が未確定な設計提案であり、今回は Tier 2 (convention/lesson) として記録するに留める。
