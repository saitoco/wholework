...
### Key Decisions
- `_append_hints_to_actual()` は `_completion_spec()` の mismatch ケースのみに適用（spec フェーズが最も頻発するため）。他フェーズへの拡張は将来の Issue に委ねる
- `mid-run-api-error` パターンは既存 elif チェーンの最後（`elif [[ "$EXIT_CODE" == "0" ]]` の直前）に挿入し、first-match-wins の順序を維持
- テスト: `git` と `gh` の両方をモックする必要があり、`WHOLEWORK_SCRIPT_DIR` + `PATH` 両方を設定

### Deferred Items
- 他フェーズ（code, review, merge）への復元ヒント拡張（現在は spec フェーズのみ）
- `hint_spec_file` フィールドの明示出力（現在は既存 `spec_file` フィールドで代替）
- reconcile + retry の自動実行ロジック自体は `/auto` スキル側の実装に委ねる（本 Issue は条件定義のみ）

### Notes for Next Phase
- PR #537 は 5 件の変更（detect-wrapper-anomaly.sh, orchestration-fallbacks.md, reconcile-phase-state.sh, auto/SKILL.md, phase-state.md + bats テスト）
- bats テスト全 671 ケース PASS（新規 4 ケース含む）、CI でも再確認が必要
- `/review` では reconcile-phase-state.sh の引数 signature 不変を重点確認すること（既存テスト 49 ケースが継続 PASS していることで確認済み）
