...
### Key Decisions
- PR #537 をスカッシュマージ（`--squash --delete-branch`）。`mergeable=true`、CI SUCCESS、reviewed 済みのため競合解消不要
- `hint_spec_file` doc-impl mismatch の修正コミットも含めてスカッシュ済み。main に統合完了

### Deferred Items
- 他フェーズ（code, review, merge）への復元ヒント拡張（現在は spec フェーズのみ）
- `apply-fallback.sh` への `mid-run-api-error` 対応（XL bash path は Tier 3 fallback のまま）、将来の Issue で対応
- downstream プロジェクトでの post-merge 観察（mid-run API 障害時に Tier 2 自動 reconcile + retry が機能するか）

### Notes for Next Phase
- verify はPost-merge 観察が主目的。bats CI は 671 ケース（うち新規 4 ケース）PASS 済み
- `reconcile-phase-state.sh` の `_append_hints_to_actual()` は spec フェーズの mismatch 時のみ出力する点に注意
- `phase-state.md` の `hint_spec_file` フィールドはドキュメント記載だが実装では既存 `spec_file` で代替（重複回避設計）
...
#### spec
- AC を What レベルで明確化し rubric + github_check で検証可能化。SPEC_DEPTH を light→full 相当で再評価し Size を M→L に更新（変更 7 ファイル）。設計粒度は一発実装に耐えた。

#### design
- `_append_hints_to_actual()` の signature・出力フィールドは Spec 通り実装され乖離なし。Spec 設計テンプレートは復元ヒントを `hint_spec_file` と命名したが、実装/出荷ドキュメントは重複回避のため既存 `spec_file` フィールドで代替。出荷 `phase-state.md` / `orchestration-fallbacks.md` は `spec_file` で整合しており drift なし（命名の Spec-実装ズレは review で検出）。

#### code
- 一発全テスト通過、rework ゼロ。bats 新規 4 ケース追加。`git` モックの PATH 解決は Spec テンプレートで明示済みで問題なし。

#### review
- ⚠️ **最重要**: review フェーズが「全CI PASS」と報告したが、実際には Forbidden Expressions check が FAIL していた。/review の review retrospective が非推奨用語を spec へ直接引用し、`docs/spec/` が `check-forbidden-expressions.sh` の SCAN_DIRS 対象のため CI 失敗した。review の CI 確認が全チェック完了前に判定されたか、高速チェック（Forbidden Expressions 等）を待たなかった可能性が高い。
- 一方 `hint_spec_file` の Spec-実装命名ズレは review が正しく検出し、出荷ドキュメントを `spec_file` で整合させた（良い検出）。

#### merge
- 親 `/auto` セッションが review の CI 誤判定を手動リカバリ: spec review retrospective の非推奨用語を `verify command` へ修正（commit `ea71218`）→ CI green 確認 → squash merge。手動介入 1 回で復旧。

#### verify
- 全 Pre-merge AC 5/5 PASS（4 rubric + 1 github_check bats）。Post-merge AC は downstream 実運用での mid-run API 障害観察が必要なため manual で保留し `phase/verify` に遷移。出荷物に drift なし。

### Improvement Proposals
1. **/review の「全CI PASS」報告が高速 CI チェックの失敗を見落とす**: 本 Issue で review が Forbidden Expressions check の FAIL を見落として「全CI PASS」と報告し、親 /auto がマージ前に手動リカバリを要した。/review の CI 確認ロジックが結論前に全必須チェックの完了を待っていない可能性がある。/review が結論前に `wait-ci-checks.sh` 相当で全必須チェック完了を確認する、または retrospective を spec へ書き込む直前に `check-forbidden-expressions.sh` をローカル実行する guard を追加することを提案。
2. **retrospective 記述時の非推奨用語直接引用の再発防止**: tech.md で明示警告済み（`docs/spec/` は SCAN_DIRS 対象、`旧称:` prefix か descriptive language を使う）にもかかわらず、/review の retrospective writer が非推奨用語を直接引用した。/code・/review の retrospective 書き込み手順に、コミット前ローカル scan guard または明示的注意喚起を追加することを提案（提案1と統合実装可能）。
...
|-------|-------|--------|-------|
| spec  | pr    | SUCCESS | SPEC_DEPTH 再評価で Size を M→L に更新 |
| code  | pr    | SUCCESS | PR #537、一発実装で rework ゼロ |
| review | pr (--full) | SUCCESS（CI 誤判定→親手動リカバリ） | 「全CI PASS」報告だが Forbidden Expressions check が FAIL |
| merge | pr    | SUCCESS | 非推奨用語修正（commit `ea71218`）→ CI green → squash merge |
| verify | -    | SUCCESS（auto 5/5 PASS） | post-merge manual 条件保留 → `phase/verify` |

### Orchestration Anomalies
- **review CI false-pass**: review フェーズが exit 0 かつ「全CI PASS」と報告したが、実際は Forbidden Expressions check が FAIL（review retrospective が非推奨用語を spec へ直接引用、`docs/spec/` は `check-forbidden-expressions.sh` の SCAN_DIRS 対象）。正規の 3-tier recovery（wrapper 非ゼロ exit が前提）には乗らず、親セッションが merge precondition 前に `gh pr checks` で FAIL を検知し手動リカバリ（spec 修正・`ea71218`・CI green 確認・再 merge）した。手動介入 1 回で復旧。

### Improvement Proposals
- 上記 anomaly の改善提案は #538（retro/verify）として起票済み。詳細は `## Verify Retrospective` の Improvement Proposals を参照（重複起票回避のため本セクションからの再起票は行わない）。
