...
- 抑制条件を `"matches_expected":true` 単独チェックに変更 (旧: `&&` で `"commits_found":true` を AND)。reconciler の completion 判断を anomaly detector より上位に置く reconcile-first authority を適用
- 新テストは既存 line 228 パターンと同形で `commits_found:false` のみ変更。mock git + success phrase detection path を通して false positive が消えることを確認する構造
...
- 既存テスト (line 228: `matches_expected:true` AND `commits_found:true`) は AND 条件削除後も PASS することを実装前に確認すること
- `orchestration-fallbacks.md` の Exception Condition 追加箇所は `## code-patch-silent-no-op` セクション内; `section_contains` verify の境界は次の `---` まで
- `grep "matches_expected|reconcile"` verify: 修正後スクリプトに `grep -q '"matches_expected":true'` (条件部) と `# reconcile-first authority` (コメント部) の両語が存在することを実装後に確認すること
