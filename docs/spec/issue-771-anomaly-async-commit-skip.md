...
- 抑制条件を `"matches_expected":true` 単独チェックに変更 (`commits_found:true` の AND 条件を削除)。reconcile-first authority を実装
- `orchestration-fallbacks.md` の `## code-patch-silent-no-op` セクションに Exception Condition サブセクションを追加し、`matches_expected:true` 例外条件を SSoT に明文化
- 新テスト (async external commit: `matches_expected:true`, `commits_found:false`) を追加し、全 33 テスト PASS を確認
...
- PR #784 で全 AC PASS 済み; bats 33/33 PASS, grep PASS, section_contains PASS
- `orchestration-fallbacks.md` の Exception Condition は `## code-patch-silent-no-op` セクション内、`---` 区切り前に追加済み
