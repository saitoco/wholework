...
### Key Decisions
- AC1/AC2 とも `rubric` verify command で PASS 判定 (SKILL.md Step 3 の文言追加、script の defense-in-depth warning とも Issue #1037 の要求を満たすと判断)
- Code Retrospective に記載済みの実装偏差 (check 挿入位置の変更) を `tests/run-verify.bats` の dedup guard 前提と突き合わせて再検証し、妥当と確認した
- フルテストスイート実行時に発生した `post_merge_check.bats` の2件の失敗は、単体実行では再現せず並列実行 (`--jobs`) 時のみ発生したため、この PR と無関係な並列実行フレークと判断し、ブロッキングとしない

### Deferred Items
- Post-merge AC (次回 batch mode での実挙動観察) は `/verify` フェーズで evaluate される (verify-type: observation event=auto-run) — review フェーズでは対応不要

### Notes for Next Phase
- `/merge` 時点で MUST issue はゼロのため、そのまま `/merge 1040` を実行してよい
- Post-merge AC の観察対象は「次回 batch mode での XS patch route Issue の /verify 実行時に defense-in-depth warning が出ないこと」。`/verify` フェーズで evaluate される
