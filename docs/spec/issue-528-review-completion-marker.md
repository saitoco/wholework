...
#### spec
- Light spec で 4 実装ステップ / 8 pre-merge 検証項目を設計。マーカー方式が SKILL.md・reconcile・phase-state.md の3ファイルに跨るため rubric + file_contains の機械検証ペアを採用。全項目 PASS で検証可能性は良好。

#### design
- マーカートークン `review-summary` を Issue 段階で確定したことで /code が一意に実装でき逸脱ゼロ。verify command 引数に内部トークンのみを使い HTML コメントのネスト破壊を回避する設計も機能した。

#### code
- 逸脱なし、1回の実装で全テスト PASS。worktree 初回 push タイミング（PR 作成前に push 必要）が Code Retrospective に記録されているが既存挙動の範囲で対処済み。

#### review
- MUST / SHOULD / CONSIDER いずれも 0。Spec 準拠の実装で review 指摘なし。

#### merge
- CI 全 SUCCESS、CLEAN マージ。reviewDecision 空は wholework フローの通常状態（merge precondition は --warn-only のため継続）。

#### verify
- **本 Issue が修正対象とする false-negative が、本 `/auto` 実行の review phase で実際に再現した**（PR #544 のサマリ見出し `## レビューレスポンスサマリー` を未マージの旧 reconcile が検出できず wrapper exit 1）。親セッションが ground-truth（サマリ投稿済・全 CI SUCCESS・/merge 推奨）を確認のうえ手動 override で merge へ継続。マージ後、bats 回帰テストで localized 見出し + マーカー → matches_expected:true を確認し、post-merge manual 条件を機構実証で PASS（ユーザー承認済み）。修正の有効性が同一実行内で end-to-end に裏付けられた稀なケース。

### Improvement Proposals
- N/A（修正は意図どおり機能。残る唯一の懸念は review LLM が将来の実行で `review-summary` マーカーを確実に出力するか。SKILL.md の逐語保持注記 + 旧テキスト署名 fallback の二重担保で軽減済みのため新規 Issue は不要）
...
|-------|-------|--------|-------|
| code | pr | SUCCESS | PR #544 作成 |
| review | pr | SUCCESS (manual recovery) | run-review.sh exit 1（reconcile false-negative）だが review 実質完了、親が ground-truth 確認のうえ手動 override |
| merge | pr | SUCCESS | CLEAN マージ（全 CI SUCCESS） |
| verify | - | SUCCESS | 全 9 条件 PASS、phase/done でクローズ |

### Orchestration Anomalies
- **review phase: reconcile false-negative による wrapper exit 1（本 Issue #528 の修正対象パターンそのものを実装実行中に踏んだ）**
  - script: `run-review.sh`, exit code: 1
  - state: PR #544 にレビューサマリコメント投稿済（見出し `## レビューレスポンスサマリー`）、全 CI SUCCESS、MUST 0、`/merge` 推奨。しかしマージ前の `reconcile-phase-state.sh` は旧2署名（`## Review Response Summary` / `## レビュー回答サマリ`）のみ検出するため `matches_expected:false` を返却 → `run-review.sh` が silent no-op と誤判定し exit 1。
  - Tier 1（reconcile 再チェック）: `matches_expected:false`（マージ前の旧コードのため override 不可）
  - Tier 2（`detect-wrapper-anomaly.sh`）: **空出力（既知パターン未該当）** — review-completion-false-negative を検出できず
  - 手動回復: 親セッションが PR コメント・CI rollup を直接確認し ground-truth で review 成功と判定、merge へ継続。マージにより #528 のマーカー方式が main に反映され、以降は本パターンが解消。bats 回帰テストで同一見出し + マーカーの検出も確認済み。

### Improvement Proposals
- **`detect-wrapper-anomaly.sh` に review-completion-false-negative パターンを追加**: review wrapper が exit 非0 かつ reconcile が review completion を false と返すが、PR にレビューサマリコメントが存在し CI が全 SUCCESS のケースを既知パターンとして検出し、Tier 2 で reconcile 再チェック経由の auto-recovery に乗せる。#528 のマーカー方式で根本原因は解消したが、review LLM がマーカーを欠落させ、かつ見出しが既存 fallback 2種以外にローカライズされた場合の defense-in-depth として有効。
