...
#### spec
- Issue の受入条件は機械的検証可能な 5 件（command 行数 / rubric dedup / github_check 2 件 / rubric coverage）でよく整形されていた。曖昧さ起因の rework なし。
- `/issue` 転記の retrospective が pre-merge AC を「3 → 4 項目」と記録していたが実際は 5 項目。spec retrospective で既に軽微な数え違いとして記録済み・実害なし。

#### design
- in-file state パラメータ化 vs 共有モジュール抽出の判断が的確。「Step 11 は /verify で常に到達するため module 抽出は peak context を減らさず逆効果、先例 phase-state.md は複数 caller 共有が根拠で単一 caller の Step 11 には非該当」という根拠は本 Issue の主目的（peak context 削減）に正しく整合。Issue の rubric が両方式を許容していたため AC との齟齬なし。

#### code
- Spec の実装ステップに忠実、deviation・rework なし（fixup/amend パターンも git log 上に無し）。
- 旧 CLOSED パスの「テキストは close を指示するがコードブロックに `gh issue close` が無い」という潜在的な text/code 乖離を dedup の過程で発見・解消（退行ではなく改善）。

#### review
- full review が分岐単位でロジック保存を検証し、(a)〜(d) 各ケースと state 依存差分（OPEN 時 close / CLOSED 時 reopen / FAIL コメント文言）の保存を確認。MUST/SHOULD 指摘なし、CONSIDER 2 件（非ブロッキング）。code が発見した text/code 乖離修正を正しく評価。

#### merge
- squash merge クリーン、CI 全 GREEN（DCO / Forbidden Expressions / Run bats tests / Validate skill syntax / macOS shell compatibility）、コンフリクトなし。

#### verify
- pre-merge 全 5 条件 PASS。post-merge manual 条件（複数回 /verify での parse 失敗頻度の定性確認）は縦断観察が必要なため未チェックのまま phase/verify を維持。
- **dogfooding 成功**: 本 /verify 515 実行自体が統合後の 589 行 Step 11（`ISSUE_STATE` パラメータ化版）を用いて parse 失敗なく完走。case (a) 分岐（全 PASS + 未チェック manual 残存 → phase/verify、Issue は CLOSED 維持）が統合後ロジックで正しく動作することを実地確認。
- **verify command の不整合（改善提案あり）**: `github_check "gh pr checks" "..."` ヒントは PR 番号引数を持たないため、post-merge /verify が verify worktree ブランチ（PR 非紐づけ）上で実行されると `gh pr checks` が当該ブランチの PR を見つけられず fail する。本実行では executor が post-merge コンテキストを認識し merged PR #516 を明示注入（`gh pr checks 516`）して PASS 判定したが、リテラル実行では FAIL/UNCERTAIN の誤判定リスクがある。

### Improvement Proposals

- **verify-executor の `github_check "gh pr checks"` ハンドラに merged PR 番号の解決フォールバックを追加する**: 現状 Step 5 の patch route 検出は「PR_NUMBER が空かつ `github_check "gh pr checks"` を含む → UNCERTAIN（`gh run list` 形式を案内）」を処理するが、pr route の post-merge では PR_NUMBER は非空であるにもかかわらず、`gh pr checks`（番号なし）が verify worktree ブランチの PR を見つけられず fail するギャップが残る。`/verify` が Step 2 で解決済みの PR_NUMBER を `gh pr checks` に注入する（または verify-executor が `gh pr list --search "closes #N" --state merged` で merged PR を解決する）フォールバックを追加すべき。`spec-test-guidelines.md` が CI 検証に `github_check "gh pr checks"` パターンを積極推奨しているため、この hint を持つ全 Issue の post-merge /verify が影響を受ける（再発性・横断性が高い）。
