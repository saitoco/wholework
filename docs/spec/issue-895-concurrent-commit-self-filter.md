...
- N/A — implementation followed the Spec's Implementation Steps exactly (same regex pattern, same emit-loop restructuring, same two bats test scenarios); no reordering, omission, or approach change occurred.

### Design Gaps/Ambiguities
- N/A — the boundary-aware regex design in the Spec Notes was re-verified independently during implementation (issue=89 self-pattern vs. a `#895` commit subject correctly does not match; a `#89` commit subject correctly matches), confirming the design as documented.

### Rework
- N/A — the single-block edit to `scripts/run-auto-sub.sh` and the two new `tests/run-auto-sub.bats` cases both passed on the first run (`bats tests/run-auto-sub.bats`: 43/43; full suite `bats tests/`: 1066/1066).
...
### Key Decisions
- Implemented the exact regex-based self-issue exclusion filter (`#${issue}([^0-9]|$)`) specified in the Spec's Implementation Steps rather than an alternative approach — the boundary-aware pattern was already re-verified in a standalone bash check during implementation, confirming no digit-count collision risk (e.g., issue `#89` vs. commit referencing `#895`).
- Added the two bats test cases (self-issue-only / mixed self+other-issue) exactly as scenario-specified in the Spec, reusing the existing mock `git` script pattern already present in `tests/run-auto-sub.bats` for the prior `concurrent_commit_detected` test.
- Ran the full `bats tests/` suite (not just the direct-counterpart test file) because `run-auto-sub.sh` is referenced by `tests/auto-sub-observability.bats` and `tests/run-code.bats` in addition to its direct counterpart — full-suite override triggered per the Behavioral Change Detection rule.

### Deferred Items
- Post-merge observation AC ("次回 `/auto --batch` 実行時、Concurrent commits detected が単独セッションでは 0 になることを観察") is left for `/verify` — cannot be confirmed until a real `/auto --batch` run occurs post-merge.
- Issue #668 (icebox: correlating concurrent commits with Issue outcomes) remains explicitly out of scope, as noted in the Spec Notes; its premise should be re-examined only after this Issue lands.

### Notes for Next Phase
- No deviations from the Spec occurred, so the Spec's Implementation Steps section was not modified.
- Both pre-merge AC rubric conditions were self-assessed as PASS against the actual `git diff` (regex exclusion added; both no-false-positive and true-detection bats cases present and passing) before checking the Issue's pre-merge checkboxes.
