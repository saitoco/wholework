...
- **B 案 (常時投稿) を採用**: `/review` は Pre-merge に `ac-tier: preview` AC が 1 件以上ある限り毎回マーカーを投稿し、未検証ゼロは `ac=none` で表す。`/verify` は latest-wins のまま変更不要。A 案 (counter-marker) を退けた理由は Spec § Alternatives Considered を参照。
- **マーカー解決ロジックを `scripts/resolve-preview-ac-fallback.sh` へ切り出す**: AC3 が要求する「fix-cycle 再検証の模擬テスト」を prose の grep ではなく実挙動として書けるようにするため。`/verify` Step 5 はこのスクリプトの stdout を fallback 対象リストとして使う。
- **`ac=none` sentinel**: 空値 `ac=` は属性境界が曖昧になるため採用しない。
- **fail-open 方針**: スクリプトは `gh` 失敗時も空出力 + exit 0 とし、`/verify` を止めない (空出力 = 従来の SKIPPED 挙動に落ちる)。

### Deferred Items

- `docs/tech.md` L241 / `docs/ja/tech.md` L228 の `HAS_PR_PREVIEW_CAPABILITY` 説明更新は Steering Docs sync candidate として Changed Files に載せたが、include/exclude の最終判断は `/code` に委ねた。
- `gh` の `--jq` 式 (`sort_by(.createdAt) | .[-1]`) の正しさは bats では検証できず、Post-merge の observation AC に委ねる。
- prose-driven SKILL.md ステップのスクリプト切り出しを他ステップへ横展開する話は本 Issue のスコープ外。

### Notes for Next Phase

- `skills/review/SKILL.md` / `skills/verify/SKILL.md` の本文編集時、半角 `!` を持ち込まないこと (`scripts/validate-skill-syntax.py` の MUST 制約)。
- `scripts/resolve-preview-ac-fallback.sh` は bash 3.2+ 互換で書くこと (macOS システム bash)。`set -euo pipefail` 下で `grep` の no-match が正常系であるため、各段に `|| true` を付ける。
- `skills/verify/SKILL.md` frontmatter `allowed-tools` への `${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-ac-fallback.sh:*` 追加を忘れないこと (Implementation Step 4 に含めてあるが、frontmatter 編集は本文編集と別作業になりやすい)。
- `tests/review.bats` / `tests/verify.bats` の既存 `@test` は削除・改変しないこと。`tests/verify.bats` には Step 5 節抽出ヘルパを新規追加する必要がある (既存は Step 2 用のみ)。
