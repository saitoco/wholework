...
### Key Decisions
- rebase 時に `docs/spec/issue-984-auto-recovery-pr-issue-mixup.md` の `## Auto Retrospective` (origin/main 側、Tier 2 fallback 記録) と `## Code Retrospective` / `## Phase Handoff` (worktree 側) が競合したため、両セクションを削除せず両方保持する形で解消した (どちらも追記的な記録セクションで内容の競合はなし)
- squash merge は `gh pr merge --squash --delete-branch` で正常完了。ローカル `main` ブランチが親 worktree で使用中のため、`gh pr merge` の delete-branch ステップは git 側でエラーを出したが、remote 上の PR merge 自体は成功しており (`state=MERGED` を確認)、remote branch は手動で `git push origin --delete` により削除した

### Deferred Items
- Post-merge の observation AC (次回 review/merge phase での Tier 2/3 recovery 発火時の実記録確認) は `/verify` フェーズに委譲 (変更なし、review phase からの継続)

### Notes for Next Phase
- bats フルスイート (`tests/run-auto-sub.bats` 56件 + `tests/spawn-recovery-subagent.bats` 13件、計69件) は rebase 後の worktree 上で PASS 済み
- Steering Docs 更新不要判断は merge フェーズでも変更なし

## review retrospective

### Spec vs. implementation divergence patterns
- 乖離なし。Spec に記載された 5 箇所の変更対象・行番号・引数名は PR diff と完全に一致していた。

### Recurring issues
- 新規の SHOULD 指摘を 1 件検出・修正した: `run_phase_with_recovery()` が `_EXTRA_SELF_ISSUE`/`EMIT_ISSUE_NUMBER` をプロセス export された値として参照する設計 (#974/#987 由来) は、review/merge phase セッションにネストして `tests/run-auto-sub.bats` を実行すると子プロセスへ漏れ伝播し、code-patch phase 系の既存テスト 4 件が誤った Issue 番号宛てに記録することを本 `/review` セッション自身の環境で再現確認した (CI のクリーンな環境では顕在化しない)。本 Issue が解消した「PR番号/Issue番号 混同」と同じ根本原因クラスが、ネストしたセッションでの環境変数漏れという別経路で再発する形。`tests/run-auto-sub.bats` の `setup()` に明示的な `unset` を追加してテストの環境分離を回復した。本リポジトリは自身の `/auto`/`/review` skill を自己ホストしているため、今後もこの種のネスト実行由来の環境汚染リスクには注意が必要。
