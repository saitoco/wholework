...
- **Step 10 verify command のスキップ**: Pre-merge hints（`file_exists "install.sh"` 等）は install.sh 方式を前提としており、probe commit の時点では前提未達成で全 FAIL する。partial implementation であることを明示し、verify command 実行は follow-up iteration に defer
...
- Code Retrospective で 3 項目（route override、Step 2–4 未実施、verify command スキップ）を事前記録していたため、review フェーズで追加指摘として挙がる事項がなかった
