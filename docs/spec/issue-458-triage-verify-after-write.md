...
#### spec
- 2 つの根本原因に 2 つの局所修正（D: verify-after-write + B': --no-cache フラグ）を対応させた設計は明快で実装と乖離なし
- verify-after-write の配置先（triage SKILL.md vs shared module）という曖昧性が Issue body の Auto-Resolved で早期解決されていた点は良好
- Gap: Spec Notes §「bats テスト Mock 互換性」が `run-auto-sub.bats` の mock 互換性を記載したが、`get-issue-size.bats` の `gh api graphql` mock が `--jq` 引数を処理しない点を見落とした（→ code rework 1 件の原因）

#### design
- N/A（spec フェーズに統合）

#### code
- `test: fix --jq handling in gh mock for no-cache path tests` の rework 1 件が発生
- 原因: `--no-cache` パスでは `gh-graphql.sh` が `--cache` フラグなしで呼ばれ、`gh api graphql` に `--jq` を渡す経路が開くが、既存 mock がこれを無視して生 JSON を返したためテスト失敗
- 実装本体（SKILL.md / get-issue-size.sh / run-auto-sub.sh）の rework なし

#### review
- PR #472 に formal レビューコメントなし
- rework が 1 件あったが、テストで自己検知されており review が欠如していてもリリース品質への影響なし

#### merge
- main への ff-only マージが正常完了。コンフリクト・CI 失敗なし

#### verify
- 全 4 pre-merge 条件が初回 verify で PASS（実装と acceptance criteria の整合が高い）
- Post-merge opportunistic 条件（実環境での `/auto --batch` 動作確認）は引き続きユーザー検証待ち

### Improvement Proposals
- `--no-cache` 系フラグを追加する際は、bats mock の `gh api graphql --jq` 引数処理を明示的にチェックする旨を `docs/migration-notes.md` に追記するとrework を防げる
- 将来 Priority/Value でも同様の eventual consistency 問題が発生した場合、verify-after-write を `modules/project-field-update.md` shared module に移動して 3 種共通化する選択肢がある（Spec Notes §「実装ファイル選択の確定」参照）
