...
- `docs/reports/skill-loader-cache-behavior.md` を唯一の新規ファイルとして作成する。他のファイルへの変更は不要（翻訳 mirror 不要、workflow.md 更新不要）。
- 調査フレームは「仕様として明文化」（Issue 目的 Option 3）。fix ではなく document アプローチ。
- 文書内で必ず "snapshot" と "reload" キーワードを含める（grep verify command の PASS 条件）。

### Deferred Items

- docs/guide/troubleshooting.md への "session 中 skill 更新は反映されない" 注記は Post-merge 手動 AC として defer。
- メカニズムの確定（セッション内スナップショット vs キャッシュ層）は Post-merge 再現テストに委任。

### Notes for Next Phase

- Changed Files は 1 ファイルのみ: `docs/reports/skill-loader-cache-behavior.md`
- レポート内に "snapshot"（いつスナップショットされるか）と "reload"（reload trigger は何か）の両キーワードを必ず含めること。verify command の PASS 条件。
- `docs/workflow.md:131` の既存記述（`/auto` 向け）を参照リンクとして含めると文書の整合性が高まる。
