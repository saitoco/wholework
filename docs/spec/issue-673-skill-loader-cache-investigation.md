...
#### issue
- AC 設計が `verify-executor` の制約を踏まえて適切に分割（AC #2 を snapshot/reload 2 個に分け OR 弱化を回避）。Auto-Resolved Ambiguity Points が verify command 構文の落とし穴を事前に解消。

#### spec
- Investigation-only Issue として Implementation Steps を 1 ファイル (`docs/reports/skill-loader-cache-behavior.md`) 生成に集約。Size 判定が M → spec 段階で XS 相当と判明し、本来 demote する設計だったが、user override で pr route 強制。実装規模は XS 相当だったが merge phase の `source=ci` test_result observation 取得のため pr route 採用。

#### code
- Investigation document 単体生成。bats / forbidden-expressions / skill-validation 全 PASS。Implementation rework なし。watchdog kill が 1 回発火（00:48 silent window 1800s）するも reconcile で `matches_expected: true` 判定により override success。PR #683 作成完了。

#### review
- light review で MUST 0 件 SHOULD 0 件。CI 全 SUCCESS。reviewDecision は空（auto approve 設定なし）だが MUST なしのため merge OK 判断。

#### merge
- `wait-ci-checks.sh` (`gh pr checks --watch`) が **3h15m hang**。timeout=1200s が効かず watchdog 1800s も発火せず。parent session が手動 kill → `gh pr merge --squash --admin` で復旧。
- 結果として `run-merge.sh` の本体（CI fetch → test_result emit）が実行されず、`source=ci` test_result event は emit されなかった → #679 / #662 / #630 の post-merge observation 連鎖は **本セッションで trigger されず**。
- worktree 削除も branch 使用中のため失敗 → 手動 `--force` で removal。

#### verify
- Pre-merge 3/3 PASS。`file_exists` / `grep "snapshot"` / `grep "reload"` すべて matched。
- Post-merge 2 件は manual（仕様注記、別セッション再現テスト）— Claude executability なし、`phase/verify` 維持。

### Improvement Proposals

1. **`wait-ci-checks.sh` の `gh pr checks --watch` hang 対策（高優先）**
   - 症状: `gh pr checks --watch --interval 60` が CI 全 SUCCESS 状態でも終了せず長時間 hang。`timeout 1200` が効かない事象を観測。
   - 影響: pr route /auto が merge phase で stall し、parent session が手動 kill + `gh pr merge --admin` で復旧する必要あり。`source=ci` test_result emit がスキップされ #679 observation 連鎖が壊れる。
   - 提案: `gh pr checks --watch` 依存をやめ、polling ループ（`gh pr checks` 単発 → conclusion 全 SUCCESS で抜ける）に置き換える、または `timeout` の SIGKILL 強制を `--kill-after` 付きで設定。Issue 起票候補。
2. **`run-merge.sh` 部分 kill 時の `source=ci` test_result emit リカバリ**
   - 症状: `wait-ci-checks.sh` 段階で kill されると後続の CI fetch → emit ロジックが skip。
   - 提案: `/verify` skill 側で post-hoc に `gh run list --workflow=test.yml --json conclusion` を実行し source=ci test_result を補填、または `wait-ci-checks.sh` を非ブロッキング化。Issue 起票候補。
