...
- `docs/reports/skill-loader-cache-behavior.md` を作成し、「session-start snapshot は仕様」として文書化した。"snapshot" と "reload" キーワードを両方含めており、verify command はすべて PASS。
- 日本語ヘッダーで統一（Spec の「英語ヘッダー混在」記述を採用せず）。verify コマンドに影響なし。
- Issue type は Bug だが commit prefix `fix:` を適用。

### Deferred Items

- docs/guide/troubleshooting.md への注記は Post-merge 手動 AC として残留。
- メカニズム確定（セッション内スナップショット vs キャッシュ層）は Post-merge 再現テストに委任。

### Notes for Next Phase

- Changed Files: `docs/reports/skill-loader-cache-behavior.md` のみ（新規作成）
- PR #683 で pre-merge AC 3件すべて PASS 確認済み
- `/review` フェーズでは文書の内容正確性（"snapshot" の定義、"reload" の定義）を重点的に確認すること
