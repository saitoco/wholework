...
- **2026-08-23 の post-merge observation 再評価で FAIL 判定 (documented deferral 付き)**。2026-08-08 の実測コメント (session `46468-1786195191`) に、post-merge AC が要求する状況の実測が記録されていた: Issue #1278 (`skills/verify/SKILL.md` を変更、pr route) merge 直後の同一セッション内 `/verify 1278` が **stale な版の Step 8b を実行し、自動警告も出なかった**。Spec の `## Notes > ### #1168 との関係` が本残存リスクを明示的にスコープ外とし「残存する場合は新規 Issue として起票する」と事前指定していたため、documented deferral として処理し (auto-retry はスキップ)、#1206 を reopen したうえで FAIL マーカーを付与した。実装 (proposal A + C) 自体の不備ではなく、`/verify` の会話セッション単位キャッシュという別機構に起因する既知の残存ギャップの確定という位置づけ
...
- **`/verify` の会話セッション単位 skill キャッシュにより、同一セッション内で直前に merge した skill 修正が反映されないまま実行され、自動警告も出ない (確定・要起票)** — 2026-08-08 実測 (#1278) で確認済み。`/spec`/`/code`/`/review`/`/merge` は `run-*.sh` wrapper 経由で新規 `claude -p` プロセスを起動するためディスク上の最新版を読むが、`/verify` のみ wrapper を持たず親セッション内 `Skill()` 呼び出しで動くため、会話開始時にキャッシュされた版を使い続ける。`modules/verify-classifier.md` の `session=next` 属性は AC 側の誤判定 (UNCERTAIN 化) は防ぐが、`/verify` 自体が stale な手順を実行することは防がない。Spec の `## Notes` が「残存する場合は新規 Issue として起票する」と事前指定した状況が実際に発生したため、本項を新規 Issue 起票対象とする
