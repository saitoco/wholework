...
### 追記 (2026-08-10, /auto #1342 セッションからの再評価)

予測どおり、`/auto` の event-based observation scan により再度 dispatch された。状況に変化なく結論を維持 (UNCERTAIN)。前半「後続フェーズに参照される」の追加確認事例として、同一 `/auto` セッション自身 (Issue #1334/#1335/#1342) が spec→code→review→merge→verify の各フェーズで自身の Spec の Phase Handoff を読み判断に反映している。後半「重複/矛盾記述が減っている」は引き続き測定不能。#1118 は既に CLOSED だが本 AC 自体の再設計は着地しておらず、重複起票はしない。

### 追記 (2026-08-11, /auto --batch --until 953 セッションからの再評価)

`/auto --batch --until "label:theme/observability"` (session `29601-1786367167`) の Batch Completion Report observation scan により 5 回目の dispatch。状況に変化なく結論を維持 (UNCERTAIN)。前半「後続フェーズに参照される」の追加確認事例として、同一セッションが Round 1 で処理した 10 Issue すべてが spec→code→(review→merge→)verify の各フェーズで自身の Spec の Phase Handoff / Consumed Comments を読み判断に反映している (例: #1321 の `/spec` が session `83307-1786372673` の Issue コメントを Root Cause セクションへ統合)。後半「重複/矛盾記述が減っている」は引き続き測定不能。#1118 CLOSED 後も本 AC 自体の再設計は未着手のまま 2 回連続で確認された — この observation AC が dispatch の度に不変の UNCERTAIN を返し続ける状態自体が、re-typing (単一実行判定可能な条件への切替) の必要性を裏付ける追加データ点になっている。それでも重複起票はしない (前回セッションの判断を継続)。

### 追記 (2026-08-15, /auto --batch 1349 1350 1351 1352 セッションからの再評価)
...
### 追記 (2026-08-16, /auto --batch 1132 1348 1072 1363 1095 セッションからの再評価、判定訂正あり)

`/auto --batch 1132 1348 1072 1363 1095` (session `24095-1786827554`) の Batch Completion Report observation scan により8回目の dispatch。本セッションは当初 Condition 6 を PASS と誤判定しチェックボックスを一時的にチェックしたが、既存7回分の一貫した UNCERTAIN 判断との不整合に気づき、コメント投稿・Issue 本文ともに UNCERTAIN へ訂正した (チェックボックスは未チェックに戻した)。前半「後続フェーズに参照される」は本セッションが処理した5 Issue (#1132, #1348, #1072, #1363, #1095) の verify フェーズでも同様に Phase Handoff read を通じて確認できた。後半「重複/矛盾記述が減っている」は引き続き測定不能。8回連続で同一結論。

**教訓 (今回のミスから)**: observation AC を評価する際は、判定を確定する前に Spec の既存 Verify Retrospective 履歴 (先行する再評価エントリ) を必ず確認し、確立された判断パターンとの整合性を優先すること。単発のセッション内証拠だけで判定すると、蓄積された複数セッションの慎重な判断を覆す形になりうる。

### 追記 (2026-08-17, /auto --batch 1096 1229 1243 1302 1273 セッションからの再評価)
...
### 追記 (2026-08-19, /auto #1410 セッションからの再評価)
...
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5236909390
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5236937979
...
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5249926043
- saito / MEMBER / first-class / <!-- wholework-event: type=observation-trigger phase=observation-trigger issue=5 / https://github.com/saitoco/wholework/issues/562#issuecomment-5255739433
- saito / MEMBER / first-class / <!-- wholework-event: type=observation-trigger phase=observation-trigger issue=5 / https://github.com/saitoco/wholework/issues/562#issuecomment-5296374350
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5296471044
- saito / MEMBER / first-class / <!-- wholework-event: type=observation-trigger phase=observation-trigger issue=5 / https://github.com/saitoco/wholework/issues/562#issuecomment-5304271011
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5304441286
- saito / MEMBER / first-class / ## Correction to previous comment / https://github.com/saitoco/wholework/issues/562#issuecomment-5305429683
- saito / MEMBER / first-class / <!-- wholework-event: type=observation-trigger phase=observation-trigger issue=5 / https://github.com/saitoco/wholework/issues/562#issuecomment-5310543871
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5310581997
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5313284655
- saito / MEMBER / first-class / <!-- wholework-event: type=observation-trigger phase=observation-trigger issue=5 / https://github.com/saitoco/wholework/issues/562#issuecomment-5327718635
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5327797855
- saito / MEMBER / first-class / ## Acceptance Test Results / https://github.com/saitoco/wholework/issues/562#issuecomment-5329443463
