...
### Key Decisions
- session_id 伝播を #631 で実装（R1 任せにしない）: AC が `run-auto-sub.sh` への session_id を要求、R1 は 6 metric event 種に限定されるため分離。
- AUTO_SESSION_ID は pointer file（`.tmp/auto-session-current`）経由で親→子に伝播（Bash 呼び出し間 env 非永続のため）。
- データ生成スクリプトは shell + jq で markdown を直接 rendering（progress precedent の JSON 出力方式ではなく）。
- bats hermetic 化のため `--no-github` フラグを導入し、3 ケースは event log 集計を主検証対象とする。
- R1 由来 metric（watchdog_kill / silent_window / token_usage / concurrent_commit）は不在時 0/N/A に degrade。

### Deferred Items
- narrative section の LLM auto-fill は R3（本 Issue は skeleton TBD のみ）。
- `/audit stats --use-event-log` は将来拡張（本 Issue scope 外）。
- batch / 単一親ルートの event 網羅は event coverage 拡張（R1 系後続）に委ねる。
- 過去 event log（session_id 無し時代）への遡及生成は不要（issue スコープ削減）。
- post-merge: 次回 `/auto` 完走後に実レポート生成を確認（observation event=auto-run）。

### Notes for Next Phase
- `/code` 実装前に R1（#630）のマージ状況を再確認。マージ済みなら 6 metric event 種を実値集計、未マージなら degrade を維持。
- `skills/audit/SKILL.md` は validate-skill-syntax.py 制約に注意（frontmatter description は single-line、body に半角 `!` 不可、code fence は既存慣例に倣う）。
- 新スクリプト追加に伴い `docs/structure.md` の件数 `(54 files)`→`(55 files)` + ja ミラー同期を忘れない（AC #9）。R1 が先にマージされたため件数ベースラインは 54 だった。
- `skills/audit/SKILL.md` allowed-tools に `get-auto-session-report.sh:*` を追加（settings.json.template は wildcard 済みで変更不要）。
- grep AC は ripgrep alternation 前提（`session_id` / `SESSION_ID` / `AUTO_SESSION_ID` が対象ファイルに present であることを実装時に確認）。
