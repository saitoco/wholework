...
- なし。Spec の Implementation Steps 1〜7 を順通りに実装した。

### Design Gaps/Ambiguities

- run-code.bats のみ、SKILL.md のパス解決方式が他の bats と異なっていた（SKILLS_DIR 変数が存在するが未使用）。WHOLEWORK_SCRIPT_DIR 移行時に SKILL.md fixture を `$BATS_TEST_TMPDIR/skills/code/SKILL.md` に配置する方針を選択した。
- run-review.bats・run-merge.bats で `wait-ci-checks.sh` が `SCRIPT_DIR` 経由で呼ばれており、WHOLEWORK_SCRIPT_DIR 移行後はモック必須だった（Spec に明示なかったが手順内で自明）。
...
特筆すべき乖離なし。実装は Spec の Implementation Steps 1〜7 を厳密に追っており、受け入れ基準は全項目 PASS（CI 含む）。

### 繰り返し問題パターン

run-merge.sh のリファクタリング（inline PR-state ガード → reconcile 統一）により、旧テスト「post-validation: exits 0 when gh pr view fails (API error — no false alarm)」が削除されたが、等価な新テストが抜け落ちていた（抽出失敗 + gh API 失敗 → false alarm なし の経路）。実装は正しいが、旧テストを置き換える際に対応ケースの網羅性確認が漏れるパターン。テスト置き換え時は「削除テストが検証していたシナリオをすべて新テストでカバーできているか」のチェックを明示的に行うことで防止できる。
