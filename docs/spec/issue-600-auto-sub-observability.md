...
### Key Decisions
- `emit_event()` を `run_phase_with_recovery` 定義の直前に配置し、関数内から参照できる順序を維持した
- `phase_start` は `set +e` の前（`log_file` 設定直後）に呼ぶ実装を採用：flock が正常動作する前提でイベントを確実に記録する
- 既存テストへの `flock` mock 追加は repair attempt として行った（Spec 未記載だったが必須修正）
- Spec 指定の実装手順と実装結果に差異なし（Implementation Steps 通りに実装）

### Deferred Items
- `flock` が macOS で利用不可である点は本番 Ubuntu CI では問題にならないが、ローカルで `run-auto-sub.sh` を直接実行すると `emit_event` が失敗する。graceful fallback（`|| true`）を追加する改善は別 Issue として保留
- `phase_complete` の `duration_sec` フィールドは Issue スキーマにあるが実装スコープ外（Spec 通りに省略）

### Notes for Next Phase
- PR #625 の CI で bats テスト全21ケースが PASS することを確認すること
- `flock` mock は `MOCK_DIR` 経由（PATH 先頭）で差し替え。macOS 本番実行時には `flock` コマンドが存在しないと `emit_event` が失敗する点を review で指摘された場合は既知事項として対応
- verify commands は全て PASS 済み（pre-merge チェックボックス更新完了）
