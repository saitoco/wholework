# Issue #84: terminology: verify command への用語統一と残存 verify command の置換

## Overview

L2 の `<!-- verify: ... -->` 構造を指す用語が 2 つのステアリングドキュメント間で不整合である問題を解消する。

- **Terms SSOT** (`docs/product.md:159`): 「verify command」（旧称: verify command）
- **Forbidden Expressions** (`docs/tech.md:70`): 「verify command → verify command」（末尾の閉じ引用符が欠落した typo あり）

Terms SSOT の「verify command」は L1 `acceptance criteria`（受入条件）との語感衝突が強いため、本 Issue では **「verify command」** を採用し、Terms SSOT を正本化する。あわせて、Python 定数 `KNOWN_VERIFY_COMMANDS` を `KNOWN_VERIFY_COMMAND_TYPES` にリネームし、L2（verify command 全体）と L3（command type = 第一トークン）の階層を命名に反映する。

**スコープ**: 「verify command」/「verify commands」の文字列除去に限定。「verify command」の広範置換（24 ファイル・83 occurrences）は後続 Issue で扱う（Scope Declaration 参照）。

## Changed Files

**測定範囲**: `docs/spec/**` を除くアクティブファイル。測定コマンド: `Grep pattern="verify command|verify commands|validate_verify_hints|verify_hint_errors" exclude=docs/spec/**` および `Grep pattern="KNOWN_VERIFY_COMMANDS" exclude=docs/spec/**`

**Phase A — 用語 SSOT の整理** (4 files):

- `docs/product.md`: Terms 表 (line 159) の「verify command」エントリを「verify command」に置換。旧称記載は「verify command / verify command」として残す（移行履歴保存）
...
**Phase B — コード・ドキュメント一括置換** (7 files, "verify command" 文字列除去):
...
- `modules/browser-verify-security.md`: line 44 のテーブルセル内 "verify commands" → "verify commands"
- `modules/verify-classifier.md`: lines 38, 40, 41 の「a `<!-- verify: ... -->` hint」「without a hint」等の表記を "verify command" に置換
- `skills/verify/SKILL.md`: lines 416, 419 の「verify commands」見出し・本文を "verify commands" に置換
- `skills/spec/SKILL.md`: line 296 の見出し「Section rename — update verify commands simultaneously」を "verify commands" に置換
- `skills/issue/SKILL.md`: line 194 の「verify command assignment」を "verify command assignment" に置換
...
1. **Phase A-1: Terms SSOT 正本化** — `docs/product.md:159` および `docs/ja/product.md:155` の Terms 表で、「verify command」/「受入チェック」エントリを「verify command」に置換。定義文に旧称として "verify command / verify command" を記載（移行履歴保存）。日本語側も同等の書式で更新。(→ Pre-merge 1-3)

2. **Phase A-2: Forbidden Expressions typo 修正** — `docs/tech.md:70` および `docs/ja/tech.md:62` の `verify command` 行で、閉じ引用符欠落 typo (`changed to "verify command)`) を正しく (`changed to "verify command")`) に修正。(parallel with 1) (→ Pre-merge 4-5)

3. **Phase B-1: Python script / bats test の "verify command" 表記置換** — `scripts/validate-skill-syntax.py:272,273,274,512,561,563-564,592` のコメント・docstring・エラーメッセージを "verify command" 系に置換。`tests/validate-skill-syntax.bats:480,482,522,541,560,578` のセクションコメント・テスト名を同様に置換。※関数名・変数名・定数名の実リネームは Step 5 で実施。(after 2) (→ Pre-merge 6-7)

4. **Phase B-2: modules / skills ファイルの "verify command/hints" 表記置換** — `modules/browser-verify-security.md:44`, `modules/verify-classifier.md:38,40,41`, `skills/verify/SKILL.md:416,419`, `skills/spec/SKILL.md:296`, `skills/issue/SKILL.md:194` の「verify command/hints」系表記を "verify command/commands" に置換。`modules/verify-classifier.md` は backtick 介在の複雑パターンに注意（Notes 参照）。(parallel with 3) (→ Pre-merge 8-12)
...
- <!-- verify: section_contains "docs/product.md" "## Terms" "verify command" --> `docs/product.md` Terms 表に "verify command" エントリが存在し、旧称として verify command / verify command が記載されている
- <!-- verify: section_contains "docs/ja/product.md" "## Terms" "verify command" --> `docs/ja/product.md` Terms 表が同様に更新されている
- <!-- verify: file_not_contains "docs/product.md" "| verify command |" --> `docs/product.md` Terms 表の旧 "verify command" 行（pipe-delimited）が削除されている
...
**Phase B — コード・ドキュメント一括置換（"verify command" 除去）**

- <!-- verify: file_not_contains "scripts/validate-skill-syntax.py" "verify command" --> `scripts/validate-skill-syntax.py` の "verify command" 表記（コメント・docstring・エラーメッセージ）が除去されている
- <!-- verify: file_not_contains "tests/validate-skill-syntax.bats" "verify command" --> `tests/validate-skill-syntax.bats` の "verify command" 表記（セクションコメント・テスト名）が除去されている
- <!-- verify: file_not_contains "modules/browser-verify-security.md" "verify commands" --> `modules/browser-verify-security.md:44` の "verify commands" 表記が除去されている
...
- <!-- verify: file_not_contains "skills/verify/SKILL.md" "verify commands" --> `skills/verify/SKILL.md:416,419` の "verify commands" 表記が除去されている
- <!-- verify: file_not_contains "skills/spec/SKILL.md" "verify commands" --> `skills/spec/SKILL.md:296` の "verify commands" 表記が除去されている
- <!-- verify: file_not_contains "skills/issue/SKILL.md" "verify command assignment" --> `skills/issue/SKILL.md:194` の "verify command assignment" 表記が除去されている
...
- 後続 Issue で "verify command" / "verify command" / "受入チェック" の広範な置換（残 24 ファイル・83 occurrences）が追跡されている <!-- verify-type: manual -->
...
- **新用語の選定**: "verify command" を採用
  - 却下案 1: "verify command"（現 Terms SSOT）→ L1 `acceptance criteria` との語感衝突、日本語「受入条件」「受入チェック」の近さで混乱
  - 却下案 2: "verify command"（旧 Forbidden Expressions 移行元）→ 役割（動作指示）が曖昧
...
**[同一ファイル内の非推奨用語 `verify command` の置換] 含まない**

- 本 Issue では Terms SSOT のみ先行修正し、各 skill / module / docs で使用されている "verify command" / "verify command" / "受入チェック"（計 24 ファイル・83 occurrences）の広範な置換は後続 Issue で扱う
- Phase B は **"verify command" / "verify commands" の文字列**に限定（"verify command" や "command hint" 等のサブタイプ参照は対象外）
...
- `docs/spec/issue-66-terms-unify-japanese.md`: 用語統一系 Issue の参考（手法・verify command 設計）
...
本 Issue の出発点は「`verify command`（Forbidden Expressions の旧用語）がコード/ドキュメントに残っている」という単純な残存調査だったが、事前調査中に 2 つのステアリングドキュメント間の用語不整合が判明したため、調査範囲を「Terms SSOT の正本化」まで拡張した。

- `docs/product.md:159` Terms (SSOT for terminology): 「verify command」
- `docs/tech.md:70` Forbidden Expressions: 「verify command → verify command」（末尾の閉じ引用符欠落 typo あり）

ユーザーとの対話で以下を明確化:

1. **L1 `acceptance criteria`（受入条件）** と **L2 の `<!-- verify: ... -->` 構造** は別概念だが、Terms SSOT の「verify command」は日本語「受入チェック」と相まって語感衝突が強い。`/verify` 実行時に「受入条件のチェックボックスに完了チェックを入れる」のような文脈で混乱しやすい。
...
- **Scope 宣言**: 「verify command」の広範置換（24 ファイル・83 occurrences）は本 Issue スコープ外
...
- `docs/product.md` Terms 表の旧エントリ削除確認に当初 `section_not_contains "Formerly called \"verify command\""` を用いようとしたが、新エントリにも旧称を legacy mention として残す方針だと false-negative になるため、`file_not_contains "| verify command |"` (pipe-delimited テーブル行) に変更。テーブル行削除検出の典型パターンとして再利用可能
...
- **verify-classifier.md 置換の scope 拡張**: 当初 Issue では「verify command」文字列のみを対象としたが、同ファイル line 38-41 は「hint」単独表現で `<!-- verify: ... -->` を指すため、本 Issue のスコープ（L2 用語統一）に含めるべきと判断。Spec 作成時に実ファイルを読んで確認した上で Verification section に反映
...
- **Terms SSOT 正本と一般文書の過渡的不整合**: 「verify command」系 83 occurrences の広範置換を同 Issue に含めるか後続 Issue に分離するか → 後続 Issue に分離（`docs/tech.md` Terminology Migration Scope Rule 準拠）。Scope Declaration で明示
