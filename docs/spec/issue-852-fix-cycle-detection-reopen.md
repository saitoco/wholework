...
### Key Decisions
- Fix B (reconcile-phase-state.sh) を最小変更で実装: reopen_ts != null チェックを fallback ブロック先頭に挿入し、phase/verify を completion 根拠に使わない分岐を追加。既存の CLOSED / phase/done 判定は影響なし。
- Fix A (SKILL.md Step 2a) は criterion 1 を OR 拡張し、allowed-tools に gh-graphql.sh を追加。docs/workflow.md を同期更新。
- bats test は既存の "async external commit" テスト直後に追加し、reopen_ts != null のパスをカバー。

### Deferred Items
- Spec には記載なかった `docs/workflow.md` 同期は /code フェーズで対応済み。
- `_completion_code_pr` 関数は phase/verify フォールバックを持たないため変更不要 (設計通り code-patch ルートのみ修正)。

### Notes for Next Phase
- 全 1047 bats テスト PASS 確認済み (新テスト含む)。
- allowed-tools に gh-graphql.sh を追加したため、/auto が reopen_ts 取得の Bash 呼び出しをパーミッション問題なく実行できる。
- post-merge AC は `verify-type: manual` のため /verify フェーズで手動観察が必要。
