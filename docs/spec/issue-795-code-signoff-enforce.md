...
- None: 実装は Spec の Implementation Steps に完全に沿った。挿入位置・コード片・テストケースいずれも Spec 記載の通りに実装。

### Design Gaps/Ambiguities

- `while IFS= read -r _h; do ... done <<< "$_new_commits"` は `_new_commits` が空のときに空文字列の 1 行が read されてしまう可能性があるが、`[[ -n "$_new_commits" ]]` ガードが外側にあるため実害なし。Spec Notes に herestring の bash 3.2+ 互換確認済みの記載があり問題なし。

### Rework

- None: 1 回のコミットで完了。テスト追加後に全 39 件 PASS を確認し修正不要だった。

## Phase Handoff
<!-- phase: code -->

### Key Decisions
- SKILL.md Step 8 への DCO bullet 挿入: Step 11/12 が既存で `-s` 付きだったため、中間 commit を対象とする明示指示が唯一の gap だった
- `run-code.sh` の sign-off 検出は `_PRE_HEAD..HEAD` 範囲で新規 commit のみを対象に限定 — patch route のみ有効 (PR route では HEAD が変わらないため新規 commit を検出しない)
- bats テストは `$MOCK_DIR/git` を直接上書きする方式でケースを分離; setup() の no-op git mock との競合なし

### Deferred Items
- PR route での DCO 検証は CI (dco.yml) に委ねる設計 — この Issue のスコープ外
- post-merge AC (`verify-type: opportunistic`): 次回 `/code` 実行後に git log で確認

### Notes for Next Phase
- 全 AC (CI check 除く) は pre-merge 段階で PASS 確認済み
- CI check (`github_check "gh run list --workflow=test.yml..."`) のみ commit 後の CI 完了待ち
- `_missing_sob` の warning は stderr (`>&2`) なので `/verify` では rubric + file_contains で確認する
