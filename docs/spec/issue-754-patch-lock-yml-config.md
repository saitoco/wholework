...
- <!-- verify: github_check "gh run list --limit=1 --json conclusion --jq '.[0].conclusion' --workflow=test.yml" "success" --> All bats tests pass
...
- N/A (Spec に記載の通り 2 ステップのみ実行: Step 1 英語文追記、Step 2 日本語訳同期)

### Design Gaps/Ambiguities

- `customization.md` line 133 の説明文が長くなったが、テーブルセルの許容範囲内であり問題なし。
...
### Key Decisions
- `docs/guide/customization.md` の `patch-lock-timeout` テーブル行末尾に env var override 説明 (`WHOLEWORK_PATCH_LOCK_TIMEOUT` > このキー > `300`) を追加した。
- `docs/ja/guide/customization.md` の対応行を日本語で同期した。
- コード変更はゼロ — 残ギャップはドキュメントのみで、実装 (detect-config-markers.md、worktree-merge-push.sh) は既存済み。

### Deferred Items
- None

### Notes for Next Phase
- verify はドキュメントのみの変更のため `file_contains` / `section_contains` が主なチェック。
- `rubric "docs/guide/customization.md に patch lock config key が記載され、env var override も合わせて説明されている"` が核心 AC — テーブル行に両方 (key と override) が揃っているか確認する。
