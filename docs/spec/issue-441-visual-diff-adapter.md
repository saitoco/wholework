...
- **visual-diff-adapter.md Step 5d 見出し (repair commit)**: 初回実装時に `#### 5d. Frontend Visual Review Sub-Agent Dispatch` とした見出しが `scripts/check-forbidden-expressions.sh` に検出された。`Dispatch` は `/auto` の旧称 (deprecated term) として禁止されていたため、`#### 5d. Frontend Visual Review Sub-Agent Invocation` に修正する repair commit が必要になった。Spec では見出しの用語指定がなかったため、禁止表現チェックを事前に考慮できなかった。
...
- repair commit 1件: `fix: replace deprecated term in visual-diff-adapter section heading` — `Dispatch` → `Invocation` の見出し変更のみ。機能・内容の変更なし。
