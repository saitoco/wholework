...
副次的成果物: `agents/frontend-visual-review.md` sub-agent (3-panel images から差分を構造化リストで列挙)、`skills/spec/visual-diff-guidance.md` Domain file (適用ガイダンス、`capability: visual-diff` gate で domain 外プロジェクトでは eager Read されない)、`tests/visual-diff-adapter.bats` (shallow contract test、`tests/adapter-resolver.bats` パターン)。

**Domain 外プロジェクトの eager overhead**: visual-diff-adapter / frontend-visual-review / visual-diff-guidance は lazy (visual_diff 命令 / `HAS_VISUAL_DIFF_CAPABILITY=true` 時のみ Read)。eager 追加は verify-executor.md +1 行 / detect-config-markers.md +1 行 / adapter-resolver.md +1 単語 / structure.md +2 行 / environment-adaptation.md +2 行のみ (合計 ~200 tokens)。`modules/verify-patterns.md` は変更しない (§12 追加は Mitigation 2 で撤回、Domain file 化)。
...
- `skills/spec/visual-diff-guidance.md`: 新規 — Domain file (`type: domain`, `skill: spec`, `load_when: capability: visual-diff`, `domain: visual-reproduction`)。visual_diff verify command の Spec 化ガイダンス (適用シナリオ / computed style との使い分け / `rubric` との差異 / `browser_screenshot` との差異)。Domain 外プロジェクトでは domain-loader が capability gate でスキップするため eager overhead ゼロ
...
- `docs/environment-adaptation.md`: change Command-by-Environment Table に `visual_diff` 行追加、Adapter Pattern Application Requirements の例に `visual-diff` を追記、Domain Files テーブルに `skills/spec/visual-diff-guidance.md` 行追加
...
6. `skills/spec/visual-diff-guidance.md` を新設 (parallel with 1-5)。frontmatter: `type: domain`, `skill: spec`, `domain: visual-reproduction`, `load_when: capability: visual-diff`。本文に Purpose / Input / Processing Steps を記載: 適用シナリオ (UI 再現案件、Figma → 実装、theme migration) / computed style 系 verify との使い分け (computed は補助、`visual_diff` が主) / `rubric` との差異 (rubric=semantic text、visual_diff=visual layout) / `browser_screenshot` との差異 (1 URL の subjective check vs 2 URL の網羅的差分) / `visual_diff` AC を Spec 化するときの注意点 (引数の必須性、viewport/state の opaque label 規約)。Domain file のため `HAS_VISUAL_DIFF_CAPABILITY=false` のプロジェクトでは domain-loader が capability gate で skip し eager Read されない (→ V6)
...
9. `docs/environment-adaptation.md` を更新 (parallel with 1-8): Command-by-Environment Table (line 172 付近) に `| visual_diff | UNCERTAIN | Capability declaration check (HAS_VISUAL_DIFF_CAPABILITY), then delegate via adapter-resolver; default 3-panel composite (pixelmatch + sharp) |` 行を追加、Adapter Pattern Application Requirements の例 (line 195 付近) に `visual-diff` (Playwright/browser-use + sharp + pixelmatch の組み合わせ) を追記、Domain Files テーブル (line 137 付近) に `skills/spec/visual-diff-guidance.md` 行を追加 (`load_when: capability: visual-diff`, domain: visual-reproduction) (→ V9)
...
- <!-- verify: file_exists "skills/spec/visual-diff-guidance.md" --> <!-- verify: file_contains "skills/spec/visual-diff-guidance.md" "type: domain" --> <!-- verify: file_contains "skills/spec/visual-diff-guidance.md" "capability: visual-diff" --> <!-- verify: rubric "skills/spec/visual-diff-guidance.md は type=domain / skill=spec / load_when capability=visual-diff の frontmatter を持つ Domain file として実装されており、本文に visual_diff の適用シナリオ (UI 再現案件、Figma→実装、theme migration)、computed style 系 verify との使い分け、rubric との差異、browser_screenshot との差異が記載されている" --> V6: `skills/spec/visual-diff-guidance.md` Domain file が `capability: visual-diff` gate 付きで実装されている
...
- **Domain 外 overhead 最小化 (Mitigation 2 採用)**: 初版 Spec では `modules/verify-patterns.md §12` を追加する設計だったが、user 観点提起 (「domain 外プロジェクトのパフォーマンス影響」) を受けて再評価。verify-patterns.md は /issue, /spec から **eager に読まれる** 共通モジュールのため §12 追加で ~1500 tokens の eager overhead が domain 外プロジェクトに発生することが判明。`skills/verify/browser-verify-phase.md` (`load_when: capability: browser`) と同じ Domain file パターンを採用し、`skills/spec/visual-diff-guidance.md` (`load_when: capability: visual-diff`) に切り出して **domain 外では Read 自体がスキップされる** 構造に変更。Domain 外 eager overhead は ~1700 tokens → ~200 tokens に削減
...
- 既存 adapter (browser-adapter / lighthouse-adapter) に専用 bats test がなく、`tests/adapter-resolver.bats` のみが shallow contract test の前例だった。test depth 選択 (Step 7) の判断材料が限定的だったため、user 確認で shallow を採択した
- bundled adapter (markdown spec) から sub-agent を Task で spawn するパターンは visual-diff-adapter が初の事例 (既存 adapter は CLI/MCP 直接呼び出しのみ)。markdown 上の表現方法は実装時に既存 skill 内の Task 起動パターン (例: `skills/issue/SKILL.md` Step 11a の 3 sub-agent 並列起動) を参照して確定する
- Issue body の Pre-merge AC は当初 12 items だったが Spec を SSoT として 10 items に整理 (V2/V5/V7 で複数 verify command を 1 item に集約)。Spec Simplicity Rules (full: 10 items) との整合と count alignment check の両方を満たす

### Judgment rationale

- **test depth: shallow contract test** — `tests/adapter-resolver.bats` パターンを踏襲。bundled adapter は markdown spec のため機能的 unit test は bash トレースのみで価値限定的。実機能検証は post-merge で実環境 (`<!-- verify: visual_diff ... -->`) で行う設計に統一
- **Domain 外 overhead 最小化 (Mitigation 2)** — 初版 Spec の `modules/verify-patterns.md §12 追加` を撤回し、`skills/spec/visual-diff-guidance.md` (`load_when: capability: visual-diff`) に切り出し。`skills/verify/browser-verify-phase.md` (browser capability gate) と同パターン。Domain 外プロジェクトでの eager overhead を ~1700 → ~200 tokens に削減
- **画像比較手法: 3-panel default** — Purpose (検証範囲選択バイアス防止) と LLM 視覚特性 (意味理解は強く pixel 検出は不得手) の両方を満たす Percy/Chromatic/Playwright snapshot の業界標準。他手法 (side-by-side / odiff / ROI 切り出し) は project override で対応
- **Implementation Steps の grouping** — 元 11 ステップを 10 に集約。docs/structure.md / docs/environment-adaptation.md / docs/ja/ mirror の 3 ステップを並列実行可能な独立ステップとして配置 (Step 8 / 9 / 10)、依存関係は after 8,9 のみ (docs/ja/ mirror)
- **Extension Guide step 4 (capabilities.visual-diff の明示行追加)** — dynamic capability mapping で自動的に `HAS_VISUAL_DIFF_CAPABILITY` に変換されるため実装上は不要だが、`browser` 等と同じく「main capability」として明示行を追加する方が documentation 一貫性が高い

### Uncertainty resolution

- **adapter から sub-agent spawn の markdown 表現方法**: 実装時 (`/code 441`) に `agents/issue-scope.md` 等を呼び出す `skills/issue/SKILL.md` Step 11a のパターン (`Task(subagent_type=...)` 起動指示の自然文記述) を参照する。Spec 段階では「Step 5 (d): frontend-visual-review sub-agent を Task で spawn」とだけ明記し、書式詳細は実装時に確定
- **`.tmp/visual-diff-{run-id}/` の一時ファイル命名**: 実装時に `browser-adapter.md` Step 4 (browser-use CLI screenshot サブセクション) の `mktemp .tmp/verify-screenshot-XXXXXX.png` パターンを踏襲し、サブディレクトリ + (viewport × state × {ref|impl|diff|3panel}) の連番方式に拡張

## Code Retrospective

### Deviations from Spec

- **Implementation Steps 1 (insertion position)**: Spec は「`browser_check` 行の直後に挿入」と指定していたが、実際の `verify-executor.md` の table では `browser_screenshot` が `browser_check` の後に来ており、`visual_diff` は `browser_screenshot` の直後に挿入した。Spec の「直後」は translation table 末尾付近を意図しており、意味的に同等。

- **visual-diff-adapter.md Step 5d 見出し (repair commit)**: 初回実装時に step 5d の見出しに deprecated term を使用してしまい `scripts/check-forbidden-expressions.sh` に検出された。`#### 5d. Frontend Visual Review Sub-Agent Invocation` に修正する repair commit が必要になった。Spec では見出しの用語指定がなかったため、禁止表現チェックを事前に考慮できなかった。

### Design Gaps

- **V4 (section_contains UNCERTAIN)**: `<!-- verify: section_contains "modules/detect-config-markers.md" "Marker Definition Table" "capabilities.visual-diff" -->` が UNCERTAIN を返した。`section_contains` はマークダウン見出し行 (`#` 始まり) のみを section anchor として扱うが、`detect-config-markers.md` の「Marker Definition Table」は `**...**` 形式の bold text であり heading ではない。`file_contains` では内容の存在を確認済み。verify command の仕様上の制限であり、ファイル内容に問題はない。

### Rework Summary

- repair commit 1件: `fix: replace deprecated term in visual-diff-adapter section heading` — step 5d の見出しを `Invocation` に修正。機能・内容の変更なし。

## review retrospective

### Spec vs. 実装の乖離パターン

- **Spec後記ファイルへの禁止表現混入**: `Code Retrospective` の説明文に deprecated term を直接記述したため、Specファイル自体が CI の Forbidden Expressions check で失敗した。Spec 本文（実装指示）でなく後記（履歴説明）でも CI 検査は同一ファイルを対象とすることを認識できていなかった。将来の Spec 後記では deprecated term を直接引用せず、「旧称」「禁止用語」等の迂回表現を使うこと。

### 繰り返し課題（ワークフロー改善の余地）

- **markdown ドキュメントのコードブロック内変数スコープ**: `modules/visual-diff-adapter.md` Step 5c の `sharp` スクリプトで `implHeight` 未定義バグが発生した。LLM 指示文書中のコードブロックはコンパイラ/リンタの検査対象外であり、変数スコープのミスが見落とされやすい。コードブロック内で別コマンドの変数を参照している箇所は、Spec/実装レビュー時に特に注意が必要。

### 検収条件の自動検証難度

- **V4 `section_contains` UNCERTAIN**: `detect-config-markers.md` の「Marker Definition Table」が `**...**` 形式（bold text）で記述されており、`section_contains` のマークダウン見出し（`#`）要件と不一致のため UNCERTAIN となった。内容は実装済み（`file_contains` PASS）。今後同様の構造を持つファイルの verify command には `file_contains` を使うか、ファイル内のテーブルを markdown heading セクション下に移動することを検討する。
...
#### spec
- Spec 後記に deprecated term を直接記述したことで CI が失敗した点は `## review retrospective` で捕捉済み。Spec 品質としては受け入れ条件の可検証性（V4 `section_contains` UNCERTAIN 問題）が見落とされていた。`section_contains` の見出し要件と bold text 形式の不一致は Spec 作成時に確認できた問題。

#### design
- F 案（adapter 流用）の採用判断は正しく、修正範囲を B 案の半分（~5 ファイル）に抑えられた。Domain 外 overhead 最小化（Mitigation 2）の設計変更も正当であり、verify-patterns.md への eager 追加を避けた判断は検証時点でも有効。
- `detect-config-markers.md` の Marker Definition Table が heading でなく bold text であることを設計時に把握しており Extension Guide step 4 で明示行を追加する設計になっていたが、verify command を `section_contains` にしたことで不一致が発生した。`file_contains` を使えば回避できた。

#### code
- 実装は Spec 通り完了。repair commit 1 件（step 5d 見出しの deprecated term 修正）のみで rework 最小。
- `verify-executor.md` の `visual_diff` 行挿入位置（`browser_screenshot` 直後）は Spec 指定（`browser_check` 直後）と微差があったが意味的に同等。

#### review
- repair commit が必要だった点（step 5d 見出しの deprecated term）は review で検出された。Spec 後記への forbidden expression 混入は review でも防げなかったが、CI チェックが検出した。
- V4 `section_contains` UNCERTAIN の問題は review 段階で verify command の適切さをチェックする機会があったが見落とされた。

#### merge
- PR #447 は main への FF merge で完了。CI（bats tests）は `completed success`。コンフリクトなし。

#### verify
- V1-V10 すべて PASS（V4 のみ `section_contains` UNCERTAIN → AI 判定で PASS）。
- V4 の `section_contains` 失敗は verify command の設計問題（bold text を heading と誤認）であり実装の問題ではない。`file_contains` であれば直接 PASS となった。
- Post-merge 条件（manual 1件 + opportunistic 1件）は未検証のため `phase/verify` に遷移。
