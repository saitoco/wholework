...
- **[起票済み #1070]** 外部 kill 検出時の completion check 先行を明文化すべき。merge phase は kill 前に実処理が完了していたが、親セッションは Step 6 の External kill pre-check に直行し、手順どおりなら respawn を指示される状態になった (merge 済み PR への再実行は非冪等フェーズで有害) 。
  - **当初の記述は不正確だったため訂正する**: 「Step 6 の事前チェックが completion check より前に置かれている」と書いたが、実際には **Step 4 の pr route step 8 / step 11 に「review/merge 失敗時は completion check → `matches_expected: true` なら success override」が既に明記されており、順序自体は正しい**。今回の merge kill は step 11 で処理され、Step 6 に到達すべきではなかった。
  - **真の問題は Step 6 の記述が逸脱を誘発する点**: 見出しが `On Failure: 3-Tier Recovery`、本文冒頭が「If any phase exits with a non-zero exit code」、続いて「**Always first**」とあるため、**あらゆる非ゼロ終了の入口**として読める。External kill pre-check の「before Tier 1」は Tier 1 に対する相対位置であって、Step 4 completion check との前後関係は書かれていない。本セッションの親エージェント自身がこの読み方をして Step 6 に直行した — SKILL.md の記述が実行者の逸脱を誘発した事例であり、単なる実行ミスとして扱うべきではない。
- **[No action: #1049 と同根のため同 Issue にコメントで補強済み]** `restore_auto_session_pointer` が本 `/auto` セッション (`25781-1785289625`) とは別の session_id (`46196-1785292524`) を復元した。`.tmp/auto-session-*` の残存数は実測 **458 件** で、PGID 再利用による衝突確率は無視できない。`#1049` (manual_intervention event の under-counting 調査) は候補として「pointer file が存在せず restore が fail」を挙げているが、今回観測したのは「pointer は存在するが別セッションの値を返す」ケースであり、復元の成否ではなく**鮮度**の検証が必要という点で同 Issue の修正候補 B では塞げない。`.tmp/auto-session-${PGID}` は PGID をキーにしており、**PGID は OS により再利用される**ため、過去セッションの残存ポインタファイルに新しい Bash 呼び出しの PGID が一致しうる。結果として verify phase の `phase_start` / `verify_user_confirm` / `phase_complete` イベントが誤った session_id で記録され、L3 session retrospective の境界検出と `get-auto-session-report.sh` の集計が汚染される。ポインタファイルに書き込み時刻または生成元セッションの検証子を含め、読み出し側で鮮度を確認する必要がある。`.tmp/auto-session-current` も同じ汚染経路を持つ。
- **[No action: #1054 にガイドライン設計への入力としてコメント済み]** AC10 のような「該当行が 0 件なら自動的に充足する」形の AC (vacuous truth) は、`#1054` (不在を検証する AC に参照点を持たせる) と構造的に同型。`modules/verify-patterns.md` のガイドラインが「ファイルに文字列が無いこと」だけでなく「集合・テーブルに条件を満たす要素が無いこと」も射程に含むかを `#1054` 実装時に判断してもらう。#1062 の AC 自体は verify 側の実データ照合で補強済みのため修正不要。
