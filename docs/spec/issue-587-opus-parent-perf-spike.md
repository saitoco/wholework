...
### Key Decisions
- Implementation Step 5（ja docs 3 件の sync 更新）を本 Issue にスコープ追加: `check-translation-sync.sh --fail-if-outdated` AC が pre-existing の OUTDATED 状態で失敗するため、スコープ拡張コストが軽微と判断
- `docs/ja/reports/` への ja 版レポート作成を含める: `translation-workflow.md` が `docs/reports/` を翻訳除外と定義するが、既存の `docs/ja/reports/` ディレクトリと複数の ja 版レポートが存在し、Issue AC が明示的に要求するため実態に従う
- 候補 Issue 選定（Step 1）は `/code` 実装者に委任: 実施日のバックログ状況に依存するため、Spec では XS/S/M 優先の選定方針のみ記載

### Deferred Items
- Phase 2（`--batch` List mode 計測）: Phase 1 で劣化兆候が観測された場合のみ実施。本 Spec の範囲外。
- `docs/translation-workflow.md` の `docs/reports/` 除外ポリシーと実態（ja 版レポートあり）の矛盾解消: 別 Issue 化が望ましいが本 Issue には含めない

### Notes for Next Phase
- **実行型 spike**: `/code` はコードを書くのではなく、実際に `/auto` を 5 Issue 連続実行して結果を記録する。通常の code フェーズとは性質が異なる。
- **Fable 5 停止下の現在モデル**: 本セッションが Sonnet 4.6 で動作中であっても、`/auto` 親は現在の設定に従う（Opus 4.8 または同等）。モデル確認を先に行うこと。
- **レポートファイル名の日付**: 実施日が 2026-06-14 以外になる場合、ファイル名と全 AC の日付を実施日に変更すること（現在 10 件の AC に `2026-06-14` がハードコード）。
- **translation sync 事前確認**: Step 5 実行前に `scripts/check-translation-sync.sh` を実行して OUTDATED ファイルを特定し、該当 diff のみを ja に適用すること。
