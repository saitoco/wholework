...
- **2026-07-12 追記 (`/review 999` 経由の observation cascade)**: AC3 (`event=pr-review-full`) を PASS 判定。`/review 999` の Event-based observation scan から本 Issue にマッチし nested `/verify 930` が dispatch されたが、実行エージェントが `/verify` Step 2 (`git checkout main`) 実行前に `detect-foreign-worktree.sh "verify/issue-930"` を能動的に確認したところ `foreign /Users/saito/src/wholework` (= 呼び出し元 `/review` の worktree `review+pr-999` を継承した状態) を検出した。誤コミット発生前に outer `/review` の Worktree Exit (push-and-remove、push 完了済みのため安全) を先に完了させ、main リポジトリへ復帰してから nested `/verify` を再開したところ `detect-foreign-worktree.sh` は `none` を返し、専用 `verify/issue-930` worktree を正常に作成できた。最終的に誤コミット・worktree混入は発生しなかったが、**この `foreign` 検出は `/verify` 自身の Step 3 (Worktree Entry) 到達時ではなく実行エージェントの能動的な事前確認によるもの**であり、`/verify` Step 2 (base branch checkout) 自体には foreign-worktree ガードが構造的に存在しない残存ギャップを確認した。詳細は Improvement Proposals 参照。全 Post-merge AC (AC3・AC4) が PASS したため `phase/done` へ遷移し、Issue はクローズ済み (既に CLOSED) のまま。
...
- 本 PR (#933) 自身が次回 `/review --full` を実行した際の `pr-review-full` イベント opportunistic verify dispatch で、Post-merge observation AC の実挙動を観察できる。(2026-07-12 実施済み — 上記 verify セクション参照)
- **`skills/verify/SKILL.md` Step 2 (base branch checkout) に foreign-worktree ガードがない**: 現在の foreign-worktree 検出は `/verify` Step 3 (Worktree Entry、`modules/worktree-lifecycle.md` 経由) でのみ機能する。しかし Step 2 の `git checkout "${BASE_BRANCH}"` / `git pull` は Step 3 より前に実行されるため、呼び出し元が worktree を継承した状態でこのステップに到達すると、`git checkout main` が (a) 別 worktree で main が既にチェックアウトされている場合は失敗する、または (b) 失敗しない構成では呼び出し元の worktree のブランチを意図せず切り替えてしまう可能性がある。今回は実行エージェントが能動的に事前確認して回避したが、構造的な保護ではない。提案: Step 2 の直前に `detect-foreign-worktree.sh` の "foreign" 検出と "その場合は先に呼び出し元の Worktree Exit を要求する" ガードを追加するか、Step 2/3 の順序を入れ替えて Worktree Entry (foreign 検出込み) を先に実行してから base branch checkout を worktree 内で行う設計に変更する。
- **`/review` SKILL.md の実行順序遵守は文書順序のみに依存**: 「Worktree Exit (push-and-remove)」→「Opportunistic Verification」の順序は文書上正しいが、これを機械的に強制する仕組み (例: Opportunistic Verification 冒頭で `ENTERED_WORKTREE` が既に exit 済みであることをアサートする) がなく、実行時の順序取り違えを防げない。今回は実行エージェントが気づいて自己修正したが、気づかなければ #930 と同型の誤コミットが再発しうる。
