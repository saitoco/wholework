...
- PR #622 を squash merge（`--delete-branch`）で main にマージした。CI 全 SUCCESS・review APPROVED 状態で mergeable=true を確認してから実行。
- BASE_BRANCH=main のため `closes #617` が自動的に Issue を close する（手動 close 不要）。
- Phase Handoff は review フェーズのもの（4 視点問題なし・CI SUCCESS）を引き継ぎ、verify フェーズへ渡す。

### Deferred Items

- Post-merge AC（次回 Tier 3 起動時に `orchestration-recoveries.md` へエントリが自動追加されること）は observation 型で verify フェーズが確認する。
- XL 並行書き込みリスクは `WHOLEWORK_MAX_RECOVERY_SUBAGENTS=1` で許容済み（follow-up issue なし）。

### Notes for Next Phase

- verify command: `grep "recovery-sub-agent.*Available" "skills/auto/SKILL.md"` および `bats tests/auto-recovery.bats`（5/5 PASS 済み）で pre-merge AC を再確認可。
- Post-merge observation AC（Tier 3 起動イベント検出）は verify フェーズで `opportunistic-search.sh --event auto-run` が自動評価する設計。
- `orchestration-recoveries.md` の Sources テーブルが `Available (#617 shipped)` に更新済みなので monitoring 時は Source 列を参照する。
