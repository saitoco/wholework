...
### Key Decisions
- grep パターンは `[[:space:]]*:[[:space:]]*` 形式とし、JSON キー・値間のスペース有無を吸収する
- EXIT_CODE は変更しない（warn-only 方針を貫徹）
- PR 番号取得は `jq -r '.actual.pr_number // empty'` で null/空を安全に処理

### Deferred Items
- patch route の事後衝突検出は Out of scope（Issue で明示）
- merge phase 自体の自動解決ロジックも Out of scope

### Notes for Next Phase
- bats テスト 2 ケースで warn 出力あり/なしを検証済み（PR #613）
- CI green 確認は github_check verify command で post-push 確認が必要
- `gh-pr-merge-status.sh` の JSON は `"reason": "conflicts"` とスペースあり形式なので review でも同パターンに注意
