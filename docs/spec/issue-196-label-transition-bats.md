...
#### spec
- Issue 本文の前提（「bats テストが存在しない」）が実際の状態と矛盾していることを `/spec` フェーズで調査・検出し、自動解決方針を設定した。これにより実装スコープが 1 テストケース追加に絞り込まれ、不要な実装を防いだ。

#### design
- N/A（デザインフェーズは spec と統合）

#### code
- 逸脱・ギャップ・リワークなし。既存テストパターンに従った単純な 1 ケース追加で、設計通りに実装された。

#### review
- パッチルートのため PR レビューなし。変更範囲がテスト 1 ケース追加のみと小さく、レビュー省略のリスクは低い。

#### merge
- パッチルート（main 直コミット）。単一コミット `cda03ee` で完結、コンフリクトなし。

#### verify
- Pre-merge 条件（ファイル存在確認・bats 全17テスト PASS）はいずれも PASS。
- Post-merge 条件（遷移パターン検証）はヒントなしのため自動検証対象外。ユーザー確認ガイドとして提示した。
