...
(以下は `/issue` フェーズの Issue Retrospective コメントを転記したもの)

### Auto-Resolved Ambiguity Points (issue phase)

non-interactive モードで以下 3 点を自動解決し、Acceptance Criteria を更新済:

1. **`file_contains` の OR パターン不正** — `file_contains` は固定文字列マッチ (regex 非対応) のため OR 演算子は機能しない。3 つの個別 `file_contains` (`L1 Report` / `L2 Assisted` / `L3 Unattended`) に分割。根拠: `verify-patterns.md §1`
2. **無効な observation event 名** — `skill-invocation` は `verify-classifier.md` の有効 event 名に含まれない。post-merge 条件を `verify-type: manual` に変更 (#700 着地後の人間観察が前提)
3. **ガイドのコンテンツ未検証** — `file_exists` のみでは内容を確認できないため `rubric` (セマンティック確認) + 補助 `file_contains "L2 Assisted"` を追加。根拠: `verify-patterns.md §9`

## spec retrospective

### Minor observations
- 「薄い実装 (thin enforcement)」と銘打たれた Issue でも、新規 `.wholework.yml` key + 新規 module + 新規 guide の組み合わせは structure.md / tech.md / customization.md / guide index / workflow.md と複数の steering・project doc へ整合更新が波及する。config 1 行追加でも doc consistency surface は小さくない
- `get-config-value.sh` が汎用 flat-key 抽出機だったため、新 key 追加に伴うスクリプト/bats 変更が不要だった (marker は LLM が読む module 側 SSoT のみ)。設定 key 追加コストを下げる既存設計の利点を確認

### Judgment rationale
- Issue body の `HAS_AUTONOMY_TIER` は How レベルの実装詳細が body に漏れたもの。marker テーブル慣習 (値 key は plain 名) に従い `AUTONOMY_TIER` を採用。`/issue` (What) / `/spec` (How) 境界に照らすと、変数名は spec の裁量で確定してよい
- structure.md の count 確認 AC は maintenance rule が推奨するが、9 AC の verbatim 整合と issue body 改変リスク回避を優先し独立 AC 化を見送った。doc consistency は実装 Step + `/review` で担保するという判断 ([[verify-patterns]] の verbatim-sync rule と count-alignment warning の重み付け)
- 経路 D (Detached subprocess) は Issue body が非対応と明記。カタログには載せるがマトリクスの許可列から外す、という「網羅記載 + 明示除外」で整合

### Uncertainty resolution
- 参照する Claude Code primitive (`CronCreate` / `ScheduleWakeup` / `CronList` / `/loop` / `/goal`) の実在性は本セッションの deferred tool / skill 一覧で確認。module は descriptive SSoT のため runtime 呼び出しはなく、名称整合で十分と判断
- enforcement の実配線 (skill frontmatter 宣言 + loader 照合) は #700-703 へ委譲という設計前提を明確化。本 Issue でマトリクスを先に確定させることで、後続 tail 拡張が tier ゲートを最初から参照できる

## Phase Handoff
<!-- phase: spec -->

### Key Decisions
- config marker の export 変数は `AUTONOMY_TIER` (string `L1`/`L2`/`L3`、default `L1`)。Issue body の `HAS_AUTONOMY_TIER` ではなく marker テーブル慣習 (値 key は plain 名) に合わせる
- `modules/autonomy-tier.md` は 4 セクション標準構造。Tier × L0 write の per-surface 判定は `modules/l0-surfaces.md` を参照 (重複定義しない)
- 経路 D は非対応をカタログに明記、マトリクスの許可列は A/B/C/E のみ
- 本 Issue は thin enforcement: SKILL.md / agents は編集せず、実ゲーティングは #700-703 に委譲
...
- 新規テーブル/リストには `(exhaustive)` マーカーを付す (skill-dev-checks)。L0 layer / 経路カタログ / Tier マトリクスはいずれも exhaustive
- pre-merge 9 AC は Issue body と verbatim 一致済。AC8 (`rubric`) は AC9 (`file_contains "L2 Assisted"`) を補助とする組み合わせ。ガイドには必ず `L2 Assisted` の文字列を含めること
- structure.md は `(37 files)` → `(38 files)`、Key modules 箇条書き、`guide/` 説明リストの 3 箇所を更新。漏れると `/review` doc consistency で指摘される
- translation sync は top-level docs (structure/tech/workflow) のミラーのみ本 PR 対象 (translation-workflow.md の top-level 限定 obligation)
