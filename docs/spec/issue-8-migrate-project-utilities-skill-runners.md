...
### 設計からの逸脱
- `skills/*/SKILL.md` スタブファイルの作成: Spec には記載がなかったが、スキルランナーテストが `SKILL.md` を参照するため、テストが通るよう6つのスタブ SKILL.md を新規作成した。スタブは `ARGUMENTS:` 行を含まない最小限の構成とし、テストの誤マッチを防いだ
- `validate-skill-syntax.py` のスキップ: スクリプトが wholework に未移植のため実行不可。CI の同等ジョブで代替される

### 設計の不備・曖昧さ
- スタブ SKILL.md の必要性が Spec に未記載。`run-*.sh` テストは SKILL.md が存在しないと失敗するが、Spec は「テストが PASS すること」のみ要求しており、その前提条件（SKILL.md の存在）に言及がなかった
- SKILL.md スタブに `ARGUMENTS:` 行を含めると、テストの `grep -o 'ARGUMENTS:.*' | head -1` が誤ったマッチをすることを発見。スタブからは `ARGUMENTS:` 行を除外する必要があった

### 手戻り
- 最初のスタブ SKILL.md に `ARGUMENTS: {{ISSUE_NUMBER}}` を含めたため、run-code.bats と run-issue.bats の複数テストが失敗。スタブを修正して再テストした

## spec レトロスペクティブ

### 軽微な観察
- Issue 本文のテスト数「~13 本」は近似値であり、実際は 10 本。`get-sub-issue-graph.bats`, `run-auto-sub.bats`, `run-spec.bats` は claude-config にも存在しない。Issue の `/issue` フェーズで正確なファイル数を確定していれば Spec で確認する手間が省けた
- run-*.sh は全て同一パターン（SKILL.md body 抽出 → claude -p 実行）であるため、移植作業は並行可能度が高い

### 判断経緯
- 実装ステップを5つにグループ化: プロジェクトユーティリティ（Step 1）、スタンドアロンユーティリティ（Step 2）、スキルランナー（Step 3）は並行可能とし、run-auto-sub.sh（Step 4）は他スクリプトへの依存があるため後続とした
- #7 の Spec パターン（テンプレート構造、英語化スコープ、migration-notes 記載方針）を踏襲し、一貫性を確保した

### 不確定要素の解決
- 特になし（移植作業のため外部仕様への依存なし。run-*.sh の SKILL_FILE パス解決は wholework のディレクトリ構造で検証済み）

## review レトロスペクティブ

### 設計と実装の乖離パターン
- `run-auto-sub.sh` に SIZE 再取得の欠落という実際のロジックバグが検出された。spec フェーズで Size が設定される通常フロー（`/auto 123`）でエラー終了する問題だが、Spec にこのフローの前提条件が明記されていなかったため、レビューまで発見されなかった
- `docs/structure.md` に `tests/` ディレクトリの記載が Issue #7 の時点から欠落しており、本 PR でもドキュメント更新が漏れていた。移植 PR では新規ディレクトリ追加時に Steering Document (structure.md) の更新を Spec の「変更対象ファイル」に明示するとよい

### 頻出する指摘事項
- 特になし（指摘2件は異なる種別。同種の指摘の繰り返しパターンは見られない）

### 受け入れ条件の検証困難さ
- 全14条件が PASS となり UNCERTAIN/FAIL は0件。受け入れチェックの品質が高く、CI 未設定でもローカル実行でカバーできた
- `command "bats ..."` ヒントは CI があれば自動検証できるが、なければローカル実行に頼ることになる。CI を整備するか `github_check` ヒントに置き換えると自動検証の確実性が上がる
...
#### spec
- Issue 本文のテスト数「~13 本」は近似値だったが、受け入れ条件は全14件が verify command付きで自動検証可能な品質だった。spec レトロで記録済みの観察通り

#### design
- 特になし（Spec は実装と一致しており、設計判断に大きな乖離はなかった）

#### code
- `skills/*/SKILL.md` スタブの追加が Spec 未記載で手戻りが発生した（code レトロ記録済み）。verify では問題として顕在化しなかった（テストは全件 PASS）

#### review
- run-auto-sub.sh の SIZE 再取得欠落バグが review フェーズで検出・修正済みであり、verify 時点では問題なし。レビューが有効に機能した

#### merge
- Squash merge により c1d8875 として main に取り込まれた。コンフリクト等の問題なし

#### verify
- 全14条件が PASS（file_exists ×10、file_not_contains ×1、command ×2、grep ×1）
- bats テスト: get-issue-size.bats 9/9、run-code.bats 15/15 全件成功
- Issue は PR の closes #8 により自動クローズ済み。スムーズに完了

### 改善提案
- `command "bats ..."` ヒントは CI 整備後に `github_check` ヒントへ置き換えると、--non-interactive 環境での自動検証がより確実になる（ローカル実行不要になる）
