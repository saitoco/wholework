...
- **[優先度引き上げ、Tier 2 相当]** AC11 (`observation event=pr-review-full config=capabilities.workflow`) は fork context の `/review` 実行では原理的に発火条件と実証条件が乖離する (`when=` 軸に "実行が main/fork のどちらか" を表現する手段がない)。1 日で 3 回 (重複含め実質 4 回) の同一結論再現により、`modules/observation-trigger.md` の `when=` 宣言可能軸への execution-context 軸追加、または AC 条件文自体を「main context での直接実行時に限る」旨に明記する改善が実証的に裏付けられた。Issue filing restraint 方針 (既存 Issue 追記 → メモリ → 起票の順で保持先を検討) に従い、まずは本節への記録に留め、次回 `/verify` の Step 16 (Improvement Proposals collection) での起票判断に委ねる
