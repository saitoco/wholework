...
#### issue
- AC verify command 5 点改善（rubric 化、customization.md AC 追加、load_when 値検証、github_check 追加）が機械検証カバレッジを 8/10 から 10/10 に引き上げた。`detect-config-markers.md` への記載パターンが Marker Definition Table 行 / Dynamic Capability Mapping のどちらでも成立しうる曖昧さを rubric で吸収した判断が verify 段で機能した（PASS）
- 規約「実装時間=spec time」を勘違いせず、verify 段から振り返ると、AC10（bats github_check）は PR 作成直後の PENDING を想定し Notes に明記していた点が良い前例（手動再 verify が不要になる構造）

#### spec
- Opus / xhigh での L サイズ設計が 8 ファイル・7 ステップに収まり、code フェーズで rework 0（1 回目で全 CI green）。`scripts/validate-skill-syntax.py` の `KNOWN_TOOLS` + `BODY_TOOL_CHECK_SKIP` 二重同期を「**必須**」と Notes に明記した判断が実装段でそのまま機能した（hidden gotcha の正しい予見）
- Spec の「実装ステップ 3(c)」（args 渡し形式）が code フェーズで本文反映漏れし、review で指摘されて修正された（Code/Review Retrospective に記録あり）。spec の Design Gaps/Ambiguities セクションに書かれた知見を「Done」扱いで先に閉じず、当該実装ステップ本文に直接補記する習慣が次回以降の改善点

#### code
- 1 回目で全 CI green（bats 697件 + validate-skill-syntax + forbidden-expressions + translation-sync）。L サイズで rework 0 は良好
- args 渡し形式の本文反映漏れ（spec retrospective からの転記漏れ）は review で検出され修正されたが、code 段でセルフチェックする手順があれば早期検出可能だった

#### review
- `--full` での review-spec + review-bug×2 並列 + 検証 sub-agent が想定通り機能。「N-vote adversarial verify」表現の spec→Domain file 転記時の意味乖離を検出し、実装に合わせて表現を修正（Review Retrospective 記録あり）。spike の aspirational 表現が実装時に縮小する場合の transcription 検査として有効に働いた
- CI green を待ってからの review wait が想定通り（CI が PR 作成直後の PENDING を完了まで待ち、merge precondition の `reviewDecision: ""` を `--warn-only` で許容して続行する設計が機能）

#### merge
- squash merge + `--delete-branch` で conflicts なし、`closes #575` で Issue 自動クローズ
- reviewDecision が空のまま precondition `--warn-only` を経由した経路は、現状の reconcile-phase-state.sh ロジックでは「approve 必須」を前提にしているが、本リポジトリは PR 設定で approve 必須化されていないため `--warn-only` 経由で続行した。fix cycle 系での false positive（#569 関連）と類似する判定の柔軟性として記録
...
- **2026-08-07 四度目の訪問**: 同日中さらに `/review 1250 --full --non-interactive` の Event-based observation scan からも同一の `observation event=pr-review-full` を検出し、`/verify 575` を dispatch した。結果は全く同一 (fork context → 静的 Task fan-out フォールバック → UNCERTAIN)。同日 4 回目の再現により、下記 Improvement Proposal を本 verify 実行の Step 16 で起票することとした (直前の再訪コメントが明記した「次回 `/verify` の Step 16 での起票判断」を消化)
...
- **[Step 16 で起票]** AC11 (`observation event=pr-review-full config=capabilities.workflow`) は fork context の `/review` 実行では原理的に発火条件と実証条件が乖離する (`when=` 軸に "実行が main/fork のどちらか" を表現する手段がない)。1 日で 4 回の同一結論再現により実証済み。`modules/observation-trigger.md` の `when=` 宣言可能軸への execution-context 軸追加、または AC 条件文自体を「main context での直接実行時に限る」旨に明記する改善を Issue 化する
