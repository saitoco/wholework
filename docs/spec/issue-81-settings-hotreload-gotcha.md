...
#### spec
- Issue 初稿では記録先が `docs/notes.md` 案だったが、Issue 精査フェーズでユーザが `docs/tech.md` + `## Gotchas` セクションを提案し採用。既存ドキュメントへの追記で変更スコープを最小化した判断は妥当。
- 受入条件が `section_contains` を使って `## Gotchas` スコープを限定している点は良設計。他セクションでの偶発マッチを防ぐ具体的な verify command が事前に設計されていた。
- `ssot_for: gotchas` の追記を受入条件に含めたことで、steering document 整合まで検証スコープが広がっており、抜け漏れが少ない。

#### design
- N/A（本 Issue はドキュメント追記のみで設計フェーズなし）

#### code
- 実装コミット: `31c0a55` 1件のみ、docs/tech.md に 11行追加。fixup/amend パターンなし。パッチルート（XS サイズ）として適切なシンプルな実装。
- 3 つの Purpose bullet（事実/含意/偽陰性リスク）をすべて 1 セクション内にカバーしており、受入条件との整合が取れている。

#### review
- N/A（パッチルート：PRなし、直接 main コミット）

#### merge
- パッチルートによる直接コミット。コンフリクトなし、CI不要（ドキュメント変更のみ）。

#### verify
- 全6条件 PASS。`section_contains` による Gotchas セクション限定検証が有効に機能した。
- `probe` キーワードは "in-session probes" に含まれる部分文字列として一致 — verify command の設計として問題なし。
