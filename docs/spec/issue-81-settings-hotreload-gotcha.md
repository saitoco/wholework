...
- 受入条件が `section_contains` を使って `## Gotchas` スコープを限定している点は良設計。他セクションでの偶発マッチを防ぐ具体的な verify hint が事前に設計されていた。
...
- `probe` キーワードは "in-session probes" に含まれる部分文字列として一致 — verify hint の設計として問題なし。
