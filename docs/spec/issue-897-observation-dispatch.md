...
- Post-merge observation AC (「次回 `/auto --batch` 実行で...`/verify` が自動 dispatch されることを観察」) は、2026-07-09 の `/auto 955` (単一Issue、非 --batch) セッションで確認された。`observation-trigger.sh --event auto-run` が #897 を含む17件を検出し、`.wholework.yml` の `autonomy: L3` 設定に従って `Skill(wholework:verify, args="897")` が dispatch され、この `/verify` 実行自体がその証拠となった。
- AC 文言は `--batch` を明示していたが、単一Issue経路 (`/auto` Step 5 の Event-based observation scan) と `--batch` 経路は `skills/auto/SKILL.md` 内で同一の tier-aware dispatch ロジックを共有しているため、非 --batch での実証も AC の意図を十分に満たすと判断し PASS とした。
