...
#### spec
- 受入条件6件が rubric/file_exists/github_check に適切に分類されており、verify 自動化率が高い。rubric 条件の記述が具体的（ファイル名・フィールド名・ステップ番号まで明記）で PASS/FAIL 判定が容易だった。
- Spec retrospective (per-Issue, disposable) と orchestration-recoveries.md (cross-Issue, persistent) の責務分担が Issue 本文・Spec 双方で一致しており、設計の一貫性が高い。

#### design
- Spec の Changed Files リストが実装と完全に一致。スコープ逸脱なし。
- `collect-recovery-candidates.sh` の重複チェックは substring match と明記されており、LLM semantic match との責務境界が明確。

#### code
- `allowed-tools` に `Edit` を追加する必要があった（Spec 未記載）。validate-skill-syntax.py がこれを検出したため CI で問題なく修正されたが、Spec の実装ステップに「allowed-tools 更新」を含める慣習があると見落としが減る。
- bats テスト名に日本語を含めたため parse エラーが発生し rework が1件。CLAUDE.md の "test names must be in English" ルールの適用範囲チェックを実装前に確認する習慣が必要。

#### review
- Spec vs. 実装の乖離なし。`collect-recovery-candidates.sh` の `sys.argv` パターンが SHOULD コメントとして review retrospective に記録されており、類似スクリプト実装時の参照資料として有効。

#### merge
- マージ時に競合・CI 失敗なし。PR #346 の全 CI チェックが pass。

#### verify
- 全6件 PASS（自動検証5件 + github_check 1件）。Post-merge の manual 条件3件は `verify-type: manual` で適切にマークされており、verify スキップ対象として正しく分類された。
- rubric 条件3件が全て PASS：条件テキストにファイル名・セクション名・ステップ番号が含まれていたことが LLM グレーダーの判定精度向上に寄与した。
