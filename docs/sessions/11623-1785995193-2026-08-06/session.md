...
- **`manual-recovery-respawn` が 21 件積み上がっている。** #1014 (`recoveries: manual-recovery-respawn の再発原因を特定・解消`、CLOSED 2026-07-13) の着地後に再発した分。#1152 の実装により初めて可視化された。[No action: 根本原因は上流 `anthropics/claude-code` の external kill (`docs/reports/external-kill-investigation.md`、上流の未対応 Issue 3 件) であり wholework 側の実装では解消できない。#1191 の Section 10 で定常的に可視化されるようになったため、数値の推移で上流修正の効果を測れる]
