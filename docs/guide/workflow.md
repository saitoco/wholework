...
Merges the PR (strategy configurable via `merge-strategy` in `.wholework.yml`, default `squash`) and deletes the remote branch. Before merging, checks that the issue's pre-merge acceptance criteria are all checked off; if any are unchecked, it presents them and blocks the merge unless a recorded override exists.
