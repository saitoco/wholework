...
| `preview-url-command` | string | `""` | Shell command that resolves `PREVIEW_URL` for a project-side script (e.g. a hosting-provider adapter under `.wholework/adapters/`). Requires `capabilities.pr-preview: true`; without it this key is ignored (the resolution call site lives inside `run-review.sh`'s `pr-preview` gate). Supports a `{pr}` placeholder substituted with the PR number. Consulted by both `scripts/run-review.sh`'s preview-wait gate and `/review`'s own Step 8.0 when invoked directly as a skill (via the shared `scripts/resolve-preview-env.sh`, Issue #1428). The command string must not contain a space followed by `#` (stripped as an inline comment by `scripts/get-config-value.sh`). The value is executed verbatim via `bash -c`, so `.wholework.yml` must be treated as trusted on the checked-out branch, the same trust level as any other executable-command key in this file. |
...
Coverage: this key is consulted by both `scripts/run-review.sh`'s preview-wait gate (`/auto`, scheduled runs, direct wrapper invocation) and `/review`'s own Step 8.0 Fast path when invoked directly as a skill.
