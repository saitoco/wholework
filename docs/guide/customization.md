...
| `themes` | block mapping | `{}` | Project-specific `theme/*` label catalog (`{name}: {description}`) consumed by `scripts/setup-labels.sh`. Unset means no `theme/*` labels are created at all — there is no default/fallback catalog (wholework's own dogfooding themes live in this repository's own `.wholework.yml`, not as a built-in default). Color is fixed at `006B75` for all theme labels; not configurable per theme. `/triage` Step 6a's classification logic reads the resulting `theme/*` labels dynamically via `gh label list` and is unaffected by this key directly. |
