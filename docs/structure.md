...
- `scripts/detect-unrecorded-kills.sh` — detect unrecorded external kills and group them into bursts by cross-referencing `.tmp/auto-events.jsonl` `phase_start` duplication (respawn signal — no `wrapper_exit`/`phase_complete`/`manual_intervention` between a consecutive pair) against `docs/reports/orchestration-recoveries.md` entries within a `--window` seconds tolerance (default 120s); called from `/verify` Step 15 (Issue #1387)
