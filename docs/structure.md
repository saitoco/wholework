...
│   ├── reports/         # Optimization and audit reports (includes routines-adoption.md — direction not pursued, retained as a record)
│   ├── stats/           # Project health diagnostic reports (generated by /audit stats; YYYY-MM-DD.md)
│   └── sessions/        # Session-related auto-generated outputs
│         {SID}-{DATE}/  # L3 session retrospectives (generated by /auto Step 5 L3 trigger; notable batch/XL only)
│           session.md       # L3 narrative (What worked / Findings — disposition-tagged) plus a machine-generated `## Metrics` section (via get-auto-session-report.sh --metrics-only)
│           events.jsonl     # session-scoped events extracted from .tmp/auto-events.jsonl (not-notable sessions commit only this file)
...
- `modules/verify-classifier.md` — post-merge condition verifiability classification (the same `manual` tag value is also assigned to pre-merge conditions via a separate `/issue` path — see module)
...
- `scripts/emit-verify-event.sh` — single-command wrapper around `scripts/emit-event.sh` for `/verify`'s non-wrapper emit sites: `emit-verify-event.sh <issue> <event> [--require-session-id|--unconditional] [key=value ...]` or `emit-verify-event.sh --persist-session <sid-or-empty> <issue>`; internally sources `emit-event.sh` and calls `restore_auto_session_pointer()`/`persist_auto_session_pointer()`, avoiding the `source`-based compound command the worktree isolation guard rejects (#1458); used exclusively by `skills/verify/SKILL.md`
...
- `scripts/resolve-merge-strategy.sh` — resolve `.wholework.yml`'s `merge-strategy` key to a bare strategy name or a `gh pr merge` flag (`--flag`); fail-safe fallback to `squash` with a stderr warning for invalid/special-character values (truncated to 40 printable characters before being echoed back, since the caller is an LLM agent), a silent fallback to `squash` for a genuinely empty value (absorbed by `get-config-value.sh`'s own default substitution, indistinguishable from unset), and a fail-closed fallback to `squash` with a distinct dependency-failure warning if `get-config-value.sh` itself fails; called by `skills/merge/SKILL.md` Step 2
...
- `scripts/reclaim-stale-worktrees.sh` — inventory and reclaim stale worktrees and orphan branches for Issues/PRs that have already completed (CLOSED/MERGED); dry-run by default, `--apply` to perform local deletion; concurrent-session guard (locked + HEAD matches main), uncommitted-changes guard, and safe branch deletion for branches whose merge did not preserve ancestry — `squash` and `rebase` (`git branch -d` fallback to `-D` when the branch tip matches a MERGED PR's `headRefOid` — for a `kind=pr` branch that PR itself, for a `kind=issue` branch one resolved by searching `closes #<N>` and verifying the match, since `/code` pr route also creates `<phase>+issue-N` branches squash-merged via their own PR). Also reclaims orphan `worktree-*` branches on `origin`: always enumerated (dry-run report), `--apply-remote` (independent from `--apply`) to actually run `git push origin --delete`; safety guards equivalent to the local ones — a branch with a live local checkout is excluded (left to the local reclaim path), a `kind=pr` branch must match the MERGED PR's `headRefOid` exactly, and a `kind=issue` branch uses the same closes-PR `headRefOid` match when found, otherwise falls back to an ancestor-of-`origin/<default-branch>` check
...
- `scripts/detect-unrecorded-kills.sh` — detect unrecorded external kills and group them into bursts by cross-referencing `.tmp/auto-events.jsonl` `phase_start` duplication (respawn signal — no `wrapper_exit`/`phase_complete`/`manual_intervention` between a consecutive pair) against `docs/reports/orchestration-recoveries.md` entries within a `--window` seconds tolerance (default 300s); called from `/verify` Step 15 (Issue #1387)
