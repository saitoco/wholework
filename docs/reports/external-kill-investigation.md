...
> **Note (#1214)**: the `next_cycle_seeded` event referenced above was removed in #1214. For batch-session identification going forward, use the `"mode": "batch"` field in `.tmp/auto-session-<AUTO_SESSION_ID>.json` instead. This field is only available while the session's `.tmp/` artifacts persist (`.tmp/` is gitignored and not archived) — unlike the removed event, it is not retained in `docs/sessions/**` for post-hoc investigation once the session's temp files are gone.
