...
**Sonnet 5**: Sonnet 5 (`claude-sonnet-5`, launched 2026-06-30) is a substantial agentic-performance upgrade over the current default parent (Sonnet 4.6), approaching Opus 4.8 at `effort: xhigh` on a meaningful share of tasks while priced at roughly 40–60% of Opus 4.8 ($2/$10 per MTok introductory through 2026-08-31, $3/$15 standard thereafter — the same band Sonnet 4.6 already occupies in Wholework's cost model). It ships a tokenizer update in the same family as the Opus 4.7 change (1.0×–1.35× more tokens for the same input), which is unmeasured for Wholework's actual prompt mix and has direct implications for `claude-watchdog.sh` timeout calibration and context-budget heuristics. This entry is a **note only** — the matrix table above still lists Sonnet 4.6 as the default parent; the actual default-parent swap is sequenced behind two blocking measurements (`#877` `/verify` interactive-friction re-measurement, `#878` tokenizer/watchdog impact measurement) and is tracked as a separate, CI-sensitive Issue. See `docs/reports/claude-sonnet-5-impact-strategy.md` for the full impact analysis, decision matrix (§4.1), and candidate Issue execution plan (§8).
