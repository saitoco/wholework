...
- **fork context vs main context**: Context isolation level is set per skill. Fork justification: "independence/safety" (since 1M context GA, cost/capacity motivation has largely diminished). Fork decision per skill (exhaustive):
  ...
  For context determination criteria and per-context constraints, see [`modules/execution-context.md`](../modules/execution-context.md).
...
- **`/auto` skill**: Orchestrator that chains spec→code→review→merge→verify sequentially via `run-*.sh`. Each phase runs as an independent `claude -p` process with configurable permission mode (`--permission-mode auto` by default; `--dangerously-skip-permissions` when `permission-mode: bypass` is set in `.wholework.yml`), guaranteeing fresh context isolation. `verify-max-iterations` (default: 3, max: 20, configurable via `.wholework.yml`) caps the verify-reopen loop; when the counter reaches the limit, the Issue stays in `phase/verify` and awaits human judgment. The `/auto` skill detects `MAX_ITERATIONS_REACHED` in verify output and stops the chain instead of looping indefinitely. For flag behavior, batch processing, resume, and release branch workflow, see [docs/workflow.md § Orchestration](workflow.md#orchestration).
  - **Two-tier orchestration**: `/auto` itself (parent orchestrator) runs in the user's Claude Code session and makes adaptive decisions using LLM reasoning (label state evaluation, Size-based routing, sub-issue dependency analysis). For XL Issues, `run-auto-sub.sh` (child orchestrator) runs each sub-issue's full phase sequence. `run-auto-sub.sh` is bash-orchestrated with a tiered adaptive recovery: (1) `reconcile-phase-state.sh` completion check, (2) `apply-fallback.sh` known-pattern recovery, (3) `spawn-recovery-subagent.sh` (`claude -p` to `agents/orchestration-recovery`) on unknown anomaly. Normal path stays bash to minimize cost and maximize parallel stability; `claude -p` is invoked only for diagnosis when tiers 1–2 fail, with a `WHOLEWORK_MAX_RECOVERY_SUBAGENTS` cap to bound parallel cost. The cap defaults to 1 (serial recovery) and can be raised by setting the env var explicitly.
    - **Parent-session manual respawn (outside the Tier 1/2/3 machinery)**: when a background `run-*.sh` wrapper's own process group is killed externally (see `modules/orchestration-fallbacks.md#external-kill-parent-respawn`), none of the three tiers above can fire — Tier 3's Layer B retry runs inside the same process group being killed. Only the parent `/auto` session can observe the kill and respawn the phase; it records the recovery via `run-auto-sub.sh --write-manual-recovery` (see `#manual-recovery-spec-write`), which writes to the sub-issue Spec (only if a formal Spec already exists — no stub is created otherwise), `docs/reports/orchestration-recoveries.md`, and a `manual_intervention` event — not the Tier 1/2/3 vocabulary or metrics.
- **Phase state reconciliation**: `scripts/reconcile-phase-state.sh` is a general-purpose phase state reconciler that verifies live GitHub/git state against phase-specific expected signatures — preconditions and success signatures — defined in `modules/phase-state.md` (SSoT, `ssot_for: phase-signatures, reconcile-json-schema`). Callers (`/auto` watchdog recovery, precondition checks before phase execution) consume the JSON v1 output schema fixed in that module, so they do not couple to the reconciler's internal implementation. This replaces the earlier issue-specific `watchdog-reconcile.sh` with a shared, per-phase-extensible mechanism. The reconciler implements the **Observe-Diagnose-Act pattern** applied to each `/auto` phase: (1) **Observe** — read live GitHub/git state before phase execution (precondition check); (2) **Diagnose** — compare the observed state against expected signatures and decide to proceed, skip, or trigger recovery; (3) **Act** — execute the phase, then re-check the success signature as a completion check. Introduced in #314 as the orchestration reliability foundation for downstream issues #315–#319 (per-phase state detection for verify, merge, review, and code phases): centralizing Observe-Diagnose-Act in one reconciler makes each phase independently verifiable and recoverable without ad-hoc per-phase state checks.
- **`/doc` skill**: Project foundation document management. Manages Steering Documents (`product.md`, `tech.md`, `structure.md`) and Project Documents. Key operations: `sync` (bidirectional normalization and drift detection; per-document variants: `sync product` / `sync tech` / `sync structure` for selective reverse-generation; `--deep` extends with codebase analysis + .md integration scan + Narrative Semantic Drift Check + Terms consistency check), `init` (initial setup wizard), `add` / `project` (document registration), `translate {lang}` (multi-language translation generation). Complements `/audit`: `/doc sync` proposes documentation-side fixes; `/audit drift` generates Issues for code-side fixes.
- **`/triage` skill**: Initial Issue assessment that assigns Type/Priority/Size/Value before the main workflow starts. Runs Title normalization (`modules/title-normalizer.md`) and 4-perspective deep analysis (`/triage --backlog`). `/auto` chains `/triage` automatically when an Issue lacks a `phase/*` label.
- **`/merge` skill**: Squash-merges a PR and deletes the remote branch after CI passes and review approval. Decision rule: only merge when AC verification is GREEN (or explicitly approved). Reads Phase Handoff from `/review` and writes Phase Handoff for `/verify`. Mechanical operation; `model: sonnet` + `low` effort suffice.
- **`/code` skill**: Local implementation phase that reads the Spec from `docs/spec/issue-N-*.md` and executes the implementation steps. Routes to patch (direct commit to main for XS/S) or PR (branch + PR for M/L) based on Issue size. Reads existing AC verify commands to inform implementation; emits Phase Handoff for downstream `/review` / `/merge` / `/verify`.
- **operate route (diff-less workflow path)**: A third `/code` route, orthogonal to Size, for Issues whose Spec describes only external-tool operations (CMS edits, infrastructure operations) with no repository file changes — detected mechanically from the Spec's `## Changed Files`/`## Implementation Steps`, with no new label or Issue metadata added. Implemented as a third value on `/code`'s existing patch/pr branch point rather than a new Skill: the #437 lesson favors extending an existing pattern over introducing a new mechanism, and a standalone Skill would have touched 6–8 additional SSoT documents. Results are recorded as an `## Execution Log` Issue comment in place of a git diff; `phase/code` → `phase/verify` label transition mirrors patch route, skipping `/review`/`/merge` since no PR exists. Since operate route never produces a `closes #N` commit, `code-patch`'s completion signature (`scripts/reconcile-phase-state.sh`) also accepts the Execution Log/Execution Plan marker comment as an alternate success signature — see `modules/phase-state.md` § "Operate Route Completion Signature". See `modules/size-workflow-table.md` § "Diff-less Axis (operate route)".
...
- **Cross-phase memory mechanisms**: Two complementary mechanisms carry context across phases that run in fresh fork contexts.
  - **Spec retrospectives**: Each phase appends a Retrospective section to the Spec recording observations, decisions, and uncertainty resolutions. Surfaces accumulate within the disposable Spec and feed downstream phases through the same file.
  - **Phase Handoff** (`modules/phase-handoff.md`): A structured summary written by the producing phase and read by the consuming phase (e.g., review → merge → verify). Decouples short-lived hand-off signals (acceptance condition results, scope notes, residual risk) from the Spec body so that fork-context phases can pick up only what they need without re-reading the full Spec history. Spec retrospectives store *history*; Phase Handoff carries *the next step's working context*.
- **Progressive disclosure (Core/Domain separation)**: SKILL.md body contains only generic logic independent of project type or tool. Logic specific to particular tools (Figma, Copilot, etc.) or project types (skill development, IaC, etc.) is extracted to auxiliary files (`skills/{name}/xxx-phase.md`), read only when applicable. Decision criterion: "Is this logic needed in projects that don't use this tool/project type?" — If No, extract it. **This criterion applies to guidance content (application scenarios, decision criteria, comparison tables) as well as execution logic** — placing capability-specific guidance in eager-loaded shared modules (e.g., `modules/verify-patterns.md`) causes domain-external projects to incur the full token cost on every skill invocation. Use a Domain file with `load_when: capability: {name}` gate instead (reference: Issue #441 visual-diff capability with `skills/spec/visual-diff-guidance.md`). For the exhaustive list of extraction patterns (Marker-detection / File-existence / MCP-availability / Depth-routing / Capability-flag / Directory-scan), see [docs/environment-adaptation.md § Extraction Patterns](environment-adaptation.md#extraction-patterns-exhaustive) (SSoT: `environment-adaptation-architecture`).

- **Autonomy tier (L0 write governance)**: Wholework operates across four layers — L0 (GitHub state: Issues, Labels, PRs, blockedBy, `closes #N`), L1 (Claude Code primitives: `/loop`, `/goal`, `ScheduleWakeup`, `CronCreate`), L2 (Wholework skill internals: Spec, retro, `auto-events.jsonl`), and L3 (OS scheduler). The `autonomy:` field in `.wholework.yml` declares how far skills may write L0 and fire L2→L1 paths (A Advisory / B CronCreate / C ScheduleWakeup / E Seed file emission). The SSoT for the tier × path matrix and Tier × L0 write matrix is [`modules/autonomy-tier.md`](../modules/autonomy-tier.md); per-surface L0 write classification is in [`modules/l0-surfaces.md`](../modules/l0-surfaces.md). This layer is orthogonal to `permission-mode` (which governs Claude Code subprocess permissions, not Wholework's GitHub state scope). Skill frontmatter `loop-paths-used` declares which L2→L1 paths the skill uses (e.g., `loop-paths-used: [A]` = advisory print only); implemented in #700 (`/verify` auto-retry-on-fail tail extension). Remaining gating enforcement (#702, #703) is tracked in follow-up Issues.
...
| orchestration-recovery | auto (recovery) | Sonnet | — | Tier 3 recovery diagnostician spawned by `spawn-recovery-subagent.sh` when bash tiers 1–2 fail. Analyzes phase state and produces minimal recovery plan in JSON |
| frontend-visual-review | verify (visual-diff) | Opus | — | Visual gap enumeration from 3-panel comparison images. Spawned by `modules/visual-diff-adapter.md` for `visual_diff` verify commands |
...
**Opus 4.8 effort calibration** (historical — this note's exact heading text is retained because `#922` cites it by name as its judgment basis; see the **Opus 5 effort calibration** note below for the guidance that actually applies to the `opus` alias today): Opus 4.8 enforces strict effort calibration — `low` and `medium` aggressively scope to literal task requirements. `max` carries a diminishing returns risk (overthinking) with Opus 4.8; reserve it for intelligence-demanding experimental tasks only. `xhigh` is the Opus 4.8 recommended default for most coding and agentic use cases.

**Opus 5 effort calibration**: Opus 5 (`claude-opus-5`, launched 2026-07-24) revises the Opus 4.8 guidance above — for coding/agentic tasks, start at `xhigh`; for other tasks, start at `high`; then sweep downward from there, since `low`/`medium` are reported to be stronger than their tier names suggest. Reserve `max` for extremely difficult, latency-insensitive tasks only. Sub-agent `model: opus` / `model: sonnet` alias values in agent frontmatter auto-resolve to the current Opus (5). See `docs/reports/claude-opus-5-impact-strategy.md` for the full impact analysis (`#1062`).

**Fable 5 (Mythos class)**: Fable 5 (`claude-fable-5`) is a tier above Opus and is **not** reachable via the `opus` alias — the explicit model string `claude-fable-5` is required. Adoption is **opt-in only** (never a default model swap) due to hard constraints: cost $10/$50 per MTok (2× Opus 4.8, ~3.3× Sonnet 5 at standard pricing), 30-day retention required (zero-data-retention orgs not supported), and usage-credit gated on subscription plans. Skills that expose a Fable 5 opt-in (e.g., `/spec --fable`) document it per-skill. See `docs/reports/claude-fable-5-impact-strategy.md` §3.3 and §5.2 for adoption guidance. When running on Fable 5, security-related queries in review phases may be automatically routed to Opus 4.8 via the cyber classifier (transparent via CLI) — do not assume Fable 5 handles security analysis directly. Fable 5's biology-classifier fallback target changed with the Opus 5 launch: requests blocked by Fable 5's biology classifier now route to **Opus 5** rather than Opus 4.8 (the cyber-classifier fallback target is unchanged — still Opus 4.8).

**Sonnet 5**: Sonnet 5 (`claude-sonnet-5`, launched 2026-06-30) is a substantial agentic-performance upgrade over the former default parent (Sonnet 4.6), approaching Opus 4.8 at `effort: xhigh` on a meaningful share of tasks while priced at roughly 40–60% of Opus 4.8 ($2/$10 per MTok introductory through 2026-08-31, $3/$15 standard thereafter — the same band Sonnet 4.6 already occupies in Wholework's cost model). It ships a tokenizer update in the same family as the Opus 4.7 change (1.0×–1.35× more tokens for the same input), with direct implications for `claude-watchdog.sh` timeout calibration and context-budget heuristics. Both blocking measurements have now landed: `#877` (`/verify` interactive-friction re-measurement) resolved **NO-GO** (no redesign required), and `#878` (tokenizer/watchdog impact measurement) resolved **significant**, addressed by `#903`'s recalibration (`WATCHDOG_TIMEOUT_CODE_DEFAULT` 3600→4680, `WATCHDOG_TIMEOUT_REVIEW_DEFAULT` 2000→2600). With both blockers landed, the default-parent swap to Sonnet 5 is **confirmed and final** — this is no longer a note-only entry; the matrix table above now lists Sonnet 5 as the default parent (see the sentence preceding the table). See `docs/reports/claude-sonnet-5-impact-strategy.md` for the full impact analysis, decision matrix (§4.1), and candidate Issue execution plan (§8).

**Alias pin policy**: Wholework continues to use the bare `sonnet` CLI alias (and `ANTHROPIC_MODEL=sonnet`) rather than pinning explicitly to `claude-sonnet-5`. Rationale: (1) reactive recalibration is an already-proven operating pattern — both the Fable 5 → Sonnet 4.6 transition (`#628`) and this Sonnet 5 transition itself (`#877`/`#878`/`#903`) were handled successfully via post-swap watchdog/effort follow-up rather than a pre-swap gate; (2) an explicit pin would require coordinated edits across the 5 `run-*.sh` scripts and roughly 10 skill/sub-agent frontmatter files carrying `model: sonnet`, for limited safety benefit, since Anthropic only repoints the bare alias at deliberate, major model launches; (3) trade-off: future model generations will likewise be auto-adopted as default parent via the alias before a dedicated measurement Issue lands (as happened here with Sonnet 5) — Wholework accepts this risk given the reactive-recalibration SOP's track record (`#628`, `#903`). The same trade-off materialized a third time for the `opus` alias with Opus 5's 2026-07-24 launch — see the note directly below.

**Opus 5 default parent evaluation — deferred (`#1062`)**: The Sonnet 5 → Opus 5 default-parent swap was evaluated and explicitly deferred, not adopted. Reasons: (a) the Sonnet 5 default-parent confirmation (`#914`) landed roughly four weeks before this evaluation — too recent to re-open without new evidence; (b) `#921`/`#922`/`#923`, Wholework's three Sonnet-5-era effort recalibrations (code/review, spec, issue respectively), all independently concluded **maintain** with no design-reasoning gaps found in production samples — no quality signal motivates a more expensive default; (c) Opus 5 costs 1.7× (standard pricing) to 2.5× (against Sonnet 5's introductory pricing) more than Sonnet 5, and no measurement chain comparable to `#877`/`#878` (which gated the Sonnet 4.6 → Sonnet 5 swap) has been run to justify that premium. **Re-evaluation trigger**: revisit this deferral if either (i) a future Sonnet 5 effort or capability re-evaluation surfaces a genuine quality gap, or (ii) Claude Max's own default model now being Opus 5 is judged to carry independent weight for Wholework's default-parent choice — as of this note, neither condition holds. See `docs/reports/claude-opus-5-impact-strategy.md` §4.1 for the full decision matrix.

**Opus 5 watch items**: Two Opus 5 changes are recorded for monitoring, not acted on: (1) prompt caching now activates from a **512**-token minimum prompt, half of Opus 4.8's 1024-token floor — favorable for any Opus-routed prompt in the 512–1024 token range, though Wholework does not currently track per-component prompt sizes against this threshold; (2) Opus 5's rate limit sits in a pool **separate from the unified Opus 4.x pool**, which changes the parallel-capacity picture for `/auto --batch` runs that fan out multiple L/XL Issues' Opus sub-agents (`issue-scope`/`issue-risk`/`issue-precedent`, `review-bug`/`review-spec`) concurrently. Neither item has a concrete incident behind it yet; see `docs/reports/claude-opus-5-impact-strategy.md` §3.3/§3.4.
...
- **Watchdog timeout calibration**: Phase-specific timeout constants in `scripts/watchdog-defaults.sh` are calibrated against the dominant parent orchestrator model's per-token latency. Recalibrate when the default parent model changes (e.g., Fable 5 → Sonnet 4.6 transition in #628 required raising `WATCHDOG_TIMEOUT_ISSUE_DEFAULT` from 600 to 1200). **#903 recalibration (Sonnet 5)**: following #878's measured ~1.3-1.4× tokenizer ratio for Sonnet 5, real `/code`/`/review` production wall-clock samples (n=10 / n=9, `docs/reports/sonnet-5-watchdog-recalibration.md`) showed p95/max usage already at or above the 80%-of-timeout recalibration threshold for both phases (code: p95 81.3%, max 93.8% of 3600s; review: p95 92.2%, max 100.2% of 2000s — one real sample already at the limit). Raised `WATCHDOG_TIMEOUT_CODE_DEFAULT` 3600→**4680** and `WATCHDOG_TIMEOUT_REVIEW_DEFAULT` 2000→**2600** (both ×1.3, the conservative end of #878's measured range, deliberately less aggressive than #628's 2× precedent per Icebox #596's timeout-inflation-vs-stuck-detection tradeoff). `WATCHDOG_TIMEOUT_SPEC_DEFAULT` / `_ISSUE_DEFAULT` / `_MERGE_DEFAULT` were out of scope for #903 (see report for rationale). **#939 (`WATCHDOG_TIMEOUT_SPEC_DEFAULT` re-check, Fable 5)**: Fable 5 redeployed 2026-07-01 after its 2026-06-13→2026-07-01 pause, making renewed `run-spec.sh --fable` measurement possible, but only one real spec-phase `--fable` execution exists on record (Issue #560, untelemetered) and this Issue's `/code` pass deliberately did not spawn new `--fable` spec runs against other backlog Issues to gather a fresh sample — doing so non-interactively would spend Fable 5's premium per-token cost and mutate unrelated Issues' state without real-time user oversight (see `docs/reports/watchdog-recovery-strategy.md` § 2026-07 re-measurement for the full rationale). **Verdict: maintain `WATCHDOG_TIMEOUT_SPEC_DEFAULT=1800`** — with no new evidence gathered, there is nothing to weigh against the existing #556 extrapolation (600–2000s, itself never confirmed against real spec-phase traffic), so the #903 80%-of-timeout recalibration threshold cannot be honestly evaluated yet. This defers, rather than closes, the recalibration question; a follow-up pass with explicit user authorization to run real `--fable` spec executions is needed before a data-backed raise/maintain decision can be made. No changes made to `tests/watchdog-defaults.bats` (value unchanged). The same #903 report also records a **prompt slimming** assessment for two candidates raised in #878/#903: `/auto`'s L3 auto-retrospective "notable judgment" step (injected full session `events.jsonl` for a purely mechanical decision — addressed in #913 by replacing the raw dump with a `jq -sc` event-count aggregation) and the L/XL parallel investigation sub-agent inputs in `/issue`/`/review` (full diff/file content — no slimming recommended; truncation risk outweighs token savings for scope/risk/bug-detection accuracy).
- **code-side auto-retry (silent no-op)**: When `auto-retry-on-fail.enabled: true` and `autonomy: L2/L3`, `run-code.sh` internally retries after detecting `matches_expected: false` (silent no-op) from `reconcile-phase-state.sh`. Max retries from `auto-retry-on-fail.max_iterations` (also accepts legacy `threshold` key; default: 3). Retry counter (`CODE_RETRY_COUNT`) is passed via exported env var across `exec`-based re-invocations. Symmetric with verify-side auto-retry in `skills/verify/SKILL.md` Step 11(b) (same tier gate: L2/L3 + `AUTO_RETRY_ENABLED=true` + count < max), except that the verify-side gate is preceded by a documented-deferral escape hatch (#947): when a FAIL is detected as an intentional, already-documented deferral (via a `deferral=true` marker attribute or the Spec's retrospective/Phase Handoff sections), the tier gate is skipped unconditionally regardless of tier/config/count. When built-in retry is active, `apply-fallback.sh`'s `code-patch-silent-no-op` Tier 2 handler is suppressed to prevent double-retry. Immediately before the `exec`-based retry re-invocation, a preflight step stashes (`git stash push --include-untracked`, excluding `docs/sessions/**`) any parent-main untracked files left behind by the silent no-op itself, so that `check-verify-dirty.sh`'s re-check on the retry pass is not blocked by the prior attempt's own stray output (#886).
...
- **Sonnet 5 effort recalibration — spec (#922, C3)**: re-evaluated whether `run-spec.sh`'s Sonnet-path default effort could drop from `max` to `xhigh` under Sonnet 5's widened effort curve (impact strategy report §3.3/§4.2), including a comparison against the existing Opus fallback (`--opus`, L-size only, `xhigh` default since #217). **Verdict: maintain `max`** for the Sonnet default path (full analysis: `docs/reports/sonnet-5-effort-recalibration-spec.md`). Unlike #921 (C2), this Sonnet-path default has never been evaluated before (#217 explicitly scoped the Sonnet path out when introducing Opus `xhigh`; #229 explicitly excluded `run-spec.sh` from its Sonnet-ladder recalibration). `/spec` is structurally analogous to `run-code.sh` — a single, unbroken 19-step reasoning chain with no sub-agent fan-out — but with strictly higher downstream blast radius, since spec errors propagate through code, review, *and* merge (one more phase than a code-phase error). The only diminishing-returns guidance documented in this table (the "Opus 4.8 effort calibration" note above) is scoped explicitly to Opus 4.8, not Sonnet 5 — the Sonnet-5-specific claims in impact report §3.3 (`medium` more competitive, `xhigh` approaches Opus) do not themselves assert that Sonnet 5's own `max` is over-provisioned relative to `xhigh`. A production-sample check across Issues coded after #914 (Sonnet 5 as default parent) found no design-reasoning-driven gaps in Code Retrospectives (all recorded gaps were environment/runtime specifics, e.g. a security-classifier interaction in #917, a macOS symlink path mismatch in #930) — net-neutral evidence that neither confirms nor rules out a downgrade. The Opus fallback's `xhigh` default is not treated as transferable evidence, since it reflects Opus 4.8's own (distinct) effort-calibration guidance for a narrower, L-size-only scope. No changes made to `run-spec.sh`, this matrix table, or `tests/run-spec.bats`.
...
| `WHOLEWORK_CI_MIN_CHECKS_WAIT_SEC` | `120` | Grace period in seconds during which `wait-ci-checks.sh` keeps polling when `gh pr checks` reports zero registered checks. After the grace period elapses with still zero checks, the script emits an explicit warning and stops waiting. Set to `0` in tests to skip the grace period. |
...
| `WHOLEWORK_PREVIEW_TIMEOUT_SEC` | `600` | Maximum wait time in seconds for `scripts/run-review.sh`'s PR preview deployment poll (`capabilities.pr-preview: true` projects only), which checks the GitHub Deployments API for the PR branch's latest deployment `state`. If not confirmed `success` within the timeout, `run-review.sh` exits `PENDING` (exit code 2) without launching the review session. |
...
| `HAS_PR_PREVIEW_CAPABILITY` | `capabilities.pr-preview: true` | `true` when the project's PRs produce a preview URL. Gates pre-merge-preview AC classification in `/issue` Step 4: ACs that can only be confirmed against the preview environment are placed in the pre-merge section with an `ac-tier: preview` tag, regardless of whether they have a verify command (auto subcase — additionally gets a `--when="test -n \"$PREVIEW_URL\""` guard) or not (manual subcase — additionally tagged `verify-type: manual`, no guard). Both subcases are executed/presented at `/review` time and skipped in `/verify` post-merge unless `/review`'s latest `type=preview-ac-unverified` marker lists the AC as unverified (see `modules/l0-surfaces.md` § "Machine-Readable Event Marker"), in which case `/verify` falls back to a production-URL check for the auto subcase (the manual subcase has no verify command to fall back with, so it is presented as a human-check item instead). Also gates `/code`'s pr route Step 13 (Preview Build Verification): when `true`, `/code` waits for the deploy/preview build to complete after PR creation and, on failure, pushes fix commits from within the code phase (bounded to 3 attempts) before falling through to `/review`. Projects that do not declare this capability keep `/code`'s prior behavior — the code phase completes at PR creation with no build wait. Also gates `scripts/run-review.sh`'s pre-session preview deployment wait: when `true`, `run-review.sh` polls the GitHub Deployments API for the PR branch's latest deployment state (bounded by `WHOLEWORK_PREVIEW_TIMEOUT_SEC`) before launching the review session, exiting `PENDING` (exit code 2) instead of starting a review session against an unconfirmed preview. |
