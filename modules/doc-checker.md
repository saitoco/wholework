...
- **Documents to check**: List of document file paths to verify (defaults to the following when omitted):
  - **Steering documents**: Get `*.md` with Glob (path: `$STEERING_DOCS_PATH`), and dynamically identify files with frontmatter `type: steering` (default: `docs/*.md`; `$STEERING_DOCS_PATH` is provided by the calling skill)
  - **Project documents**: Get `*.md` with Glob (path: `$STEERING_DOCS_PATH`), and dynamically identify files with frontmatter `type: project`
...
2. Dynamically identify candidate document files using Glob:
   - Get `*.md` with Glob (path: `$STEERING_DOCS_PATH`) to create a Steering / Project document candidate list (do not Read at this stage)
