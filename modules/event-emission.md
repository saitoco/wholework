...
Every `AUTO_EVENTS_LOG`-gated emit site in `skills/verify/SKILL.md` calls `scripts/emit-verify-event.sh $NUMBER <event> [key=value ...]` (#1458) — internally sources `emit-event.sh`, calls `restore_auto_session_pointer`, and applies the guard before calling `emit_event`. `restore_auto_session_pointer` resolves `AUTO_SESSION_ID`/`AUTO_EVENTS_LOG` in this order (exhaustive):
