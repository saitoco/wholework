...
| `html_check "URL" "selector" "--exists"` / `html_check "URL" "selector" "--count=N"` / (with `--allow-localhost`) | **Mode-dependent**: In safe mode, run URL security check from `${CLAUDE_PLUGIN_ROOT}/modules/browser-verify-security.md`. Safe mode blocks private IPs; external URLs executed with curl + pup. `full` → no restrictions; first check `which pup` (UNCERTAIN if not installed). If pup exists, run `curl -s --connect-timeout 5 --max-time 10 "URL" \| pup "selector"`; for `--exists` confirm output is not empty; for `--count=N` confirm `pup "selector" --number` output matches N. **`--allow-localhost` flag**: when present, `127.0.0.0/8` (localhost) is allowed even in safe mode; other private IP ranges remain blocked. | `always_ask` |
