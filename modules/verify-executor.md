...
- **`html_check`**: Specialized for HTML structure verification. curl + `scripts/html-selector-match.py`, a standard-library-only Python CSS selector matcher (no external binary dependency, so no UNCERTAIN branch from a missing tool). Can run in safe mode with URL security check. UNCERTAIN only when the selector syntax cannot be parsed (the script exits non-zero). `--allow-localhost` flag opts in to localhost access even in safe mode
