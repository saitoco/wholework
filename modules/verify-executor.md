...
| `grep "pattern" "path"` | Regex match using Grep. **PASS when match is found**. To assert absence (no match), use `file_not_contains` instead. **Case-sensitive by default**: match the exact case of the implementation text. When the case is uncertain, use bracket notation (e.g., `[Nn]o heading matched`) |
