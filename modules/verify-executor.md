...
| `section_not_contains "path" "heading" "text"` | Read file and confirm fixed string "text" is NOT present within the specified markdown heading section. Uses the same heading **partial match** rule as `section_contains`. If no heading matches, return UNCERTAIN with the same diagnostic message as `section_contains`: `No heading matched "{heading}". Candidate headings: "{H1}", "{H2}", ...` (up to 3 candidates). |
