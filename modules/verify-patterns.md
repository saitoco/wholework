...
<!-- verify: github_check "gh run list --workflow=dco.yml --branch=main --limit=1 --json conclusion --jq '.[0].conclusion'" "success" --> CI run succeeded
```
...
```
<!-- verify: github_check "gh run list --workflow=<specific>.yml --branch=main --limit=1 --json conclusion --jq '.[0].conclusion'" "success" -->
```
...
```
<!-- verify: github_check "gh run view $(gh run list --workflow=ci.yml --branch=main --limit=1 --json databaseId --jq '.[0].databaseId') --json jobs --jq '.jobs[] | select(.name==\"Run bats tests\").conclusion'" "success" -->
```
