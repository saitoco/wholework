...
- `run-spec.sh` detects the same silent-no-op signature (`reconcile-phase-state.sh spec --check-completion` returning `matches_expected:false`) and, under the same tier/config gate, emits `spec_retry_fire` and self-restarts via the identical `exec bash "$0" "$ISSUE_NUMBER" "${_TRAILING_ARGS[@]}"` pattern (Issue #1369)

### Applicable Phases
- code phase (patch and pr routes alike) and spec phase
- Applies regardless of the calling path — `/auto` single-Issue direct dispatch, `run-auto-sub.sh`-mediated XL sub-issue / Batch Mode execution, or a standalone `run-code.sh`/`run-spec.sh` invocation — since the wrapper itself is the recording subject in all of them
- This scope is a deliberate, evidence-gated decision confirmed by Issue #1329 (code phase) and extended to the spec phase by Issue #1369 (details below in Phase Scope Decision), not an unaddressed gap

### Phase Scope Decision

Whether this mechanism should extend beyond the code phase was evaluated in Issue #1329, prompted by a spec-phase silent no-op (#1130) that had no wrapper-level retry and fell through to Tier 3 recovery — costlier than the code-phase equivalent (#1102), which this entry's own mechanism resolved in ~25 minutes. The code-only scope was confirmed as deliberate, evidence-gated policy, not an unaddressed gap, decided per phase:

- **spec**: structurally the same shape as `code-patch` — single wrapper, local `reconcile-phase-state.sh <phase> --check-completion` signature (`modules/phase-state.md` Phase Table), no partial external side effect to double-apply on retry. `scripts/run-spec.sh` already called this check and set `EXIT_CODE=1` on `matches_expected:false` (mirroring `run-code.sh`'s own detection); only the retry-vs-fail branch itself was missing. Issue #1329 recommended this as a follow-up on the strength of the #1130 incident, and Issue #1369 implemented it — `run-spec.sh` now carries the same tier-gated `exec` self-restart, record-before-exec recovery logging, and stash preflight as `run-code.sh`.
...
- **merge**: not safely portable as-is. `gh pr merge "$NUMBER" <strategy-flag> --delete-branch` (the flag is resolved by `scripts/resolve-merge-strategy.sh --flag`, see `skills/merge/SKILL.md` Step 2) is an irreversible external side effect; a local-completion-only gate (the pattern `code`/`spec` use) risks a second merge attempt if that check is stale or racy. A safe design would need the same live `gh pr view --json state` check `detect-wrapper-anomaly.sh` already uses for merge-phase anomaly detection (see `#code-patch-silent-no-op`'s Exception Condition above), not the simpler local gate — a materially different, not-yet-designed mechanism. Deferred until a real merge-phase silent-no-op incident justifies the added design and implementation cost.

Tier 2 (`apply-fallback.sh`)'s `code-patch-silent-no-op` handler is still code-phase-specific in its retry action (`apply_code_patch_silent_no_op_retry()` hardcodes `run-code.sh --patch`) even though `detect_symptom_anchor()`'s underlying symptom detection is phase-agnostic — so a spec-phase silent no-op still relies on this entry's own built-in `run-spec.sh` retry rather than Tier 2, and any other non-code/spec-phase silent no-op still has no automatic recovery below Tier 3, matching the #1130 Background outside the two phases this mechanism now covers.

### Fallback Steps
1. Immediately before the `exec` self-restart, the wrapper itself calls `_write_code_retry_recovery(issue, iteration)` (`run-code.sh`) or `_write_spec_retry_recovery(issue, iteration)` (`run-spec.sh`), which appends a `code-retry-fire`/`spec-retry-fire` entry to `docs/reports/orchestration-recoveries.md`, commits it (`git commit -s`), and pushes with fetch+rebase retry (same pattern as `_push_with_retry()` below)
...
3. If any of the record/commit/push steps fail, the wrapper logs a warning to stderr and proceeds to `exec` regardless — this is a best-effort log, not a gate on the retry

### Escalation
- code phase: once `auto-retry-on-fail` iterations are exhausted, the existing `code-patch-silent-no-op` pattern (Tier 2/3, above) already checks whether `auto-retry-on-fail` has been exhausted and takes over from there
- spec phase: no Tier 2 handler exists yet (see Phase Scope Decision above) — once `run-spec.sh`'s own iterations are exhausted, the phase exits 1 and falls through to Tier 3 recovery, same as before #1369 for a spec-phase failure that persists past the built-in retry

### Rationale
- Resolves an asymmetry with `wrapper-retry-on-kill` (above): that pattern's `wrapper-retry-on-kill` recovery is already recorded to `docs/reports/orchestration-recoveries.md` by `run-auto-sub.sh`, but `code_retry_fire`/`spec_retry_fire` previously only reached `.tmp/auto-events.jsonl` (gitignored, ephemeral) — invisible to `/audit recoveries` frequency detection and to any Spec that might reference it
- Recording must happen before `exec`, not in `run_phase_with_recovery()` (`run-auto-sub.sh`'s wrapper-retry-on-kill recording layer), because that layer is only reached via XL sub-issue / Batch Mode dispatch — `/auto`'s single-Issue direct dispatch calls `run-code.sh`/`run-spec.sh` directly and never passes through `run-auto-sub.sh` (see Issue #1320 Spec Notes for the full investigation)
- Placing the recording inside the wrapper itself, immediately before `exec`, uniformly covers every calling shape (single-Issue direct dispatch, XL sub-issue, Batch Mode, standalone invocation) with one implementation per phase
- `run-code.sh`'s top-level bash code runs in `MAIN_REPO_ROOT` (the main repository, not the `/code` `claude -p` child's own worktree), so `modules/worktree-lifecycle.md`'s worktree write-path constraints do not apply here — the same execution context `_write_wrapper_retry_recovery()` already relies on. `run-spec.sh` runs from the main repository throughout (it never `cd`s into a worktree itself — only the `/spec` `claude -p` child does, inside its own process), so `_write_spec_retry_recovery()` reuses the wrapper's own `REPO_ROOT` for the same reason
- Pointer comment `# See modules/orchestration-fallbacks.md#auto-retry-on-fail-code_retry_fire` is placed immediately above the `_write_code_retry_recovery` call in `scripts/run-code.sh`, and immediately above the `_write_spec_retry_recovery` call in `scripts/run-spec.sh` — both point at this same anchor rather than a spec-specific one, since the retry pattern is documented here as a single mechanism shared across phases
- Issue #1329 is the decision record for the original code-phase-only scope; Issue #1369 is the decision record for extending it to the spec phase — see Phase Scope Decision above for the per-phase evaluation
...
- Root cause of the external kill itself is settled as of #1146 (2026-08-18): the most-supported hypothesis is H-a — an episodic background-task supervision behavior on the harness (Claude Code) side — with every wholework-local variable tested (uptime, concurrency, batch route, parent session duration) either ruled out or left undecided. The mechanism is not locally verifiable, and resolution depends entirely on upstream harness issues ([#76974](https://github.com/anthropics/claude-code/issues/76974), [#76942](https://github.com/anthropics/claude-code/issues/76942), [#83814](https://github.com/anthropics/claude-code/issues/83814), all OPEN with no staff response as of #1146); see `docs/reports/external-kill-investigation.md` § 2026-08-16 Update (2) and § 2026-08-17 Update
- #1014 (opened 2026-07-13, closed 2026-07-14) previously addressed this symptom, yet respawn recurrence continued afterward — 18 recorded occurrences as of #1390, spanning 2026-07-15 through 2026-08-17. Given the #1146 conclusion above, this is neither insufficient remediation nor a distinct root cause: it is the compensating layer (parent-session respawn) working as designed, which has never failed (17/17 recoveries, no work loss) — so the recurrence count is evidence of expected operation, not a regression to fix

### Operational Policy: respawn as expected recovery
...
This is deliberately not the same as either of the two obvious alternatives, because both have gaps surfaced only during this Issue's investigation:

- **Status-quo exclude-on-close does not fit.** The existing linking model implicitly assumes "Issue closed = root cause resolved," which does not hold for `manual-recovery-respawn`: #1014 was closed on 2026-07-14 (see Rationale above) but the underlying kill mechanism, being harness-side and upstream-gated, was never actually resolved by that closure.
...
- **Case: preview deployment absent**
...
- **Case: no PR-triggered CI workflow**
  - **How to tell**: `ci_result: ... zero_checks=true` is structurally unresolvable in a repository where no workflow under `.github/workflows/` triggers on `pull_request` / `pull_request_target`
  - **Fix**: none needed — `run-review.sh` proceeds to review without PENDING when `scripts/detect-pr-ci-workflows.sh` reports `absent`, and `modules/ci-failure-classifier.md` classifies the same situation as verdict `no-ci-configured`. The preview-deployment Fix above (exporting `PREVIEW_URL`) does not apply to this case
  - **Detection**: `scripts/detect-pr-ci-workflows.sh` (#1463)
...
- Extended in Issue #1463: added the "no PR-triggered CI workflow" case as a second, independent kind of structural PENDING — `run-review.sh` now avoids PENDING at the source when `scripts/detect-pr-ci-workflows.sh` reports `absent`
...
3. The subcommand then writes to two places, each with a distinct consumer, and a passed `--notification CLASS` value lands in both:
   - `_write_manual_recovery_to_recoveries_log()`: appends a `## <date> UTC: manual-recovery-<type>` (H2) entry to `docs/reports/orchestration-recoveries.md` (consumed by `scripts/collect-recovery-candidates.sh` frequency detection and `recoveries-auto-fire`) — skipped when an existing entry with the same symptom-short (`manual-recovery-<type>`) and the same `- Issue #ISSUE, phase: PHASE` Context line is found within the last 24 hours (dedup guard, #1029), so a reissued call for the same recovery does not duplicate the log entry. When `--notification` was passed, the entry's `### Diagnosis` body gets a `- notification: <class>` line
   - `emit_event "manual_intervention" ...`: appends a `manual_intervention` event to `.tmp/auto-events.jsonl` (consumed by the L3 session retrospective's `Parent session manual interventions` Metrics row) — written unconditionally, always carrying a `notification_class=<value>` field (`unspecified` when `--notification` was not passed)
4. `_validate_recovery_args` runs immediately after argument parsing, before either write, and rejects malformed ISSUE/PHASE/RECOVERY_TYPE/EXIT_CODE/NOTIFICATION values with a non-zero exit (a NOTIFICATION outside the 4-value vocabulary aborts before either write, so no record is produced) — this call moved here from the now-removed `_write_manual_recovery_to_spec()` when that function was deleted (#1181)
...
- **Extended in Issue #1049**: investigation of the #1049 recurrence itself (`session 97764-1786198856`, 2026-08-09) found that `#1181`'s removal of `_write_manual_recovery_to_spec()` did not close the bypass it was meant to close — `/verify` Step 12's own skip-judgment rule ("record it in the verify retrospective") gave the LLM no instruction other than writing plain text to the Spec when it detected an unrecorded Manual recovery, so it reproduced the same Spec-only record by following Step 12's own wording rather than any leftover bash path. `skills/verify/SKILL.md` Step 12 now has a second call site for `--write-manual-recovery`: when it detects an unrecorded Manual recovery (as opposed to Tier 2/Tier 3, which are unaffected by this change), it first runs the same `--write-manual-recovery` backfill call documented in the Fallback Steps above, then records the verify retrospective note as before. This is a retroactive backfill (the manual recovery already happened; `/verify` runs after merge), not a real-time hand-off — it exists as a backstop for cases where `/auto`'s real-time hand-off (`skills/auto/SKILL.md`'s Manual recovery hand-off paragraph) was missed during the recovery itself
- **Extended in Issue #1153**: `_write_manual_recovery_to_recoveries_log()` accepts an optional `--notification CLASS` flag (`harness-stop` / `external-signal` / `indeterminate` / `unobserved`), written as a `- notification: <class>` line in `### Diagnosis` and as `notification_class=<class>` on the `manual_intervention` event (`unspecified` when the flag is omitted). This records the parent session's classification of the task notification wording it observed for a killed background task — a channel independent of `wrapper_exit_code`, which is structurally unobtainable for an externally-killed process group (see `#external-kill-parent-respawn` above). The flag does not participate in `collect-recovery-candidates.sh` grouping (only `--cause` does) and does not change the respawn decision threshold in `detect-external-kill.sh` — it is an observation-only addition
