...
When `run-auto-sub.sh` runs the Tier 2 path (`apply-fallback.sh` succeeds), the recovery record is written to `docs/reports/orchestration-recoveries.md` via Step 4a Source 1 in the parent `/auto` session. `apply-fallback.sh`'s stdout (structured symptom-short / phase / fallback-action / result metadata) is discarded (redirected to `/dev/null`) rather than captured into a Spec file — as of #1181, `run-auto-sub.sh` no longer writes a per-Issue Spec-side paper trail for Tier 2 recovery, so there is no local mechanism consuming that metadata. The parent session's Step 4a write to `orchestration-recoveries.md` is the sole recording path for Tier 2.
