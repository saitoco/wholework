...
- **No PR-triggered CI workflow**: `ci_result: ... zero_checks=true` is structurally unresolvable in a repository where no workflow under `.github/workflows/` triggers on `pull_request` / `pull_request_target`. `run-review.sh` proceeds to review without PENDING when `scripts/detect-pr-ci-workflows.sh` reports `absent`, and `modules/ci-failure-classifier.md` classifies the same situation as verdict `no-ci-configured` (#1463)
