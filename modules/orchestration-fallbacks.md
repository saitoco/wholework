...
4. The open-PR guard (`gh pr list --search "closes #ISSUE" --state open`) applies **only** to the Spec write in step 3's first bullet — a self-induced merge conflict risk exists only there, since the open PR's branch may also touch the same Spec file (#890). The recoveries log and event writes proceed unconditionally with respect to the open-PR guard: `orchestration-recoveries.md` is not touched by PR branches, and the event log has no merge-conflict surface at all. The recoveries log write is, however, now additionally subject to the 24-hour dedup guard described in step 3's second bullet — a distinct, narrower gate than the open-PR guard, aimed at the reissue-after-merge scenario rather than merge-conflict avoidance
