...
3. The subcommand then writes to three places, each with a distinct consumer:
   ...
   - `_write_manual_recovery_to_recoveries_log()`: appends a `## <date> UTC: manual-recovery-<type>` (H2) entry to `docs/reports/orchestration-recoveries.md` (consumed by `scripts/collect-recovery-candidates.sh` frequency detection and `recoveries-auto-fire`) — written regardless of Spec existence, but skipped when an existing entry with the same symptom-short (`manual-recovery-<type>`) and the same `- Issue #ISSUE, phase: PHASE` Context line is found within the last 24 hours (dedup guard, #1029) — this covers the legitimate "1st call skips the Spec write due to an open PR → 2nd call after the PR merges" reissue described in step 4, which previously duplicated this log entry on the 2nd call even though the Spec write itself only completed once
   - `emit_event "manual_intervention" ...`: appends a `manual_intervention` event to `.tmp/auto-events.jsonl` (consumed by the L3 session retrospective's `Parent session manual interventions` Metrics row) — written unconditionally regardless of Spec existence
4. The open-PR guard (`gh pr list --search "closes #ISSUE" --state open`) applies **only** to the Spec write in step 3's first bullet — a self-induced merge conflict risk exists only there, since the open PR's branch may also touch the same Spec file (#890). When the guard fires, the record is **deferred, not discarded** (#1150 -- (b) defer + flush): `_defer_recovery_record()` appends it to `.tmp/deferred-recovery-records-<issue>.md` and emits a `recovery_record_deferred` event, in place of the prior behavior of dropping it with only a stderr warning. `_flush_deferred_recovery_records()` recovers deferred records into the Spec once no PR is open for the issue anymore; it is called both at the start of every `--write-manual-recovery` invocation and at the end of the sub-issue's main flow, so a later run (manual or automatic) always picks up records a prior run had to leave behind. The recoveries log and event writes proceed unconditionally with respect to the open-PR guard: `orchestration-recoveries.md` is not touched by PR branches, and the event log has no merge-conflict surface at all. The recoveries log write is, however, now additionally subject to the 24-hour dedup guard described in step 3's second bullet — a distinct, narrower gate than the open-PR guard, aimed at the reissue-after-merge scenario rather than merge-conflict avoidance
...
- `_write_tier2_recovery_to_spec()` first checks for an open PR linked to the issue (#1150). If one exists, it defers the metadata file's content to `.tmp/deferred-recovery-records-<issue>.md` via `_defer_recovery_record()` instead of writing to the Spec, avoiding a self-induced merge conflict with that PR's own Spec changes (#890/#1005/#1006/#1123). Otherwise it appends the metadata to the Spec's `## Auto Retrospective` section and commits/pushes immediately, as before

This write happens during the sub-issue execution phase, before the parent `/auto` Step 4a runs. When Step 4a Source 1 (`fallback-catalog`) runs later, the sub-issue Spec Auto Retrospective is already up to date (or, if deferred, is caught up by `_flush_deferred_recovery_records()` at the end of the sub-issue's main flow — see #1150). The parent session's Step 4a writes `orchestration-recoveries.md` as the session-level SSoT; the Spec write here serves the per-Issue paper trail.
...
Before recording anything new, the `--write-manual-recovery` dispatch first calls `_flush_deferred_recovery_records(ISSUE)` (#1150), which recovers any records a prior call had to defer (see below) into the Spec now that the blocking PR may have merged since.

This then invokes `_write_manual_recovery_to_spec()`, which first checks for an open PR linked to the issue and, if none exists, appends a `### Manual recovery (PHASE)` entry to the sub-issue Spec's `## Auto Retrospective` section and commits/pushes immediately — symmetric with the Tier 2 and Tier 3 bash paths described below. If an open PR exists for the issue, the write and commit/push to main are **deferred, not skipped** (#1150): `_defer_recovery_record()` appends the record to `.tmp/deferred-recovery-records-<issue>.md` and a warning is logged; no manual action is required to recover it — the next `--write-manual-recovery` call for the issue (or the sub-issue's own main-flow completion, see the Tier 2/Tier 3 sections below) flushes it into the Spec automatically once the PR is no longer open. If no formal Spec exists yet for the issue (`docs/spec/issue-N-*.md` glob is empty — e.g. a kill during triage/spec phase), the write is skipped without creating a stub Spec file; a message is logged and the record is preserved via the recoveries log + `manual_intervention` event only, to be folded into the formal Spec once spec phase creates it (#1015). `/verify` Step 12 treats `### Manual recovery` entries as "already recorded" and skips redundant retrospective writing.
...
- `_write_tier3_recovery_to_spec()` is then called with `"$issue" "$phase" "$exit_code"` to build a minimal entry from those variables into a temp file, then checks for an open PR linked to the issue (#1150). If one exists, it defers the entry to `.tmp/deferred-recovery-records-<issue>.md` via `_defer_recovery_record()` instead of writing to the Spec, for the same self-induced-merge-conflict reason as Tier 2 above. Otherwise it appends the entry to the Spec's `## Auto Retrospective` section, committing and pushing in turn, as before
...
This write establishes the same per-Issue paper trail as Tier 2 (`_write_tier2_recovery_to_spec()`), enabling `/verify` Step 12's skip-judgment logic to treat Tier 3 recovery as "already recorded" without requiring manual supplementation in the verify retrospective. If deferred, `_flush_deferred_recovery_records()` (called at the end of the sub-issue's main flow, and at the start of every `--write-manual-recovery` invocation) recovers the entry into the Spec once the blocking PR is no longer open (#1150).
