...
**When environment variables are not set — resolving via `preview-basic-auth-command` (direct `/review` execution, browser-use CLI only):** if both variables are unset, the calling skill has already loaded `HAS_PR_PREVIEW_CAPABILITY` and its value is `true`, `.wholework.yml` declares `preview-basic-auth-command`, and the tool detected in Step 2 is **browser-use CLI**, run `${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-env.sh basic-auth "<PR number>" --format user-pass 2>/dev/null` — in the **same** Bash tool call that launches the browser tool, not a separate call, since environment variables exported in one Bash call do not persist to the next. If stdout is non-empty, it is the path to a 2-line file (username on line 1, password on line 2); load it with `{ IFS= read -r PREVIEW_BASIC_USER; IFS= read -r PREVIEW_BASIC_PASS; } < "<path>"`, export both, and immediately `rm -f "<path>"`. Never write the file's contents, or the resolved username/password, to logs or verification results (same masking policy as above). If stdout is empty, resolution failed — fall through to the CDP-attach fallback below unchanged (fail-open, not FAIL). **This resolution step does not apply on the Playwright MCP path**: Step 4's Playwright branch launches via an MCP tool call, not a Bash call, so there is no Bash call to export environment variables into, and reading the file's contents back into the conversation to build the `extraHTTPHeaders` literal would violate the masking rule above — on that path, proceed directly to the pre-existing behavior described in the "Known Playwright MCP exposure" note below.

Priority order **(exhaustive)**: explicitly exported environment variables > `preview-basic-auth-command` resolution (browser-use CLI only, see above) > CDP-attach fallback (browser-use CLI only, see below) > unauthenticated.

**Known Playwright MCP exposure (not newly introduced by this resolution step):** the Playwright MCP path (Step 4) requires `Authorization: Basic <base64(user:pass)>` as a literal `extraHTTPHeaders` tool argument, so on that path the credential value necessarily appears in the conversation regardless of whether it came from an exported env var or from `preview-basic-auth-command` resolution. Step 2's detection priority makes browser-use CLI (priority 1) the default when both tools are available, and browser-use CLI resolves entirely through environment variable references, so this exposure does not occur on the default path — it is identical to the pre-existing behavior when credentials are already exported, not a new exposure this resolution step introduces.
