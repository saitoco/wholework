...
**Deterministic rotation fallback (Issue #1374)**: Step 3's rotation is prose-driven — the LLM searches for an existing `## Phase Handoff` block and replaces it via the Edit tool — with no mechanical verification that the replace actually succeeded. Under context pressure or boundary-detection ambiguity, the replace can silently fail and a new block gets appended instead of replacing the old one, leaving two `## Phase Handoff` sections in the same Spec (observed in Issue #1363). `scripts/dedupe-phase-handoff-section.sh <ISSUE_NUMBER>` is a deterministic Secondary layer that collapses any Spec file with 2+ `## Phase Handoff` headings down to the last (most recent) one. It is called immediately after this Write Procedure and immediately before the calling phase's own existing commit step, so any rewrite it makes rides along in that same commit. Three phases wire it in: `skills/code/SKILL.md`, `skills/review/SKILL.md`, and `skills/merge/SKILL.md` (the three intermediate phases that write a handoff — see Phase Position Asymmetry below; `spec` always writes first with no existing block to rotate, and `verify` never writes).
...
- **Two-layer rotation (Issue #1374)**: Primary = the LLM's in-session Edit-tool replace (Write Procedure Step 3); Secondary = the deterministic `scripts/dedupe-phase-handoff-section.sh` bash fallback described above, mirroring the same Primary/Secondary structure `modules/l0-surfaces.md`'s Consumed Comments mechanism already uses.
