...
2. **Stale worktree check** (when step 1 recorded `ENTERED_WORKTREE=true`; run before calling `EnterWorktree(name: WORKTREE_NAME)` in step 3): `detect-foreign-worktree.sh` only inspects the *current* branch, so it cannot see a worktree directory left behind by a previous session that crashed or exited without calling `ExitWorktree` — from the main repo root, such a worktree is invisible to step 1 and would otherwise conflict with a fresh `EnterWorktree(name: ...)` call. Check whether `.claude/worktrees/$WORKTREE_NAME` already exists on disk:
   ...
   - **Exists** (candidate stale worktree): treat it as a live conflict — not stale — unless there is positive evidence the owning process has actually ended (e.g., no concurrent session or `/auto` run is known to hold it); when in doubt, stop and surface the conflict instead of acting automatically. Before counting a `ListAgents` (or equivalent session-listing) row as such evidence, filter out rows that describe the current phase's own ancestor/batch rather than a genuinely separate peer session:
     - **Only relevant when this phase itself is running under `/auto` orchestration.** Check whether `AUTO_SESSION_ID` is set — as an env var, or resolvable via the `.tmp/auto-session-issue-<N>` / `.tmp/auto-session-<pgid>` pointer files (`restore_auto_session_pointer()`, `modules/event-emission.md`). If unset, this phase has no `/auto` ancestor to be confused with — skip this filter entirely and treat every `ListAgents` row as independent peer evidence per the default rule above.
     - If `AUTO_SESSION_ID` is set, call `ListAgents` and compare each row's displayed name against the `/auto` session-title conventions `scripts/hook-rename-on-auto.sh` assigns: `auto batch #N1,N2,...`, `auto batch (N issues)`, `auto #N: <title>`, `auto #N (resume): <title>`.
     - For a row matching one of those patterns, extract the embedded Issue number(s). Let `SELF_NUMBER` be the Issue number this phase is currently processing (`$NUMBER` for `/spec`, `/code`, `/verify`; `$ISSUE_NUMBER` for `/review`, `/merge` — if not yet resolved at this point, skip the filter for that row and fall through to the conservative default). If `SELF_NUMBER` is among the row's extracted numbers, that row describes the `/auto` (or `/auto --batch`) session that dispatched — directly, or via an intermediate `run-auto-sub.sh`/`run-*.sh` wrapper — this very phase run. Treat it as **self-reference**, not peer evidence, and exclude it from the live-conflict determination.
     - Rows that do not match the title pattern, or whose extracted numbers do not include `SELF_NUMBER`, describe a genuinely different session — keep treating those as live-peer evidence.
     - This filter targets the specific false-positive shape behind Issue #1394 (a session mistaking its own dispatch chain's ancestor for a live peer holding the worktree); it does not relax "when in doubt, stop" for any row that survives the filter, or for any other kind of missing positive-ending evidence.

     Once confirmed stale, decide **reuse vs. discard**:
...
Shell helpers that must be invoked via `source` because they define a function rather than being directly executable (e.g. `emit-event.sh`'s `emit_event`) cannot run inside a worktree session directly: the worktree isolation guard cannot verify that a sourced command stays inside the worktree, and blocks it outright.

Handle it with one of the following, in order of preference:

- **Wrapper script pattern (first choice)**: move the `source` + function-call sequence into a small, directly-executable script (e.g. `scripts/emit-skill-event.sh`, which internally sources `emit-event.sh` and calls `restore_auto_session_pointer()`/`emit_event()`, and `scripts/collect-run-facts.sh --session-from-issue`, which does the same for session-id resolution). The guard has no objection to running a plain executable script — only to a `source` appearing directly in the command text — so this rewrite eliminates the blocker entirely rather than working around it. #1458 (`skills/verify/SKILL.md`) and #1461 (`modules/opportunistic-verify.md`, `modules/retro-proposals.md`) both resolved their compound `source` snippets this way; a second, independent instance is enough to establish this as the general pattern for any future `source`-based function call surfaced inside a worktree session.
- **Run it after Worktree Exit**, when no wrapper script is a good fit (e.g. the call site's arguments are hard to express as a single command) and the Step naturally reaches a point where the worktree is being left anyway (e.g. a phase's final Worktree Exit Step). This keeps the event's timestamp only slightly later than the point it actually describes.
- **Skip it as best-effort**, when neither of the above applies (e.g. a `source`-based call is needed partway through a Step, with more worktree work still to follow, and no wrapper script exists yet). Note explicitly that the call was skipped — silently swallowing it risks degrading downstream event-driven aggregation (e.g. L3 retrospective metrics, session-boundary detection) with no visible signal that data is missing.
