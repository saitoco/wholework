...
| `actual.worktree_commits_found` | boolean | When `code-patch` completion does not find a `closes #N` commit, operate marker, or stray PR | `true` if the `worktree-code+issue-N` branch has commits ahead of `origin/main` (`git rev-list --count`, read-only). Distinguishes "not yet started" (`false`) from "committed in worktree, push not yet complete" (`true`) — both would otherwise collapse to the same `commits_found: false` observation. Branch not existing also yields `false` (a single `git rev-list` call folds both cases). Diagnostic only — does not affect `matches_expected`. |
