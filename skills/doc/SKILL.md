...
- `translate` → generate Japanese translations for English documentation (commit and push included)
...
If ARGUMENTS is `translate`: execute the "translate — Japanese Translation Generation" section and exit.

For any other ARGUMENTS: display "Usage: /doc [init|init --deep|product|tech|structure|product --deep|tech --deep|structure --deep|sync|sync --deep|sync {doc}|add {path}|project|translate]" and exit.
...
- **Translation output**: README.ja.md at project root, files under `docs/ja/` (generated by `/doc translate`)
...
## translate — Japanese Translation Generation

Execute when ARGUMENTS is `translate`. Generate Japanese translations of English documentation using LLM, place output under `docs/ja/` (README → `README.ja.md` at project root), commit and push automatically.
...
- Other paths → `docs/ja/{filename}.md`

**Translation instructions (apply to all files):**

Translate the entire file content to Japanese following these rules:
...
*Rule 1 — Depth adjustment (for `docs/{name}.md` → `docs/ja/{name}.md` case):*

Output directory is one level deeper, so rewrite paths starting with `../` to `../../`.

| Original (inside `docs/workflow.md`) | Rewrite for translation (`docs/ja/workflow.md`) |
|---|---|
| `../skills/code/SKILL.md` | `../../skills/code/SKILL.md` |
| `../modules/size-workflow-table.md` | `../../modules/size-workflow-table.md` |
| `../README.md` | `../../README.ja.md` (if translation exists) or `../../README.md` |
| `../CLAUDE.md` | `../../CLAUDE.md` |

Same-directory links (e.g., `[workflow.md](workflow.md)` inside `docs/product.md`) remain unchanged — `docs/ja/product.md` pointing to `workflow.md` correctly resolves to `docs/ja/workflow.md`.
...
| Original (inside `README.md`) | Rewrite for translation (`README.ja.md`) |
|---|---|
| `docs/structure.md` | `docs/ja/structure.md` (translation target → substitute) |
...
2. Apply Rule 2 (translation version substitution) after, replacing `../../` paths that have translated versions with `../../docs/ja/{name}.md`
...
If "No" is selected, display "Translation files were generated but not committed. Review them and run `/doc translate` again to commit." and exit.

### Step 5: Commit and Push

```bash
git status
git add README.ja.md docs/ja/
git commit -m "docs: regenerate Japanese translations
...
```
Japanese translation generation complete.
  Committed and pushed: README.ja.md, docs/ja/ ({N} files)

To regenerate translations, run `/doc translate` again.
```
