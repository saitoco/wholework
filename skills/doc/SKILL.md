...
### Step 0: Validate Language Argument

If `{lang}` is empty (ARGUMENTS is exactly `translate` with no language code), display:

```
Usage: /doc translate {lang}
  {lang}: BCP 47 / ISO 639-1 language code (e.g., ja, ko, zh-cn, pt-br, fr)
```

Then exit without performing any translation.
