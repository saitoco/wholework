...
This file is only loaded when `HAS_BROWSER_CAPABILITY=true`. `HAS_BROWSER_CAPABILITY` is fetched from `${CLAUDE_PLUGIN_ROOT}/modules/detect-config-markers.md` and enabled by declaring `capabilities.browser: true` in `.wholework.yml`.
...
2. Read `${CLAUDE_PLUGIN_ROOT}/modules/browser-verify-security.md` and run the URL security constraint check (if constraints are not met, treat as UNCERTAIN)
