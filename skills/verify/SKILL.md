...
3. **Skip condition check**: Before persisting, evaluate whether ALL of the following hold:
   ...
   **Tier 2/3/Manual automatic recovery handling**: When evaluating the third condition, apply the following rule for orchestration recovery observations:
   ...
   This rule makes the skip determination mechanical: check `docs/reports/orchestration-recoveries.md` for a matching entry first, then fall back to the Spec's `## Auto Retrospective` section for pre-#1181 records. If either has a match, it is already the SSoT — no duplication in the verify retrospective.
