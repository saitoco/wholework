...
Within the `### Pre-merge` section, any AC line that carries a `<!-- ac-tier: preview -->` tag is classified as a preview-tier AC. This applies equally to auto preview-tier ACs (they carry a verify command) and manual preview-tier ACs (they carry no verify command, tagged `<!-- verify-type: manual -->` instead, and were presented to a human as a preview-URL check item at `/review`). These ACs are normally treated as already verified/checked at `/review` time against the preview URL and skipped here to prevent double verification — but this assumption must be checked against the `type=preview-ac-unverified` marker collected during Step 4's comment consumption, since `/review` may have ended some preview-tier ACs as UNCERTAIN instead of actually verifying them.

Run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-ac-fallback.sh $NUMBER` and treat its stdout as the fallback-target index list: a comma-separated list of 1-based indices (per `modules/l0-surfaces.md` § "Machine-Readable Event Marker"), or empty when there is no fallback work to do. The script resolves only the single `type=preview-ac-unverified` marker comment with the greatest `createdAt` among those consumed in Step 4 (latest-wins) — it never merges indices across multiple markers — and already collapses the marker's `ac=none` sentinel, an absent marker, and an unparseable `ac=` value down to the same empty-output case.

**Review completion check (run once, only when the script's output is empty):** An empty output from `resolve-preview-ac-fallback.sh` is ambiguous — it means either "`/review` completed normally and explicitly posted `ac=none`" (verified) or "`/review` terminated abnormally before ever posting the marker" (never verified). To disambiguate, run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/reconcile-phase-state.sh review $NUMBER --check-completion --pr $PR_NUMBER --warn-only` **exactly once** for this Pre-merge section (not once per `ac-tier: preview` AC) and read `matches_expected` from its stdout JSON. Set `REVIEW_SUMMARY_FOUND=true` only when the value is the literal string `true`; treat every other outcome — `false`, unparseable JSON, or no stdout at all — as `REVIEW_SUMMARY_FOUND=false` (fail-closed: when completion cannot be confirmed, do not assume verified). This also covers the case where `reconcile-phase-state.sh` itself fails (e.g. `gh` unavailable): it produces no stdout, so `REVIEW_SUMMARY_FOUND=false` via the same default rather than any special-cased handling. The same applies when `resolve-preview-ac-fallback.sh` itself fails (e.g. `gh` unavailable): it now exits with code 2, but its stdout is still empty in that case, which is indistinguishable from the ambiguous "empty" case this review completion check exists to disambiguate — so that failure, too, flows through this check and lands on `REVIEW_SUMMARY_FOUND=false` when completion cannot be confirmed, rather than being silently treated as verified. Skip this check when `resolve-preview-ac-fallback.sh`'s output is non-empty — `REVIEW_SUMMARY_FOUND` is not used in that branch below.

For each `ac-tier: preview` AC in this Pre-merge section, using its own 1-based index in the Issue body's full AC enumeration:

- **The script's output is non-empty and this AC's index is not in the output list, or the script's output is empty and `REVIEW_SUMMARY_FOUND=true`**: unchanged — record as SKIPPED with the note "preview-tier AC; verified/checked at /review against preview URL" (auto ACs were verified against the preview URL; manual ACs were presented as a human-check item and confirmed there). A confirmed Review Response Summary means `/review` completed normally, so an empty fallback-target list here reflects genuine `ac=none`, not an abnormal termination.
- **This AC's index is in the script's output list** (i.e., `/review` left it UNCERTAIN and never actually verified/confirmed it against the preview URL), **or the script's output is empty and `REVIEW_SUMMARY_FOUND=false`** (i.e. `/review` ended without posting a Review Response Summary — likely abnormal termination, including when the completion check script itself failed): do not record as SKIPPED. First check whether this AC has a verify command:
  - **No verify command (manual preview-tier AC)**: there is no `{{base_url}}` to resolve and nothing to execute, so do not attempt `PRODUCTION_URL` resolution regardless of whether `PRODUCTION_URL` is configured. Record as UNCERTAIN with the note "preview-tier AC (manual) unverified at /review; no automatic fallback exists — human verification required" for the marker-based subcase, or "preview-tier AC (manual) unverified — /review ended without posting a Review Response Summary (likely abnormal termination); no automatic fallback exists — human verification required" for the Review Response Summary-absent subcase.
  - **Has a verify command (auto preview-tier AC)**, branch on `PRODUCTION_URL`:
    - **`PRODUCTION_URL` is non-empty**: resolve the verify command's `{{base_url}}` to `PRODUCTION_URL` and actually execute it, exactly as a normal post-merge AC would be. Record the resulting PASS/FAIL/UNCERTAIN classification with the note "preview-tier AC unverified at /review; fallback-verified against production URL" for the marker-based subcase, or "preview-tier AC unverified — /review ended without posting a Review Response Summary (likely abnormal termination); fallback-verified against production URL" for the Review Response Summary-absent subcase.
    - **`PRODUCTION_URL` is empty**: record as UNCERTAIN with the note "preview-tier AC unverified at /review; no production-url configured for fallback — manual verification required" for the marker-based subcase, or "preview-tier AC unverified — /review ended without posting a Review Response Summary (likely abnormal termination); no production-url configured for fallback — manual verification required" for the Review Response Summary-absent subcase.
...
> PR does not exist (patch or operate route). Use `github_check "gh run list"` form instead. See `${CLAUDE_PLUGIN_ROOT}/modules/verify-classifier.md`.
...
(stale skill body warning line here, plain Markdown text — omit entirely when STALE_SKILL_BODY_DETECTED=false)
...
- If `NEXT_ITERATION < VERIFY_MAX_ITERATIONS` (limit not yet reached):
    ...
    - **Tier-gated auto-retry check** (`auto-retry-on-fail` config key):
      ...
      Else if `AUTONOMY_TIER` is `L2` or `L3` AND `AUTO_RETRY_ENABLED=true` (i.e., `auto-retry-on-fail.enabled: true` in `.wholework.yml`) AND `NEXT_ITERATION` < `AUTO_RETRY_MAX_ITERATIONS`:
        ...
        c. Re-invoke code phase with Size-aware route selection:
           ```bash
           ...
           ```
        d. After `run-code.sh` completes (or is skipped for XL), if RETRY_ARGS was non-empty, restart verification from Step 5 (the LLM re-executes Steps 5–11 in the same session context). If RETRY_ARGS was empty (XL skip), do not restart.
...
3. **Skip condition check**: Before persisting, evaluate whether ALL of the following hold:
   ...
   **Tier 2/3/Manual automatic recovery handling**: When evaluating the third condition, apply the following rule for orchestration recovery observations:
   - As of #1181, `run-auto-sub.sh`-driven Tier 2, Tier 3, and Manual recovery are recorded in `docs/reports/orchestration-recoveries.md` (the SSoT), not the Spec's `## Auto Retrospective` section — Manual recovery additionally emits a `manual_intervention` event to `.tmp/auto-events.jsonl`. Recovery observations already present in `orchestration-recoveries.md` for this Issue are considered "already recorded" — they do not count as notable content for the verify retrospective.
   - If the Spec's `## Auto Retrospective` section has a pre-#1181 `### Manual recovery`, `### Tier 2 recovery`, or `### Tier 3 recovery` entry for this Issue (written before `_write_manual_recovery_to_spec()` / `_write_tier2_recovery_to_spec()` / `_write_tier3_recovery_to_spec()` were removed), that entry also counts as "already recorded" — existing Spec entries are not retroactively invalidated.
   - If a Tier 2 or Tier 3 recovery occurred but is recorded in neither `orchestration-recoveries.md` nor a pre-existing Spec entry, it qualifies as notable content. Do not skip; record it in the verify retrospective.
   - If a Manual recovery occurred but is recorded in neither `orchestration-recoveries.md` nor a pre-existing Spec entry, first backfill the canonical record by running `bash ${CLAUDE_PLUGIN_ROOT}/scripts/run-auto-sub.sh --write-manual-recovery $NUMBER PHASE RECOVERY_TYPE` (EXIT_CODE is omitted — it is unobservable after the fact; determine PHASE/RECOVERY_TYPE from the git log / Spec / Issue body already collected in Step 1 — `PHASE`/`RECOVERY_TYPE` must be the bare lowercase-hyphen form, e.g. `code`, not the `phase/*` label form, since `_validate_recovery_args` rejects anything else). This writes the entry to `orchestration-recoveries.md` and, when this `/verify` run has a resolvable `AUTO_SESSION_ID`/session pointer (i.e., it is running inside an `/auto`-orchestrated chain), also emits the `manual_intervention` event — closing the gap between `/auto`'s real-time hand-off and the retroactive detection performed here. For a standalone `/verify` invocation with no resolvable session pointer, only the `orchestration-recoveries.md` entry is guaranteed (`emit_event` silently no-ops per Issue #1224's fail-closed design; see `modules/event-emission.md` § "Manual Orchestration (Issue #1224)"). After the backfill call, still record it in the verify retrospective as before (Issue #1049; see `modules/orchestration-fallbacks.md`'s `manual-recovery-spec-write` entry for the Rationale this backstop addresses).

   This rule makes the skip determination mechanical: check `docs/reports/orchestration-recoveries.md` for an H2 entry whose `### Context` block contains `- Issue #$NUMBER, phase: <phase>` (the canonical entry format written by `_write_manual_recovery_to_recoveries_log()` for Manual recovery, `write_recovery_entry()` in `apply-fallback.sh` for Tier 2 recovery (as of #1098), `write_recovery_entry()` in `spawn-recovery-subagent.sh` for Tier 3 recovery, or Step 4a for any recovery event that reaches neither bash-path writer) first, then fall back to the Spec's `## Auto Retrospective` section for pre-#1181 records. If either has a match, it is already the SSoT — no duplication in the verify retrospective.
