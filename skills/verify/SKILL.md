...
Run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-ac-fallback.sh $NUMBER` and treat its stdout as the fallback-target index list: a comma-separated list of 1-based indices (per `modules/l0-surfaces.md` § "Machine-Readable Event Marker"), or empty when there is no fallback work to do. The script resolves only the single `type=preview-ac-unverified` marker comment with the greatest `createdAt` among those consumed in Step 4 (latest-wins) — it never merges indices across multiple markers — and already collapses the marker's `ac=none` sentinel, an absent marker, and an unparseable `ac=` value down to the same empty-output case. For each `ac-tier: preview` AC in this Pre-merge section, using its own 1-based index in the Issue body's full AC enumeration:

- **The script's output is empty, or this AC's index is not in the output list**: unchanged — record as SKIPPED with the note "preview-tier AC; verified/checked at /review against preview URL" (auto ACs were verified against the preview URL; manual ACs were presented as a human-check item and confirmed there). This is also the fallback-open outcome when the script itself fails (e.g., `gh` unavailable), since it fails open with empty output rather than erroring.
- **This AC's index is in the script's output list** (i.e., `/review` left it UNCERTAIN and never actually verified/confirmed it against the preview URL): do not record as SKIPPED. First check whether this AC has a verify command:
  - **No verify command (manual preview-tier AC)**: there is no `{{base_url}}` to resolve and nothing to execute, so do not attempt `PRODUCTION_URL` resolution regardless of whether `PRODUCTION_URL` is configured. Record as UNCERTAIN with the note "preview-tier AC (manual) unverified at /review; no automatic fallback exists — human verification required".
