...
- **The script's output is empty, or this AC's index is not in the output list**: unchanged — record as SKIPPED with the note "preview-tier AC; verified at /review against preview URL". This is also the fallback-open outcome when the script itself fails (e.g., `gh` unavailable), since it fails open with empty output rather than erroring.
- **This AC's index is in the script's output list** (i.e., `/review` left it UNCERTAIN and never actually verified it against the preview URL): do not record as SKIPPED. Instead:
