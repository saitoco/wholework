...
6. When an uncertainty was resolved by verifying a premise against real data or a real fixture (not synthetic/example data), record the input's provenance (出所) in the Notes or Uncertainty entry — the minimum information needed to re-fetch the same input (URL, file path, commit SHA, retrieval timestamp, etc.). Example: `出所: https://notes.granola.ai/xxx (取得日時: 2026-06-12)`. Without this, a later phase re-verifying the same premise has no way to reconstruct which input was actually used, and risks grabbing an unrelated one instead. If the input is a resource on an external service that may not be reliably re-fetchable (e.g., a live document that could later be edited or deleted), also consider saving a copy under `tests/fixtures/` and recording that path instead.
...
**New test case requirement for new branch logic (regardless of SPEC_DEPTH):**
...
**Skip** if no Implementation Step adds new branching logic to an existing file (behavior-only changes to existing branches, documentation-only changes, etc.).

**Differs from adjacent checks:**
- **Stale Test Assertion Check** (`/code`-side): confirms that a *removed* literal string does not linger in `tests/`. This check requires *newly added* tests for *newly added* branch logic — the opposite direction (add vs. delete).
- **Tag/enum semantic extension consumer sweep**: re-validates *existing* consumers' assumptions when a tag/enum's accepted semantics widen without the value string changing. This check requires a *new* test case for *new* branch logic, not re-validation of existing consumers.
...
First, evaluate the Diff-less Axis (operate route) determination — check whether the Spec being produced meets both `modules/size-workflow-table.md` § "Diff-less Axis (operate route)" criteria (`## Changed Files` has no repository file entries, and every `## Implementation Steps` entry is an external-tool operation). `/spec` has not yet resolved `ROUTE` at this point (that happens in Step 18), so this evaluates the same two Diff-less Axis criteria directly against this Spec's own content rather than referencing a `ROUTE` variable. Per `modules/size-workflow-table.md` § "ALWAYS_PR Override" (priority order), operate route determination takes priority over `always-pr: true` regardless of Size — an empty diff cannot produce a meaningful PR.

If `ALWAYS_PR=true` (retained in Step 5) and the Spec does not meet the Diff-less Axis criteria above, skip this entire check — `always-pr: true` promotes Size XS/S to pr route (see `${CLAUDE_PLUGIN_ROOT}/modules/size-workflow-table.md` § "ALWAYS_PR Override"), so a PR does exist and `github_check "gh pr checks"` is the correct form; auto-fixing it to `gh run list` form here would be incorrect. This skip does not apply when the Spec meets the Diff-less Axis criteria — operate route still applies regardless of `always-pr: true`.

After `## Verification > Pre-merge` is finalized and the Issue body is updated, if either (a) `ALWAYS_PR=false` and Size is `XS` or `S` (patch route — no PR exists), or (b) the Spec being produced meets the Diff-less Axis criteria above (this Spec will resolve to operate route, which also produces no PR, regardless of Size or `ALWAYS_PR`), scan `## Verification > Pre-merge` in the Spec for `github_check "gh pr checks"` entries.
- If found: output "Warning: patch/operate route — `github_check "gh pr checks"` is incompatible (no PR exists in patch or operate route). Auto-fixing to `github_check "gh run list"` form." and replace each with `github_check "gh run list --limit=1 --json conclusion,status --jq 'if .[0].status != \"completed\" then \"in_progress\" else .[0].conclusion end'" "success"` (change `expected_value` from the job name to `"success"` — `gh run list` outputs run-level conclusion, not job names; add `--workflow=<filename>` if there are multiple workflow files under `.github/workflows/`; the `status` field is synthesized into the jq output so an in-progress run is detected as PENDING rather than misjudged as FAIL/UNCERTAIN — see `modules/verify-executor.md` § "github_check: `gh run list` In-Progress Detection Constraint"). Update Spec file using Edit tool. Also update Issue body via `gh-issue-edit.sh`.
