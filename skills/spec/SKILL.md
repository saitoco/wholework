...
After `## Verification > Pre-merge` is finalized and the Issue body is updated, if `ALWAYS_PR=false` and either (a) Size is `XS` or `S` (patch route — no PR exists), or (b) the Spec being produced meets both `modules/size-workflow-table.md` § "Diff-less Axis (operate route)" determination criteria (`## Changed Files` has no repository file entries, and every `## Implementation Steps` entry is an external-tool operation — i.e. this Spec will resolve to operate route, which also produces no PR, regardless of Size), scan `## Verification > Pre-merge` in the Spec for `github_check "gh pr checks"` entries. `/spec` has not yet resolved `ROUTE` at this point (that happens in Step 18), so condition (b) evaluates the same two Diff-less Axis criteria directly against this Spec's own content rather than referencing a `ROUTE` variable.
