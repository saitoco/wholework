...
context: fork
allowed-tools: Bash(gh pr view:*, gh pr diff:*, gh pr comment:*, gh issue view:*, gh issue edit:*, gh issue create:*, gh issue list:*, gh api:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-issue-edit.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-issue-comment.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/append-consumed-comments-section.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/dedupe-phase-handoff-section.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-pr-review.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/wait-external-review.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/wait-ci-checks.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/pre-merge-check.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/run-review.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/get-issue-size.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/get-issue-type.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/opportunistic-search.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/collect-run-facts.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/emit-skill-event.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/observation-trigger.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-extract-issue-from-pr.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-label-transition.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/worktree-merge-push.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/detect-foreign-worktree.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-env.sh:*, wc:*, diff:*, git log:*, git diff:*, git show:*, git add:*, git commit:*, git push:*, git fetch:*, git checkout:*, git worktree:*, git branch:*, git merge-tree:*, git merge-base:*, python3:*), Read, Write, Edit, Glob, Grep, Task, TaskCreate, TaskUpdate, TaskList, TaskGet, EnterWorktree, ExitWorktree, Workflow, Skill
---
...
**Comment consumption (unconditional, before Step 3's Size branching):** If `ISSUE_NUMBER` was extracted in Step 1, read `${CLAUDE_PLUGIN_ROOT}/modules/l0-surfaces.md` and follow the "Comment Consumption Procedure" section with parameters: `ISSUE_NUMBER=$ISSUE_NUMBER`, `COMMENT_SCOPE=issue+pr` (a PR necessarily exists by this point in `/review`, matching the same condition `/code`'s resume path already uses to promote `issue` to `issue+pr`), `PHASE_NAME=review`. Run this before Step 3's Size branching (including the XS/S early-exit path) — `/verify`'s Worktree Entry already runs its own Comment Consumption Procedure call unconditionally regardless of Size, and this follows the same precedent. If `ISSUE_NUMBER` could not be extracted in Step 1, skip this call — there is no Issue to consume comments for.

Then run the deterministic fallback (still only when `ISSUE_NUMBER` was extracted):

```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/append-consumed-comments-section.sh "$ISSUE_NUMBER" review
```

No `--no-push` here (unlike `/spec`/`/code`/`/verify`'s convention): Step 3's Size XS/S early-exit branch can end the skill before reaching the unconditional `## Retrospective` push, so relying on a later push to carry this commit risks leaving it stranded, uncommitted-to-remote, on the worktree branch. At this point in `/review`, the PR's mergeability has not yet been decided (no `/merge`-Step-1-equivalent gate has run), so pushing immediately carries no risk beyond re-triggering CI.
...
5. **Phase Handoff dedupe fallback** (deterministic rotation safety net, see `modules/phase-handoff.md` § "Deterministic rotation fallback"): run
   ```bash
...
Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>"
     ...
     ```
7. **If improvement proposals exist**: record in review retrospective only (do not create issues; proposals are aggregated in `/verify`)
...
- **Resolve `EXECUTION_CONTEXT`**: per `${CLAUDE_PLUGIN_ROOT}/modules/execution-context.md` § Context Detection, if `ARGUMENTS` contains `--non-interactive` set `EXECUTION_CONTEXT=fork`; otherwise set `EXECUTION_CONTEXT=main`.
- If `REVIEW_DEPTH=full`: run `${CLAUDE_PLUGIN_ROOT}/scripts/observation-trigger.sh --event pr-review-full --context-file "$DESIGN_FILE_PATH" --execution-context "$EXECUTION_CONTEXT"`
- If `REVIEW_DEPTH=light`: run `${CLAUDE_PLUGIN_ROOT}/scripts/observation-trigger.sh --event pr-review-light --context-file "$DESIGN_FILE_PATH" --execution-context "$EXECUTION_CONTEXT"`
...
- When top-level `docs/*.md` files are changed, if `docs/translation-workflow.md` exists, read it and follow the sync procedure
