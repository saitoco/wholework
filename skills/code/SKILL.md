...
**CI verification AC exclusion (route-agnostic):** Step 10 runs before Step 11 creates the implementation commit (patch route) or the PR (pr route), so a CI verification AC that depends on that commit or PR cannot be evaluated accurately at this point — evaluating it here would either reference an unrelated prior commit's CI, or find no target to check against at all. Exclude both of the following forms from this Step's verify-executor full-mode pass; **neither exclusion checks off its condition** — leave each as `- [ ]` (do not include them in the checkbox-flip loop of result branch 1 below, and do not count them toward "all PASS"):

- **patch route** — `github_check "gh run list"` (the `modules/verify-classifier.md` § "Patch Route CI Verification Note" canonical form, `--branch=main --limit=1`): at this point the implementation commit is not yet pushed, so the most recent run on `main` still predates this Issue's own implementation — evaluating it here would check an unrelated prior commit's CI, not this Issue's. Verified for real when `/verify` runs post-merge, once the implementation commit (or the retrospective commit pushed alongside it) actually has a run.
