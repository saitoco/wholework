...
If ARGUMENTS is `auto-session` or starts with `auto-session` (e.g., `auto-session <session-id>`, `auto-session --since 24h`, `auto-session <id> --output <path>`, `auto-session --day YYYY-MM-DD`, `auto-session --since 7d`, `auto-session --range 2026-06-01..2026-06-07`): execute the "auto-session Subcommand" section and exit.
...
For any other ARGUMENTS: display "Usage: /audit [drift|fragility|stats|progress <XL-parent>|auto-session <session-id>|auto-session --day YYYY-MM-DD|auto-session --since Nd|auto-session --range START..END] [--dry-run] [--limit N] [--since DATE] [--no-save] [--retention] (running `/audit` without arguments executes drift + fragility integrated run)" and exit.
