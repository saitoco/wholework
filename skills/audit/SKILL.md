...
If ARGUMENTS is `auto-session` or starts with `auto-session` (e.g., `auto-session <session-id>`, `auto-session --full <session-id>`, `auto-session --since 24h`, `auto-session <id> --output <path>`): execute the "auto-session Subcommand" section and exit.
