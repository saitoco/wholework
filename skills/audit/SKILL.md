...
For any other ARGUMENTS: display "Usage: /audit [drift|fragility|stats|progress <XL-parent>|auto-session <session-id>|premise] [--dry-run] [--limit N] [--since DATE] [--no-save] [--retention] (running `/audit` without arguments executes drift + fragility integrated run)" and exit.
