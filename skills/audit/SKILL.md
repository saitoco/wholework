...
For any other ARGUMENTS: display "Usage: /audit [drift|fragility|stats|recoveries|progress <XL-parent>|auto-session <session-id>] [--dry-run] [--limit N] [--since DATE] [--no-save] [--retention] [--threshold K] (running `/audit` without arguments executes drift + fragility integrated run)" and exit.
...
7. **Narrative Section (skeleton)** — TBD placeholders for "What worked", "Limits and gaps", "Conclusion" (R3 LLM-assist or manual fill)
...
- `<session-id>` (positional): generate report for this session; required unless `--since` is given
