...
This section is read-only display only — no comment posting, body edit, label transition, or Issue creation (unlike Section 8/9's Retire-Proposal Comment Posting, which stays scoped to phase/verify and Icebox only and — for phase/verify Level 3 under autonomy tier L2/L3 — also auto-retires post-merge conditions and transitions labels, not just comments; see Notes).
...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/apply-verify-retire.sh --issue <N> --dwell <dwell_days>
```

`apply-verify-retire.sh` internally re-derives the escalation level (`compute-escalation-level.sh verify <dwell_days>`) and is the single owner of Level 3's decision — do not call `compute-escalation-level.sh` directly for phase/verify routing. Branch on the script's `action=` output line (exhaustive):

- **`action=none`** (escalation level 0–2): this script performed no L0 write. Route by level using the existing table below (unchanged from before **auto-retire**):
...
- **`action=propose`** (Level 3, autonomy tier L1 — **or** Level 3, autonomy tier L2/L3 whose Post-merge section had zero eligible `observation`/`opportunistic` targets, e.g. `manual`/`auto`-only): post the existing "manual confirmation or remove observation condition" decision-prompt comment below — the script itself resolves the zero-target L2/L3 case to `action=propose` (not `action=retire`) precisely so this comment still posts, since no retire happened and no audit-trail comment exists to serve as the record
- **`action=retire`** (Level 3, autonomy tier L2/L3, at least one eligible target): the script has already **auto-retired** the target conditions — removed unchecked `observation`/`opportunistic` post-merge conditions from `### Post-merge`, appended them to `### Retired Post-merge Conditions`, posted a `type=verify-ac-retired` audit-trail comment, and (when no unchecked post-merge condition remains, any verify-type) transitioned the Issue to `phase/done`. **Do not post an additional comment for this Issue** — the script's own audit-trail comment is the complete record. Read the script's `retired=`/`remaining=`/`transitioned=` lines to include this Issue in this section's own summary output (e.g. "N Issues auto-retired this run"). `action=retire` is also printed on the script's fail-open short-circuits (issue body fetch failure, or a malformed-structure guard for multiple `### Post-merge` headings / a `### Retired Post-merge Conditions` section that precedes `### Post-merge`) — those cases print `retired=0` with no `remaining=`/`transitioned=` lines and no comment; do not treat the fail-open `retired=0` case as needing an additional comment either, since these are transient/malformed-input conditions expected to resolve or be hand-fixed before the next `/audit stats --retention` run picks the Issue up again

**Retire target scope**: only unchecked post-merge conditions tagged `verify-type: observation` or `verify-type: opportunistic` are eligible for auto-retire. `manual` conditions are excluded because they were explicitly judged to require human confirmation — an automatic retire would silently overturn that judgment. `auto` conditions (those carrying a `verify:` command) are excluded because they are recoverable by a single future `/verify` dispatch — retiring them would discard a verification opportunity that costs nothing to keep open. Both exclusions leave `manual`/`auto` Level 3 Issues on the `action=propose` path (a human, or a follow-up Issue, still decides) — enforced by the script itself resolving to `action=propose` when zero targets are found, not merely a documentation convention.

**Autonomy tier gate**: `apply-verify-retire.sh` resolves `AUTONOMY_TIER` the same way every other autonomy-gated script in this repository does (env var, then `.wholework.yml`'s `autonomy` key, default `L1`). At **L1**, Level 3 never writes to L0 — the existing decision-prompt comment is the only output, identical to pre-**auto-retire** behavior. At **L2**/**L3**, Level 3 auto-retires without further confirmation. There is no separate opt-in config key for this feature (see the Spec's Notes for why `recoveries-auto-fire`'s opt-in shape does not transfer here) — the existing `autonomy:` tier is the only gate.
...
**Duplicate prevention**: before posting a comment, fetch existing comments via `gh issue view --json comments` and check for a comment containing the same escalation level marker (`<!-- escalation-level: N -->`). If found, skip that Issue — do not post a duplicate. This rule governs only the comments this section posts itself (Level 0/1/2 `action=none` routing, and `action=propose`). The `type=verify-ac-retired` audit-trail comment posted directly by `apply-verify-retire.sh` for `action=retire` is intentionally exempt — it is an append-only history record, not a duplicable notice, and the script's own zero-target/malformed-structure guards already prevent a redundant retire on re-runs.
