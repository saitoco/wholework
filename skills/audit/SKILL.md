...
- `.tmp/auto-session-current` — PGID-independent pointer file, also written by `/auto` Step 1 alongside the PGID-specific file. Read by `restore_auto_session_pointer()` (`scripts/emit-event.sh`) as the last-resort fallback when a caller has neither a PGID-matching nor an issue-scoped pointer — under concurrent `/auto` sessions this file does not guarantee attribution accuracy, since any session's Step 1 may have last overwritten it (Issue #902 Fix Cycle; see Issue #1075 for the concurrent-session attribution gap this file's use as fallback-only, rather than primary, now addresses)
