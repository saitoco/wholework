...
If ARGUMENTS is `auto-session` or starts with `auto-session` (e.g., `auto-session <session-id>`, `auto-session --since 24h`, `auto-session <id> --output <path>`, `auto-session <id> --no-ja`, `auto-session --day YYYY-MM-DD`, `auto-session --since 7d`, `auto-session --range 2026-06-01..2026-06-07`): execute the "auto-session Subcommand" section and exit.
...
- `<session-id>` (positional): display or generate report for this session; required unless `--since` is given or period mode flags are used
...
When `<session-id>` is present (single-session mode), check whether a report already exists before running the script:

```bash
# Glob for existing data-layer.md for the session
ls docs/sessions/${SESSION_ID}-*/data-layer.md 2>/dev/null | head -1
```

- **File exists** (report generated by `/auto` Step 5): output the file path, display the report (`cat <path>`), and check for a `session.md` sibling in the same directory. If `session.md` exists, append a cross-link:
  ```
  ---
  See also: [L3 Session Retrospective](docs/sessions/{SESSION_ID}-{DATE}/session.md)
  ```
  Then skip to Step 2 (display result) — the `fallback` script is not needed.
- **File does not exist** (single-Issue route or early manual call): proceed to Step 1 to generate the report via the fallback script.
...
Period aggregate mode stops at Step 2.

### Step 3: Generate Japanese Sibling

This step runs by default after Steps 1–2 complete. Skip entirely when `--no-ja` is present in ARGUMENTS.
...
2. Read the final report (post Step 2)
