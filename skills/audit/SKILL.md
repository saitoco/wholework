...
If ARGUMENTS is `auto-session` or starts with `auto-session` (e.g., `auto-session <session-id>`, `auto-session --since 24h`, `auto-session <id> --output <path>`): execute the "auto-session Subcommand" section and exit.
...
- `<session-id>` (positional): display or generate report for this session; required unless `--since` is given
- `--output <path>`: override output file path (default: `docs/sessions/<id>-<date>/data-layer.md`)
