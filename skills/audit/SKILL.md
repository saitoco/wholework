...
For any other ARGUMENTS: display "Usage: /audit [drift|fragility|stats|recoveries|progress <XL-parent>|auto-session <session-id>] [--dry-run] [--limit N] [--since DATE] [--no-save] [--retention] [--threshold K] [--no-ja] (running `/audit` without arguments executes drift + fragility integrated run)" and exit.
