...
Scan each Issue currently labeled `phase/verify` (closed state) for unchecked (`- [ ]`) lines containing `verify-type: manual`. Apply this per-line rule: a line without `ac-tier: preview` is always counted. A line that also carries `ac-tier: preview` is a pre-merge preview-tier AC, normally already confirmed by `/review` against the preview URL before merge and therefore excluded from the post-merge human-confirmation queue this count measures (see `modules/verify-classifier.md` § Purpose) — but only when `/review` actually verified it. Run `${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-ac-fallback.sh <issue>` (the same script `/verify` Step 5 uses for its pre-merge-preview AC skip rule; see `modules/l0-surfaces.md` § Machine-Readable Event Marker, `type=preview-ac-unverified`) to get the comma-separated 1-based AC indices `/review` left `UNCERTAIN` and never verified. Count an `ac-tier: preview` line only when its own 1-based index in the Issue body's full AC enumeration (same convention as `gh-issue-edit.sh --checkbox`) appears in that output; otherwise it was verified at `/review` and stays excluded. Count the number of Issues with at least one such counted line (`N`).
