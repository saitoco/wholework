...
2. Identify the 1-based indices of this Issue's currently-unchecked `verify-type: manual` conditions, applying the same per-line rule as the `N` calculation above: include a line without `ac-tier: preview` unconditionally; include an `ac-tier: preview` line only when its index appears in `resolve-preview-ac-fallback.sh <issue>`'s output (same enumeration convention as `gh-issue-edit.sh --checkbox`).
