...
2. Identify the 1-based indices of this Issue's currently-unchecked `verify-type: manual` post-merge conditions (read the same HTML-comment-scoped way as the Manual Waiting Count definition above), excluding any line that also carries `ac-tier: preview` (read the same way; same enumeration convention as `gh-issue-edit.sh --checkbox`).
