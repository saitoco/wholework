...
- **`action=propose`** (Level 3, autonomy tier L1): post the existing "manual confirmation or remove observation condition" decision-prompt comment below — unchanged, current behavior preserved at L1
- **`action=retire`** (Level 3, autonomy tier L2/L3): the script has already **auto-retired** the target conditions — removed unchecked `observation`/`opportunistic` post-merge conditions from `### Post-merge`, appended them to `### Retired Post-merge Conditions`, posted a `type=verify-ac-retired` audit-trail comment, and (when no unchecked post-merge condition remains, any verify-type) transitioned the Issue to `phase/done`. **Do not post an additional comment for this Issue** — the script's own audit-trail comment is the complete record. Read the script's `retired=`/`remaining=`/`transitioned=` lines to include this Issue in this section's own summary output (e.g. "N Issues auto-retired this run")

**Retire target scope**: only unchecked post-merge conditions tagged `verify-type: observation` or `verify-type: opportunistic` are eligible for auto-retire. `manual` conditions are excluded because they were explicitly judged to require human confirmation — an automatic retire would silently overturn that judgment. `auto` conditions (those carrying a `verify:` command) are excluded because they are recoverable by a single future `/verify` dispatch — retiring them would discard a verification opportunity that costs nothing to keep open. Both exclusions leave `manual`/`auto` Level 3 Issues on the existing `action=propose`-equivalent path (a human, or a follow-up Issue, still decides).
