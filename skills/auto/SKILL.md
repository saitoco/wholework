...
loop-paths-used: [A]
allowed-tools: Bash(${CLAUDE_PLUGIN_ROOT}/scripts/get-issue-size.sh:*, gh issue view:*, gh issue list:*, gh issue close:*, gh issue comment:*, gh issue create:*, gh pr list:*, gh run list:*, gh run view:*, gh pr checks:*, ${CLAUDE_PLUGIN_ROOT}/scripts/run-code.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/run-review.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/run-merge.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/get-sub-issue-graph.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/run-auto-sub.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/run-spec.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/run-issue.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-label-transition.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/detect-wrapper-anomaly.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/detect-external-kill.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/reconcile-phase-state.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/validate-recovery-plan.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/auto-checkpoint.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-batch-query.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/compute-round-order.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/observation-trigger.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/filter-session-verified-issues.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/rotate-observation-dispatch.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-issue-edit.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/set-blocked-by.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/get-blocked-by.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-check-blocking.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/emit-skill-event.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/get-auto-session-report.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/gh-graphql.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-ac-fallback.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/resolve-preview-env.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/check-session-findings-disposition.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/collect-run-facts.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/scan-pending-ac.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/apply-run-fact-match.sh:*, ${CLAUDE_PLUGIN_ROOT}/scripts/detect-pr-ci-workflows.sh:*), Read, Edit, Glob, Grep, Write, Skill, Task, TaskCreate, TaskUpdate, TaskList, TaskGet
---
...
1. Generate `SESSION_ID` and create the PGID-specific pointer file. Also write the PGID-independent `.tmp/auto-session-current` pointer — kept for `scripts/collect-run-facts.sh` and `scripts/filter-session-verified-issues.sh`'s own fail-open fallback (used when their `--session` argument is omitted), not for session_id resolution in the emit path. It is no longer read by `restore_auto_session_pointer()` in `modules/event-emission.md` (removed as its last resolution step in Issue #1224, to avoid attributing to a session that never wrote it), nor by any of the 5 `run-*.sh` wrappers' own inline fallback (removed in Issue #1317, for the same reason — a wrapper invoked manually while a concurrent `/auto` session is active has no claim to this file). In-session `Skill(skill="wholework:verify", ...)` dispatches use the `--session-id` in-band hand-off described below instead, which is immune to concurrent `/auto` sessions overwriting this file (Issue #1075):
   ```bash
   mkdir -p .tmp
   SESSION_ID="$$-$(date +%s)"
...
**In-session `Skill(skill="wholework:verify", ...)` dispatch uses `--session-id`, not pointer regeneration**: Unlike the `run-*.sh` / `run-auto-sub.sh` calls above, `/verify` has no wrapper script and runs via in-session `Skill()` invocation, where each of its own Bash tool calls gets a fresh process group — the PGID pointer file this section re-generates would never be visible to it. Instead, every `Skill(skill="wholework:verify", ...)` dispatch below passes `--session-id=<literal SESSION_ID value from step 1>` in its `args`, and `/verify` Step 1 persists that value to an issue-scoped pointer file (`persist_auto_session_pointer()` in `modules/event-emission.md`) that its own subsequent Bash calls read back. This in-band hand-off is immune to concurrent `/auto` sessions overwriting `.tmp/auto-session-current` (Issue #1075) — do not additionally regenerate the PGID pointer before a `Skill(wholework:verify)` call, since it would not be read.
...
8. verify phase: invoke `Skill(skill="wholework:verify", args="$NUMBER --session-id=<literal SESSION_ID value from step 1>")` in the parent session (enables AskUserQuestion for manual AC confirmation)
...
8. If review fails: check whether `run-review.sh` exited with code 2 (PENDING — CI/preview state not yet confirmed; see `${CLAUDE_PLUGIN_ROOT}/scripts/run-review.sh` header comment and `modules/orchestration-fallbacks.md#review-pending-not-failure`). If exit code is 2: first Read `${CLAUDE_PLUGIN_ROOT}/modules/ci-failure-classifier.md` and follow the "Processing Steps" section to obtain a verdict.
    ...
    - **verdict = `no-ci-configured`**: PR-triggered CI does not structurally exist in this repository, so no check will ever register — do not sleep and retry (skip the #1115 retry loop below entirely), and fall through immediately to the completion check below.
    ...
    If the exit code is anything other than 2, or exit code 2 persists after the retry limit (or the CI-infra wait/stop path above) is reached, or the verdict is `no-ci-configured`, fall through to the completion check `${CLAUDE_PLUGIN_ROOT}/scripts/reconcile-phase-state.sh review $NUMBER --pr $PR_NUMBER --check-completion` — if `matches_expected: true`, override to success; otherwise go to Step 6
...
15. Output `[4/4] verify`, then invoke `Skill(skill="wholework:verify", args="$NUMBER --session-id=<literal SESSION_ID value from step 1>")` in the parent session (enables AskUserQuestion for manual AC confirmation); on success output `[4/4] verify → done`
...
```
Skill(skill="wholework:verify", args="$N --session-id=<literal SESSION_ID value from step 1>")
```
...
- **L2 / L3**: first run `printf '%s\n' "$OBSERVATION_MATCHES" | "${CLAUDE_PLUGIN_ROOT}/scripts/filter-session-verified-issues.sh" --session <literal SESSION_ID value from step 1>` to filter `OBSERVATION_MATCHES`, excluding Issues that already have a `phase=verify` `phase_start`/`phase_complete` event recorded for this `/auto` session (fail-open: passes candidates through unchanged if the session id or events log cannot be resolved). Then exclude `$NUMBER` (the Issue this `/auto` run just processed) from the filtered result to get `FILTERED_MATCHES`. Run `printf '%s\n' "$FILTERED_MATCHES" | "${CLAUDE_PLUGIN_ROOT}/scripts/rotate-observation-dispatch.sh" --threshold "$OBSERVATION_DISPATCH_THRESHOLD"` to get `DISPATCH_SET`: the script rotates `FILTERED_MATCHES` against a persisted cursor (the last dispatched Issue number, in `.tmp/observation-dispatch-cursor`) before applying the `OBSERVATION_DISPATCH_THRESHOLD` cap, so each dispatch cycle continues from where the previous one left off instead of a fixed subset of Issues permanently occupying every dispatch slot. Then dispatch `Skill(skill="wholework:verify", args="$N --session-id=<literal SESSION_ID value from step 1>")` sequentially for each number in `DISPATCH_SET`. Numbers beyond the cap are not dispatched this run — `observation-trigger.sh` has already posted its notification comment to every matched Issue regardless of the cap, and the deferred Issues are re-matched on the next `auto-run` event scan. When at least one number was deferred (K = count of `FILTERED_MATCHES` not in `DISPATCH_SET`, M = count of `FILTERED_MATCHES`; K > 0), output "Observation dispatch capped at OBSERVATION_DISPATCH_THRESHOLD; deferred K of M matched Issue(s) to the next auto-run scan."

**Run-fact AC reconciliation (runs after the observation scan, best-effort):**

Read `${CLAUDE_PLUGIN_ROOT}/modules/run-fact-matching.md` and follow the "Processing Steps" section.

If `collect-run-facts.sh`, `scan-pending-ac.sh`, or `apply-run-fact-match.sh` fails, output a warning and continue — this reconciliation step never blocks `/auto`.
...
Before entering the Tier 1/2/3 diagnosis flow below, check whether the failed phase's background wrapper was killed externally (its process group was SIGKILLed rather than exiting through its own EXIT trap) — see `modules/orchestration-fallbacks.md#external-kill-parent-respawn` for the full symptom/rationale writeup and #1005/#1014 for the investigations this pre-check is based on.

- **Detection signature**: run the mechanical detector (mirrors the Tier 2 `detect-wrapper-anomaly.sh` invocation pattern below):
  ```bash
  ${CLAUDE_PLUGIN_ROOT}/scripts/detect-external-kill.sh --log .tmp/wrapper-out-$NUMBER-$PHASE.log --events .tmp/auto-events.jsonl --exit-code $EXIT_CODE --issue $NUMBER --phase $PHASE
  ```
  Exit 0 with `external-kill` on stdout: the wrapper's own process group was killed externally without emitting its usual completion markers (exit code 137 alone is conclusive; exit code 143 or an unobserved `EXIT_CODE` of `unknown` requires both the wrapper log's `Exit code: ` trailer and the `wrapper_exit` event in `auto-events.jsonl` to be absent for this issue/phase — see the script header for the full rationale). Exit 1 with `no-match`: proceed to Tier 1 as normal.
...
- **Capture the notification wording (before respawning)**: check the stopped background task's most recent task notification `status`/`summary` text, and classify it for the Recording call below. (a) Wording of the form "failed with exit code N" (a numeric exit code present) classifies as `external-signal`. (b) Wording limited to `killed` / "was stopped" with no numeric exit code classifies as `indeterminate` — this wording alone does not distinguish a harness-stop from an external signal (see the 2026-08-07 case recorded on Issue #1153; `harness-stop` remains part of the vocabulary for other observation channels, but this wording-only check cannot produce it reliably). (c) If the notification wording itself could not be observed at all (e.g. no notification arrived, or the session ended before one could be checked), pass `unobserved` — always pass one of `external-signal` / `indeterminate` / `unobserved` in this branch, never omit the flag, so `unobserved` (wording checked but uninformative/absent) stays distinguishable from `unspecified` (flag not passed at all). (d) Carry the raw wording forward as a one-line `--diagnosis TEXT` value on the Recording call below.
- **Recording (mandatory)**: this is a standalone Bash tool call — it falls under the same pointer file regeneration discipline as Step 1's `run-*.sh` / `run-auto-sub.sh` calls (the PGID pointer written by an earlier Bash call is not visible here), so regenerate it in the same call before invoking the subcommand. After the respawned phase completes, call:
  ```bash
  mkdir -p .tmp
  PGID=$(ps -o pgid= -p $$ | tr -d ' ')
  printf '%s\n' "<literal SESSION_ID value from step 1>" > ".tmp/auto-session-${PGID}"
  bash ${CLAUDE_PLUGIN_ROOT}/scripts/run-auto-sub.sh --write-manual-recovery ISSUE PHASE respawn [EXIT_CODE] [--cause SLUG] [--diagnosis TEXT] [--notification CLASS]
  ```
  ...
  When the root cause of this external-kill respawn is already known (e.g. it matches a previously diagnosed pattern), pass a short kebab-case `--cause SLUG` so `docs/reports/orchestration-recoveries.md` and `collect-recovery-candidates.sh` can group this occurrence separately from other causes of the same symptom; omit `--cause` when the root cause is not yet known. This is independent of `--diagnosis`: for an external-kill respawn, still pass `--diagnosis TEXT` with the raw notification wording captured in clause (d) above, regardless of whether `--cause` is known — omitting `--diagnosis` here (not just `--cause`) would drop the raw wording that clause (d) says to carry forward.
  `--notification CLASS` carries the classification determined in the capture step above (`harness-stop` / `external-signal` / `indeterminate` / `unobserved`) — always pass it for an external-kill respawn, per that step.
...
- **When verdict = `implementation`, `undetermined`, or `no-ci-configured`**: do nothing — proceed to Tier 1 (existing behavior unchanged).
...
1. Collect inputs:
   ...
   - `ci_failure_verdict`: the verdict returned by the CI platform failure pre-check above (`ci-infra`/`implementation`/`undetermined`/`no-ci-configured`), or empty string if the pre-check did not run
...
**Manual recovery hand-off**: If the parent session manually recovers and continues to subsequent phases instead of stopping here, first check whether the recovery reuses a residual worktree left behind by the interrupted phase (e.g. a `merge+pr-N` or `code+issue-N` directory under `.claude/worktrees/`) — if so, follow `modules/worktree-lifecycle.md` § "Manual recovery worktree reuse: fetch and check divergence before continuing" before building or pushing any commit in it, since the worktree's branch may have fallen behind `origin` while the interrupted phase sat idle. Then complete the remaining phases via manual recovery, and call `bash ${CLAUDE_PLUGIN_ROOT}/scripts/run-auto-sub.sh --write-manual-recovery ISSUE PHASE RECOVERY_TYPE [EXIT_CODE] [--cause SLUG] [--diagnosis TEXT] [--notification CLASS]` (where RECOVERY_TYPE describes the action taken, e.g., `push-only`, `pr-create`, `review-rerun`, or `respawn` for the external-kill pre-check above; EXIT_CODE is the original wrapper exit code — if it could not be observed, omit the argument entirely rather than passing the string `unknown`, since `_validate_recovery_args` only accepts a numeric exit code; `--cause SLUG` is a short kebab-case root-cause label and `--diagnosis TEXT` is an optional one-line free-text explanation — pass both when the root cause of this manual recovery is already known, so `docs/reports/orchestration-recoveries.md` and `collect-recovery-candidates.sh` group this occurrence by cause rather than by symptom alone; omit both when the cause is not yet known; `--notification CLASS` is the notification-wording classification captured per the External kill pre-check above — pass it for a `respawn` recovery type, omit it for other recovery types where no notification wording applies). This call is itself a standalone Bash tool call subject to the same pointer file regeneration discipline as Step 1's `run-*.sh` calls (the PGID pointer from an earlier Bash call is not visible here) — regenerate it in the same call before invoking the subcommand:
```bash
mkdir -p .tmp
PGID=$(ps -o pgid= -p $$ | tr -d ' ')
printf '%s\n' "<literal SESSION_ID value from step 1>" > ".tmp/auto-session-${PGID}"
bash ${CLAUDE_PLUGIN_ROOT}/scripts/run-auto-sub.sh --write-manual-recovery ISSUE PHASE RECOVERY_TYPE [EXIT_CODE] [--cause SLUG] [--diagnosis TEXT] [--notification CLASS]
```
The subcommand self-resolves the main repository root regardless of the calling CWD, and records the recovery in two places: `docs/reports/orchestration-recoveries.md` and a `manual_intervention` event in `.tmp/auto-events.jsonl` (see `modules/orchestration-fallbacks.md#manual-recovery-spec-write`). As of #1181, there is no Spec-side write and no open-PR guard — recording happens regardless of whether a PR is open for the issue. Skip this call if the Tier 2 anomaly detector already appended a recovery entry for this event to avoid duplicate entries. Then proceed to Step 5.
...
6. Record the output as `ROUND_LIST`. Read `${CLAUDE_PLUGIN_ROOT}/modules/round-ordering.md` and
   follow its Processing Steps to reorder `ROUND_LIST` (this reordering does not change which
   Issues are in `ROUND_LIST`, only their processing order — see that module's "Relationship to
   the blocked-by gate" section for why this does not conflict with step 4 of List mode below).
   Run:
   ```
...
3. For each issue in `PENDING_LIST`, run `gh issue view $NUMBER --json body -q '.body'` and count (checkbox lines inside a fenced code block are excluded from this count — see `modules/l0-surfaces.md` § AC Enumeration Convention):
...
If `OBSERVATION_MATCHES` is non-empty, read `${CLAUDE_PLUGIN_ROOT}/modules/detect-config-markers.md` and follow the "Processing Steps" section to load `AUTONOMY_TIER` and `OBSERVATION_DISPATCH_THRESHOLD`, then apply tier-aware dispatch:
- **L1**: skip dispatch (advisory-only — the comment already posted by `observation-trigger.sh` is the only action)
- **L2 / L3**: first run `printf '%s\n' "$OBSERVATION_MATCHES" | "${CLAUDE_PLUGIN_ROOT}/scripts/filter-session-verified-issues.sh" --session <literal SESSION_ID value from step 1>` to filter `OBSERVATION_MATCHES`, excluding Issues that already have a `phase=verify` `phase_start`/`phase_complete` event recorded for this `/auto` session (fail-open: passes candidates through unchanged if the session id or events log cannot be resolved). Then exclude numbers already included in `BATCH_LIST` (Issues already processed by this batch) from the filtered result to get `FILTERED_MATCHES`. Run `printf '%s\n' "$FILTERED_MATCHES" | "${CLAUDE_PLUGIN_ROOT}/scripts/rotate-observation-dispatch.sh" --threshold "$OBSERVATION_DISPATCH_THRESHOLD"` to get `DISPATCH_SET`: the script rotates `FILTERED_MATCHES` against a persisted cursor (the last dispatched Issue number, in `.tmp/observation-dispatch-cursor`) before applying the `OBSERVATION_DISPATCH_THRESHOLD` cap, so each dispatch cycle continues from where the previous one left off instead of a fixed subset of Issues permanently occupying every dispatch slot. Then dispatch `Skill(skill="wholework:verify", args="$N --session-id=<literal SESSION_ID value from step 1>")` sequentially for each number in `DISPATCH_SET`. Numbers beyond the cap are not dispatched this run — `observation-trigger.sh` has already posted its notification comment to every matched Issue regardless of the cap, and the deferred Issues are re-matched on the next `auto-run` event scan. When at least one number was deferred (K = count of `FILTERED_MATCHES` not in `DISPATCH_SET`, M = count of `FILTERED_MATCHES`; K > 0), output "Observation dispatch capped at OBSERVATION_DISPATCH_THRESHOLD; deferred K of M matched Issue(s) to the next auto-run scan."
...
**Next-cycle candidate handoff (batch):**

Print `Recommend: run /audit drift to identify next-cycle candidates`. This is the sole, unconditional behavior — it does not branch on autonomy tier or any configuration flag. Selecting next-cycle candidates is the responsibility of an independent `/audit` session; this line is only a handoff pointer to that session.
