...
If `OBSERVATION_MATCHES` is non-empty, read `${CLAUDE_PLUGIN_ROOT}/modules/detect-config-markers.md` and follow the "Processing Steps" section to load `AUTONOMY_TIER` and `OBSERVATION_DISPATCH_THRESHOLD`, then apply tier-aware dispatch:
- **L1**: skip dispatch (advisory-only — the comment already posted by `observation-trigger.sh` is the only action)
- **L2 / L3**: first run `printf '%s\n' "$OBSERVATION_MATCHES" | "${CLAUDE_PLUGIN_ROOT}/scripts/filter-session-verified-issues.sh" --session <literal SESSION_ID value from step 1>` to filter `OBSERVATION_MATCHES`, excluding Issues that already have a `phase=verify` `phase_start`/`phase_complete` event recorded for this `/auto` session (fail-open: passes candidates through unchanged if the session id or events log cannot be resolved). Then exclude numbers already included in `BATCH_LIST` (Issues already processed by this batch) from the filtered result. From the remaining numbers (already ascending-sorted by `observation-trigger.sh`, i.e. oldest-pending Issue first), take at most the first `OBSERVATION_DISPATCH_THRESHOLD` and dispatch `Skill(skill="wholework:verify", args="$N --session-id=<literal SESSION_ID value from step 1>")` sequentially for each. Numbers beyond the cap are not dispatched this run — `observation-trigger.sh` has already posted its notification comment to every matched Issue regardless of the cap, and the deferred Issues are re-matched on the next `auto-run` event scan. When at least one number was deferred, output "Observation dispatch capped at OBSERVATION_DISPATCH_THRESHOLD; deferred K of M matched Issue(s) to the next auto-run scan." (K = deferred count, M = total remaining after session-verified and `BATCH_LIST` exclusion).
