...
- **Detection signature**: the background `run-*.sh` exited with code 137 (SIGKILL — the EXIT trap cannot run under SIGKILL, so this alone is conclusive), OR it exited with code 143 (SIGTERM) **and** `.tmp/wrapper-out-$NUMBER-$PHASE.log` has no `Exit code:` trailer line **and** `.tmp/auto-events.jsonl` has no `wrapper_exit` event for this phase. The second branch requires both markers to be missing because a SIGTERM whose EXIT trap fired normally already leaves them (`_maybe_emit_phase_complete()` backfills on exit 143 too — see `docs/reports/external-kill-investigation.md` F2), so exit code 143 alone is not sufficient evidence the trap never ran. Either signature means the wrapper's own process group was killed externally without emitting its usual completion markers.
